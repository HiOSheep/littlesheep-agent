...
import { WebhookChannelPlugin, pluginManifest, littleSheepPlugin } from '@littlesheep/channel-webhook'
...
describe('e2e: plugin webhook channel', () => {
  let tmpDir: string
  let host: PluginHost
  ...
  beforeEach(() => {
    ...
    host = createPluginHost({
      ...
      pluginInstallDir: join(tmpDir, 'plugins'),
      pluginDataDir: join(tmpDir, 'plugin-data'),
      builtinSources: [{ kind: 'builtin', manifest: pluginManifest, load: async () => littleSheepPlugin }],
      log: () => {}, // 静默日志，避免污染测试输出
    })
  })

  afterEach(async () => {
    if (host.started) await host.stop()
  ...
  it('场景 1：启动 webhook 渠道并暴露监听地址', async () => {
    ...
    const plugins = host.listChannels()
  ...
  it('场景 2：健康检查 GET /webhook 返回 200', async () => {
    await host.start()
    const wh = host.listChannels().find(p => p.type === 'webhook') as WebhookChannelPlugin
  ...
  it('场景 5：Bearer 认证 — 无/错 token 返回 401，正确 token 返回 200', async () => {
    ...
    host.setConfig(makeConfig({ id: 'test-webhook-auth', bearerToken: 'secret-token-123' }))
    await host.reload()
