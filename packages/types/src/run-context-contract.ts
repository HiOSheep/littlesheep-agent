...
export const runContextFieldOwnership: readonly RunContextFieldContract[] = Object.freeze([
  field({
    ...
    writeStages: ['decide', 'execute', 'verify', 'recover', 'reply', 'ask_user', 'runner-restore'],
    lifecycle: 'run-local',
  ...
  field({
    ...
    writeStages: ['decide', 'execute', 'verify', 'recover', 'reply', 'ask_user', 'finalize', 'runner-restore'],
  ...
  field({
    ...
    writeStages: ['classify', 'decide', 'execute', 'recover', 'verify', 'evolve', 'capture', 'reply', 'ask_user'],
    lifecycle: 'run-local',
  ...
  field({
    field: 'workPolicyUpgradeRequest',
    group: 'replan',
    owner: 'bounded-loop-upgrade-boundary',
    readStages: ['execute', 'decide', 'runner-restore'],
    writeStages: ['execute', 'decide', 'runner-restore'],
    lifecycle: 'checkpoint-carried',
    purpose: 'Validated request to promote an unfinished bounded loop into a TaskBook without replaying completed work.',
  }),
  field({
    field: 'replanHistory',
    group: 'replan',
  ...
  field({
    field: 'capabilitySnapshot',
    group: 'runtimeControl',
    owner: 'runtime-capability-boundary',
    readStages: [...coreStages, 'runner-restore'],
    writeStages: ['runner-init', 'runner-restore'],
    lifecycle: 'run-local',
    purpose: 'Versioned path-free capability, permission and provider facts used to answer capability questions truthfully.',
  }),
  field({
    field: 'capabilityProbe',
    group: 'runtimeControl',
    owner: 'runtime-capability-boundary',
    readStages: [...coreStages, 'runner-restore', 'post-run'],
    writeStages: ['classify', 'runner-init', 'runner-restore'],
    lifecycle: 'run-local',
    purpose: 'Durable proof that a capability probe was actually observed in this run.',
  }),
  field({
    field: 'capabilityPermissionEvent',
    group: 'runtimeControl',
    owner: 'runtime-capability-boundary',
    readStages: [...coreStages, 'runner-restore', 'post-run'],
    writeStages: ['classify', 'runner-init', 'runner-restore'],
    lifecycle: 'run-local',
    purpose: 'Permission decision paired with the Runtime capability observation.',
  }),
  field({
