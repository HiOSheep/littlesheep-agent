...
describe('CACHE-08 request shape and lifecycle matrix', () => {
  ...
  it('keeps attachment manifests in the dynamic suffix without invalidating the stable prefix', () => {
    ...
    // Index 1 is the per-run Runtime capability block, index 2 the environment
    // brief; the attachment manifest follows both in the conversation.
    expect(String(prepared.messages[3]?.content)).toContain('Attached files manifest');
