...
export const LLM_CALL_CONTRACT_TEMPLATES: Readonly<Record<LlmCallPurpose, LlmCallContractTemplate>> = {
  classify: template({
    ...
    maxIterations: 0, maxAttempts: 1, maxOutputTokens: 200, maxPromptTokens: 4_096, temperature: 0,
  }),
  decide: template({
  ...
  execute_tool_loop: template({
    ...
    history: 'recent', attachments: 'images_and_manifest',
  ...
  ask_user: template({
    ...
    memoryIntents: NO_MEMORY, requiresMemoryEvidence: false, toolMode: 'none', runtimeApprovalRequired: false,
    maxIterations: 0, maxAttempts: 2, maxOutputTokens: 640, maxPromptTokens: 4_096, temperature: 0.65,
  }),
  finalize: template({
