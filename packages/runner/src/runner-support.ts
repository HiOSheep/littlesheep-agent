...
export function assembleResult(
  ...
  return {
    ...
    messages: ctx.produced ? [...ctx.produced] : [],
    trace,
    durationMs: Date.now() - startedAtMs,
    usage: ctx.usage ? structuredClone(ctx.usage) : undefined,
    ...
    // Snapshot arrays so post-result derived work (session compaction) cannot
    // retroactively change the run's published evidence or cost attribution.
    modelRequests: ctx.modelRequests ? [...ctx.modelRequests] : undefined,
    contextSnapshots: ctx.contextSnapshots ? [...ctx.contextSnapshots] : undefined,
