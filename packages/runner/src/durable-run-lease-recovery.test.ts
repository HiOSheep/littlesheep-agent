...
describe('durable run lease process recovery', () => {
  it('reclaims ownership after a renewing OS process is killed', async () => {
    const rootDir = await mkdtemp(join(tmpdir(), 'ls-durable-run-lease-kill-'));
    roots.push(rootDir);
    const child = spawn(process.execPath, [
      resolve('node_modules/vitest/vitest.mjs'),
      'run',
      resolve('packages/runner/src/durable-run-lease-crash-worker.test.ts'),
      '--pool=threads',
      '--maxWorkers=1',
      '--minWorkers=1',
    ], {
      cwd: resolve('.'),
      env: { ...process.env, LS_DURABLE_RUN_LEASE_CRASH_ROOT: rootDir },
      stdio: ['ignore', 'pipe', 'pipe'],
    });
    try {
      await waitForChildMarker(child, 'LS_DURABLE_RUN_LEASE_RENEWED');
    } finally {
      child.kill('SIGKILL');
    ...
    const inherited = await store.read('session-killed', 'run-killed');
    expect(inherited).toMatchObject({ status: 'active', attempts: 1 });
    await expect(store.acquire('session-killed', 'run-killed')).resolves.toMatchObject({ kind: 'conflict' });
    await delay(Math.max(0, Date.parse(inherited?.leaseUntil ?? '') - Date.now() + 25));

    const reclaimed = await store.acquire('session-killed', 'run-killed');
    expect(reclaimed).toMatchObject({
      kind: 'reclaimed',
      lease: { status: 'active', attempts: 2 },
    });
    expect(reclaimed.lease.ownerToken).not.toBe(inherited?.ownerToken);
    await store.release('session-killed', 'run-killed', reclaimed.lease.ownerToken!);
  }, 30_000);
});
