...
function validateDisposition(value: unknown, expectedId: string): RunCheckpointDisposition {
  ...
  if (record.status !== 'resuming' && record.status !== 'resumed' && record.status !== 'abandoned') throw new Error('invalid disposition status')
  ...
  return {
    ...
    history: history.map((item) => {
      ...
      if (entry.status !== 'resuming' && entry.status !== 'resumed' && entry.status !== 'abandoned') throw new Error('invalid disposition history status')
      return {
        status: entry.status,
