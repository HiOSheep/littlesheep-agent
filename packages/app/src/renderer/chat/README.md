...
每条消息下面那一行的顺序是**离开自己气泡的方向**读：agent 的回复是 `[复制][分叉][时间]`、靠左；用户的消息镜像成 `[时间][分叉][复制]`、靠右。复制字形重画成两张圆角纸（`ui/message-icons.tsx`：先画后面那张，前面那张用 `copy-icon-front` 填自己的底色，所以后张的线在前张边缘处停住，不再交叉）；时间字号与旁边的用量数字一致（11px），不再比它大一号。分叉按钮调用 `branchConversationFromMessage`：Main 在 `POST /sessions/:id/branch` 建一个**真实会话**（同一套 sessionManager，写入到该消息为止的全部消息，标题是原名加「（分支）」），随后前端刷新列表并切进这个分支——同一条对话从此有两个未来。
