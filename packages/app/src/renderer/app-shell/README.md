...
- `preferences.ts`、`list-motion.ts`：Renderer 偏好和列表过渡的纯客户端辅助。

新增业务行为应进入对应领域控制器，不要继续扩大 `use-app-controller.ts`；修改导航时必须回归设置、拓展工作区、标签和重启恢复。

## 软上限说明

`use-app-controller.ts` 暂时保留 300-600 行兼容协调代码，因为它只负责装配各领域状态、启动恢复 effect 和公开视图所需快照。进一步拆分必须等 Runtime/附件 effect 的输入输出稳定；在此之前禁止把新的领域行为写回该文件，并由仓库质量门锁定当前上限。
