...
describe('workspace code editor unification', () => {
  it('keeps Monaco loading, theme, and defaults in one workspace primitive', async () => {
    ...
    expect(primitive).toContain('continuing without enhanced syntax support')
    expect(primitive).toContain('if (!monacoReady) return <>{loading}</>')
    ...
    expect(primitive).toContain('automaticLayout: false')
    expect(primitive).toContain('trackWorkspaceEditorLayout')
    expect(primitive).toContain('COLUMN_RESIZE_END_EVENT')
    expect(primitive).toContain("document.body.classList.contains('is-resizing-column')")
