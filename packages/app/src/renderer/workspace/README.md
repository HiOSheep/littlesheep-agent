...
- **HTML 预览的当前实测边界**（`html-preview.tsx`，UX-24 基线，`pnpm run verify:html-preview-baseline`）：`srcdoc` 会剥离脚本、注入 CSP 与 `file:` base、`sandbox=""`，但**同时丢掉了文档的 `<style>` 与 `<title>`**（DOMPurify 默认按片段处理，`<head>` 内容不保留），所以重样式的页面在预览里退化成无样式黑字白底；本地 `file:` 图片会发起请求但拿不到解码结果（`naturalWidth=0`）。另外从文件树打开的**后续** HTML 文件存在竞态：帧保留创建时的空文档（`readyState=complete`、正文为空），此刻元素 `srcdoc` 已有正文却不重新导航，等 6 秒或切标签都不恢复——用户报告的“全白”就是这种形态。修这两条属于 UX-25；`<style>` 与空帧的期望值都钉在该门的断言里，改动实现时必须同步更新门。
