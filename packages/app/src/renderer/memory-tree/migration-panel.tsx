  ...
  return (
    <div className="memory-migration-panel">
      <div className="memory-migration-status-line">
        ...
        <button type="button" disabled={loading} onClick={onRefresh}>{loading ? '检查中' : '重新检查'}</button>
      </div>
      ...
      {preflight ? (
        <>
          <dl className="memory-migration-metrics">
            ...
            <div><dt>迁移阶段</dt><dd>{preflight.phase ? migrationPhaseLabel(preflight.phase) : '未开始'}</dd></div>
          ...
          {preflight.error && <p className="memory-project-callout conflict">{preflight.error}</p>}
        </>
      ) : (
...
function migrationPhaseLabel(phase: NonNullable<MemoryV3MigrationPreflightOverview['phase']>): string {
  return ({
