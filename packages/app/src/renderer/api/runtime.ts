...
export async function getProviders(): Promise<ProviderInfo[]> {
  const res = await localApiFetch(LOCAL_APP_API_ROUTES.configProviders)
...
export async function deleteProvider(providerId: string): Promise<RuntimeState> {
  const res = await localApiFetch(`${LOCAL_APP_API_ROUTES.configProviders}/${encodeURIComponent(providerId)}`, {
    method: 'DELETE',
  })
  if (!res.ok) {
    const data = await res.json().catch(() => ({ error: `Local app API error: ${res.status}` }))
...
export async function getRuntime(): Promise<RuntimeState> {
  const res = await localApiFetch(LOCAL_APP_API_ROUTES.runtime)
  if (!res.ok) throw localApiStatusError(res.status)
  return res.json() as Promise<RuntimeState>
}
...
export async function clearWebCache(): Promise<{ ok: boolean; cleared: boolean }> {
  const res = await localApiFetch(LOCAL_APP_API_ROUTES.webCache, { method: 'DELETE' })
...
export async function checkWebProvider(): Promise<RuntimeState> {
  const res = await localApiFetch(LOCAL_APP_API_ROUTES.webProviderCheck, { method: 'POST' })
  if (!res.ok) {
    const data = await res.json().catch(() => ({ error: `Local app API error: ${res.status}` }))
    throw new Error((data as { error: string }).error)
  }
...
export async function getDataRootStatus(): Promise<DataRootStatus> {
  const res = await localApiFetch(LOCAL_APP_API_ROUTES.dataRoot)
  return parseDataRootResponse(res)
}

export async function selectDataRootTarget(): Promise<string | null> {
  const res = await localApiFetch(LOCAL_APP_API_ROUTES.dataRootSelect, { method: 'POST' })
  if (!res.ok) throw await localApiResponseError(res)
...
export async function requestDataRootMigration(targetDir: string): Promise<DataRootStatus> {
  const res = await localApiFetch(LOCAL_APP_API_ROUTES.dataRootMigration, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function cancelDataRootOperation(): Promise<DataRootStatus> {
  const res = await localApiFetch(LOCAL_APP_API_ROUTES.dataRootMigration, { method: 'DELETE' })
  return parseDataRootResponse(res)
}

export async function requestDataRootRollback(): Promise<DataRootStatus> {
  const res = await localApiFetch(LOCAL_APP_API_ROUTES.dataRootRollback, { method: 'POST' })
  return parseDataRootResponse(res)
}

export async function restartApplication(): Promise<void> {
  const res = await localApiFetch(LOCAL_APP_API_ROUTES.applicationRestart, { method: 'POST' })
  if (!res.ok) throw await localApiResponseError(res)
}
...
export async function saveApiKey(envVar: string, key: string): Promise<void> {
  const res = await localApiFetch(LOCAL_APP_API_ROUTES.configApiKey, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function saveWebProvider(key: string): Promise<RuntimeState> {
  const res = await localApiFetch(LOCAL_APP_API_ROUTES.configWebProvider, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
