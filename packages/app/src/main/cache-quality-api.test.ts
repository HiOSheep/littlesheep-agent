...
describe('cache quality Local App API', () => {
  it('returns the authorized scope report and fails closed without scope inputs', async () => {
    ...
    const loadSessionDurableProjection = vi.fn(async () => ({
      modelRequests: [{
        requestId: 'request-1',
    ...
    try {
      ...
      expect(report).toHaveBeenCalledWith({
        ...
        verifications: [expect.objectContaining({ attempt: 1, verdict: 'pass' })],
        since: Date.parse('2026-09-09T00:00:00.000Z'),
        until: Date.parse('2026-09-09T01:00:00.000Z'),
      })
      expect(loadSessionDurableProjection).toHaveBeenCalledWith('session-a')
