...
- `src/v3/` 拥有 Memory v3 的原始数据记录、atom projections、有界事件/操作 journal、SQLite catalog、FTS/向量、启动恢复协调、有界维护 worker、实体关系与引用治理。原始数据记录是事件记录的权威来源，但不替内容作事实判断；atom 是可治理投影，journal 只负责恢复。
...
- 读取遵循根索引、分支索引、按需展开和分支内深搜；首次请求只选择有界 D2 working set，执行中可 release 当前 atom。写入先捕获只追加的原始数据记录，再更新带 parent、scope、tier、来源和认识状态的 atom 投影。
...
- Memory v3 的原始数据记录保留/孤儿恢复、10,000 atom 扫描、catalog 重建、离线向量批处理、due 补偿、关系引用治理、边界检索和故障重放测试位于 `src/v3/*.test.ts` 与 `src/memory-repository/v3-*.test.ts`。
