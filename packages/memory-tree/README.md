...
- 普通 GUI 不展示 Atom、关系、向量或压缩投影，只展示应用数据根中的记忆文件；Runtime 仍保留完整结构，LLM 通过有界根索引按需展开 Atom。
