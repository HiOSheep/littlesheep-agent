...
- `src/v3/` 是尚未接管正式运行路径的 Memory v3 隔离数据层：契约、原子文件、事件/操作 journal、SQLite catalog、实体关系、Embedding 端口、优先级和恢复协调器均在这里；v2→v3 适配与正式迁移尚未启用。
...
- Memory v3 的隔离契约、10,000 atom 扫描、catalog 重建、边界检索和故障重放测试位于 `src/v3/*.test.ts`。
