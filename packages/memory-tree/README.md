...
- `src/v3/` 拥有 Memory v3 的 atom 文件、事件/操作 journal、SQLite catalog、FTS/向量、有界维护 worker、实体关系与引用治理。
...
- 读取遵循根索引、分支索引、按需展开和分支内深搜；写入记录 parent、scope、tier、来源、认识状态和理由。
...
- Memory v3 的 10,000 atom 扫描、catalog 重建、离线向量批处理、due 补偿、关系引用治理、边界检索和故障重放测试位于 `src/v3/*.test.ts` 与 `src/memory-repository/v3-*.test.ts`。
