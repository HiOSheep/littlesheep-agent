...
import type { MemoryNode, MemoryTreeDocument, MemoryWritePolicy } from '../types.js';
...
export async function validateMemoryV3Stage(
  ...
  try {
    ...
    if (scan.issues.length > 0) throw new Error(`Memory v3 staging scan found ${scan.issues.length} invalid atom(s).`);
    ...
    for (const node of publicMemoryV2Nodes(source)) {
      ...
      if (!atom) throw new Error(`Memory v3 staging is missing atom ${node.id}.`);
