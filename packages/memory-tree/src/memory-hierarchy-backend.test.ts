import { afterEach, describe, expect, it } from 'vitest';
import { mkdtemp, rm } from 'node:fs/promises';
import { join } from 'node:path';
import { InjectionTier, type MemoryWriteIntent } from './types.js';
import type { MemoryEntity, MemoryRelation } from './v3/contracts.js';
import { createMemoryV3ExperimentMarker } from './memory-repository.js';
import { createMemoryRepositoryManagementFacade } from './memory-repository/management.js';
import { MemoryRepositoryV3Backend } from './memory-repository/v3-backend.js';
import { resolveMemoryWritePolicy } from './memory-repository/write-policy.js';
import { MemoryAtomHierarchyService } from './memory-hierarchy.js';

describe('MemoryAtomHierarchyService with the V3 backend', () => {
  const directories: string[] = [];
  const backends: MemoryRepositoryV3Backend[] = [];

  afterEach(async () => {
    for (const backend of backends.splice(0)) backend.close();
    for (const directory of directories.splice(0)) {
      await rm(directory, { recursive: true, force: true });
    }
  });

  it('reparents an evidenced leaf and restores the projection, relation and audit after restart', async () => {
    const dataDir = await mkdtemp(join(process.env.TEMP ?? process.env.TMP ?? '.', 'ls-memory-hierarchy-'));
    directories.push(dataDir);
    await createMemoryV3ExperimentMarker(dataDir);
    let backend = new MemoryRepositoryV3Backend({ dataDir, policy: resolveMemoryWritePolicy() });
    backends.push(backend);
    await backend.initialize();

    const storageScopeKey = await backend.ledger.storageScopeKey('project', 'project-hierarchy');
    const childEntity = entity('entity:hierarchy-child', storageScopeKey);
    const parentEntity = entity('entity:hierarchy-parent', storageScopeKey);
    await backend.graphStore.upsertEntity(childEntity);
    await backend.graphStore.upsertEntity(parentEntity);
    const relation = parentRelation(childEntity.id, parentEntity.id, storageScopeKey);
    await backend.graphStore.upsertRelation(relation);

    const oldParent = await writeAtom(backend, {
      id: 'hierarchy-old-parent',
      parentNodeId: 'project:root',
      summary: 'Old hierarchy parent',
      content: 'The old hierarchy parent is retained for the reparent test.',
      retrievalKeys: ['hierarchy', 'old-parent'],
      entityRefs: [],
      relationRefs: [],
      scopeKey: 'project-hierarchy',
      sourceRunId: 'run-hierarchy-old-parent',
    });
    const newParent = await writeAtom(backend, {
      id: 'hierarchy-new-parent',
      parentNodeId: 'project:root',
      summary: 'New hierarchy parent',
      content: 'The new hierarchy parent is the relation-backed destination.',
      retrievalKeys: ['hierarchy', 'new-parent'],
      entityRefs: [parentEntity.id],
      relationRefs: [],
      scopeKey: 'project-hierarchy',
      sourceRunId: 'run-hierarchy-new-parent',
    });
    const child = await writeAtom(backend, {
      id: 'hierarchy-child',
      parentNodeId: oldParent.id,
      summary: 'Hierarchy child',
      content: 'This leaf belongs to the new hierarchy parent.',
      retrievalKeys: ['hierarchy', 'child'],
      entityRefs: [childEntity.id],
      relationRefs: [relation.id],
      scopeKey: 'project-hierarchy',
      sourceRunId: 'run-hierarchy-child',
    });

    const service = new MemoryAtomHierarchyService({
      management: createMemoryRepositoryManagementFacade(backend),
    });
    const currentChild = await backend.atomStore.read(child.id);
    const currentParent = await backend.atomStore.read(newParent.id);
    expect(currentChild).toBeTruthy();
    expect(currentParent).toBeTruthy();

    const [result] = await service.reparent([{
      id: 'run:hierarchy:reparent:1',
      action: 'move',
      basis: 'explicit-parent-relation',
      atom: { atomId: child.id, expectedRevision: currentChild!.revision },
      parent: { atomId: newParent.id, expectedRevision: currentParent!.revision },
      relationId: relation.id,
      reason: 'The active ownership relation proves the corrected hierarchy parent.',
      evidenceRefs: ['run:hierarchy:verification:1:pass'],
    }]);

    expect(result).toMatchObject({
      status: 'committed',
      committed: true,
      atomId: child.id,
      parentAtomId: newParent.id,
    });
    await expect(backend.atomStore.read(child.id)).resolves.toMatchObject({
      parentId: newParent.id,
      revision: currentChild!.revision + 1,
    });
    expect((await backend.inspectNodeForManagement(child.id, 'D3'))?.projectionRecords)
      .toEqual(expect.arrayContaining([
        expect.objectContaining({
          event: expect.objectContaining({
            payload: { atomManagement: expect.objectContaining({ action: 'move' }) },
          }),
        }),
      ]));

    backend.close();
    backends.splice(backends.indexOf(backend), 1);
    backend = new MemoryRepositoryV3Backend({ dataDir, policy: resolveMemoryWritePolicy() });
    backends.push(backend);
    await backend.initialize();

    const restored = await backend.inspectNodeForManagement(child.id, 'D3');
    expect(restored).toMatchObject({
      atom: {
        id: child.id,
        parentId: newParent.id,
        revision: currentChild!.revision + 1,
      },
      catalog: { parentId: newParent.id, revision: currentChild!.revision + 1 },
      history: { atomId: child.id, revision: currentChild!.revision + 1 },
    });
    expect(restored?.projectionRecords).toEqual(expect.arrayContaining([
      expect.objectContaining({
        event: expect.objectContaining({
          payload: { atomManagement: expect.objectContaining({ action: 'move' }) },
        }),
      }),
    ]));
    expect(restored?.neighborhood?.relations).toEqual(expect.arrayContaining([
      expect.objectContaining({
        id: relation.id,
        fromEntityId: childEntity.id,
        toEntityId: parentEntity.id,
        type: 'belongs-to',
      }),
    ]));
    expect((await backend.managementStatus()).catalog?.integrity).toBe('ok');
  });
});

async function writeAtom(
  backend: MemoryRepositoryV3Backend,
  options: {
    id: string;
    parentNodeId: string;
    summary: string;
    content: string;
    retrievalKeys: string[];
    entityRefs: string[];
    relationRefs: string[];
    scopeKey: string;
    sourceRunId: string;
  },
) {
  const result = await backend.write({
    id: options.id,
    branch: 'project',
    parentNodeId: options.parentNodeId,
    scope: 'project',
    scopeKey: options.scopeKey,
    tier: InjectionTier.T2_RELEVANT,
    summary: options.summary,
    content: options.content,
    retrievalKeys: options.retrievalKeys,
    sourceRunId: options.sourceRunId,
    sourceStage: 'tool',
    importance: 0.9,
    confidence: 0.95,
    reason: 'The hierarchy integration fixture is supported by the test setup.',
    epistemic: {
      domain: 'project',
      statementKind: 'factual-claim',
      epistemicStatus: 'verified',
      authorityScope: { kind: 'tool-evidence', scope: 'project', scopeKey: options.scopeKey, topics: ['hierarchy'] },
      assertedBy: { kind: 'tool', id: 'hierarchy-integration-test' },
      entityRefs: options.entityRefs,
      relationRefs: options.relationRefs,
    },
  } satisfies MemoryWriteIntent);
  if (!result.node) throw new Error(`Fixture atom was not created: ${result.reason}`);
  return result.node;
}

function entity(id: string, scopeKey: string | undefined): MemoryEntity {
  return {
    version: 1,
    id,
    type: 'concept',
    owner: { kind: 'agent', id: 'ls' },
    scope: 'project',
    scopeKey,
    externalKey: id,
    label: id,
    aliases: [],
    status: 'active',
    revision: 1,
    createdAt: '2026-07-17T09:00:00.000Z',
    updatedAt: '2026-07-17T09:00:00.000Z',
  };
}

function parentRelation(fromEntityId: string, toEntityId: string, scopeKey: string | undefined): MemoryRelation {
  return {
    version: 1,
    id: 'relation:hierarchy-child-belongs-to-parent',
    fromEntityId,
    toEntityId,
    type: 'belongs-to',
    scope: 'project',
    scopeKey,
    source: { kind: 'tool', id: 'hierarchy-integration-test' },
    sourceRefs: ['conversation-source:hierarchy-integration'],
    evidenceRefs: ['tool:hierarchy-integration'],
    confidence: 0.95,
    authorityScope: { kind: 'tool-evidence', scope: 'project', scopeKey, topics: ['hierarchy'] },
    relevance: 0.9,
    feedbackRevision: 0,
    recentFeedbackIds: [],
    status: 'active',
    resolutionStatus: 'resolved',
    revision: 1,
    createdAt: '2026-07-17T09:00:00.000Z',
    updatedAt: '2026-07-17T09:00:00.000Z',
  };
}
