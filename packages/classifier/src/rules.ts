...
const MEMORY_RECALL_PATTERNS: readonly RegExp[] = [
  ...
  /(?:请|麻烦(?:你)?|帮我)?(?:分别|准确|直接)?(?:回答|说出|列出|输出|返回|告诉我|回忆)(?:[^。！？!?\r\n]{0,64})(?:最初|起初|原先|原来|先前|此前|之前|上次|上一轮|后来|后续)(?:让(?:我|你)|要求你|请你)?(?:保存|记录|约定|设定|确定|提到|说过|告诉)(?:的|过的)/iu,
  ...
  /(?:answer|state|list|output|return|tell\s+me|recall).{0,64}(?:initially|originally|previously|earlier|later).{0,24}(?:saved|recorded|agreed|set|specified|mentioned|said)/iu,
];
