...
async function main() {
  ...
  try {
    ...
    await writeFile(join(outDir, 'cold-start-interaction.json'), `${JSON.stringify({
      ...
      failures,
      gaps: [
        'a genuinely slow bootstrap: the not-ready window is lengthened through the acceptance-only readiness-delay knob (LITTLESHEEP_ACCEPTANCE_READY_DELAY_MS), which postpones publication instead of slowing the real initialization',
