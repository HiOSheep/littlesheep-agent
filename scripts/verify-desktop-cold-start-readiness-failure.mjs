...
async function main() {
  ...
  await writeEvidence(outDir, {
    ...
    limits: [
      ...
      'Retry is exercised in this run: the ledger records a concurrent first attempt (attemptsUsed 2), an in-flight refusal, and the recovered attempt reaching ready, so the bounded retry entry is asserted rather than left open.',
