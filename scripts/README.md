...
`verify:workspace-large-directory` 在 1,000/10,000 个文件的隔离目录中检查 Main 的 320 行上限、筛选后再截断带来的上限外文件可达性、逐键稳定时间、320 行 Tab 导航及滚动布局成本；门里同时记录"不做虚拟化"的依据（未截断时的创建/布局成本随行数单调上升）。（第二个工作区根与跨重启会话级现场的扩展这一轮未做成门，原因与下一步见[项目状态](../docs/decision/project-status.md) 的"同一批验收记录的开放边界"。）
