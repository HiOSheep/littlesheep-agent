...
**当前阶段**：Memory v3 阶段 0-5 已在隔离目录完成；阶段 6 已完成同源 D0-D3 管理读取、启动前迁移/回滚、实时回滚安全预检、本地向量模型资产检查与显式准备、atom 移动/合并/失效/恢复/导出、首次自动选择、运行中 release、投影变更记录孤儿恢复、对话原始来源持久化、验证反馈、commit receipt、隔离 soak、真实 Electron 窗口验收，以及正式 V2 数据副本上的迁移、Runner 会话、EVOLVE/CAPTURE 写入、重启检索和回滚边界演练。当前 Memory v2 仍是正式运行路径，正式用户数据尚未迁移。
...
- 当前正式 V2 数据已通过只读预检和隔离副本演练：40 个业务节点、11 个资源完整迁移，5 个内部 scope root 与业务 atom 分开计数；无新增写入时 V3 重启读取、catalog 完整性和回滚均通过。隔离副本重新迁移后，真实 Runner 又完成会话、EVOLVE/CAPTURE、重启恢复、索引导航、release 后 FTS 重新介入，并验证新权威写入会在登记前关闭回滚；正式源哈希未变化，临时副本已删除。
...
- 迁移页会独立显示固定本地 BGE 模型的资产状态。模型缺失不阻断安全迁移，层级导航与 FTS 仍可用；用户可显式开始或取消约 23.3 MB 的固定 revision 下载，应用不会在记忆写入或检索时暗中联网。模型仍在准备时，UI 与 Local App API 都会拒绝登记迁移，避免重启与下载发生竞态。
