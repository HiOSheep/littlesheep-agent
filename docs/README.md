...
**当前阶段**：仓库基元化阶段 0-7 已完成；用户已经确认 Memory v3 采用“原子记忆文件 + 层级 parent + 内置本地向量目录 + 实体/关系边界”的方向。当前 Memory v2 仍安全保留，正式用户数据尚未迁移。

**推荐下一步**：先完成 Memory v3 阶段 0-2，在隔离目录中建立原子文件、SQLite catalog、实体/有向关系、FTS、本地 Embedding 和恢复日志；随后再进行真实 Provider 对话校准。
