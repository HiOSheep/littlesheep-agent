...
**当前阶段**：Memory v3 阶段 0-2 已在隔离目录完成。阶段 2 已交付真实本地 Embedding、显式模型资产校验、有界向量重建、due 启动补偿和实体/关系引用治理；当前 Memory v2 仍是正式运行路径，正式用户数据尚未迁移。

**推荐下一步**：进入 Memory v3 阶段 3，在 feature flag 下建立 v3 Repository facade、Context/KnownState、工具与 UI 适配；同时把真实 Provider 对话校准作为独立验收门，不触碰正式用户数据迁移。
