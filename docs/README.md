...
**当前阶段**：Memory v3 阶段 0-1 已在隔离目录完成，阶段 2 已完成 atom catalog、FTS、向量端口、恢复协调器和边界测试；当前 Memory v2 仍是正式运行路径，正式用户数据尚未迁移。

**推荐下一步**：完成 Memory v3 阶段 2 剩余的本地神经 Embedding 候选基准、后台向量重建和 due 补偿消费者；确认阶段 2 后进行真实 Provider 对话校准，再进入 v3 facade 适配与安全迁移。

**你现在不需要追加决策**：默认按已确认方案继续设计和实现，但不触碰正式用户记忆。

- Provider `/embeddings` 已在 v3 基础设施中默认硬关闭；真实本地神经 Embedding 模型仍待基准选型，不能把测试引擎视为产品能力。
