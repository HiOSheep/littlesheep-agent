...
- [当前基线（回放与压缩修复后）2026-09-22](reference/cache-baseline/real-long-task-baseline-after-replay-fixes-2026-09-22.md)：再复测同一冻结清单——平均 H_ui **85.96%**（敏感性视图 87.2%，即排除 provider 上报矛盾后的读数），功能失败 1/12，usage 完整 12/12，仍 0/12 达标；逐次运行同时给出"排除 provider 矛盾请求"的敏感性列，[机器可读账本](reference/cache-baseline/real-long-task-baseline-after-replay-fixes-2026-09-22.json)同步提交。
