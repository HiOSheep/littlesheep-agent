...
**当前阶段**：Memory v3 阶段 0-3 已在隔离目录完成。阶段 3 已交付 feature flag 双后端 facade、保守认识状态分类、稳定实体映射、Repository 事务、Memory Service/Runner 兼容和重启恢复；当前 Memory v2 仍是正式运行路径，正式用户数据尚未迁移。

**推荐下一步**：进入 Memory v3 阶段 4，只在隔离数据根实现 v2 只读快照、atom 生成、目录校验、故障注入、可恢复切换和回滚；阶段 5 再统一检索、证据封套与 KnownState，阶段 6 才建设 atom 管理 UI 和申请正式迁移批准。真实 Provider 对话校准继续作为独立验收门。
