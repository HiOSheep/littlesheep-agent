...
**当前阶段**：Memory v3 阶段 0-5 已在隔离目录完成；阶段 6 已完成同源 D0-D3 管理读取、启动前迁移/回滚、atom 移动/合并/失效/恢复/导出、首次自动选择、运行中 release 和 immutable fact 孤儿恢复。当前 Memory v2 仍是正式运行路径，正式用户数据尚未迁移。

**推荐下一步**：完成 Memory v3 隔离长时间运行与真实窗口验收；之后再请求用户明确批准正式数据迁移。真实 Provider 对话校准继续作为独立验收门。
...
- Immutable facts 是不可修改的事件真相源；atom 是可治理投影；SQLite 向量目录管理 atom 的路径、层级、FTS、向量、状态和审计，但必须能从持久文件重建。
