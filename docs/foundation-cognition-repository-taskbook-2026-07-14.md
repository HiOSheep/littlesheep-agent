...
建议新增一个用户可编辑、运行时可注册的 `PHILOSOPHY.md`，保存经过用户确认的长期理念。它与 `USER.md`、`SOUL.md`、`AGENTS.md` 分工如下：
...
该新增文件在正式实现前仍属于待决策项，不能只创建文件而不接入注册、Context 预算、UI 管理和迁移协议。
...
| 缺口 | 当前证据 | 风险 |
| --- | --- | --- |
| 大型组合文件 | 33 个非测试生产文件超过 300 行 | 修改冲突、职责漂移、难以并行开发 |
| Renderer 总控过重 | `App.tsx` 约 9342 行 | UI、导航、会话、设置和工作区相互牵连 |
| Local App API 过重 | `local-app-api-server.ts` 约 2601 行 | 路由、服务、资源生命周期和协议难以独立验证 |
| Renderer API 过重 | `renderer/api.ts` 约 1368 行 | 类型与领域请求集中，修改容易产生无关冲突 |
| Memory 边界仍大 | `memory-repository.ts` 约 1192 行，`memory-service.ts` 约 1043 行 | 存储、迁移、索引和消费职责难以单独演进 |
...
| 编号 | 决策 | 推荐选项 | 影响 |
| --- | --- | --- | --- |
| D1 | 是否新增 `PHILOSOPHY.md` | 新增，但只存用户确认的长期理念并接入资源注册 | 明确理念、用户资料和 LS 身份边界 |
| D2 | README 覆盖粒度 | package 根 + 有独立责任的领域目录，不机械覆盖每个叶子目录 | 保持可导航且避免文档噪声 |
| D3 | 300 行规则 | 软上限；超过 600 行默认失败，例外需登记 | 避免机械拆分，同时阻止组合文件继续膨胀 |
| D4 | 是否并行分支 | 阶段 0-2 后按 A-E 所有权并行 | 降低冲突，不让多个 Agent 同时改热点 |
| D5 | “数字生命”公开表达 | 作为长期产品方向，不作为当前已实现能力宣传 | 保留总基调，同时避免错误能力声明 |
| D6 | 个人称呼和开发者资料位置 | 只进入用户私有资料，不进入公开仓库 | 满足个性化且保护公开边界 |
...
1. 确认 D1-D6；
2. 决定是否允许阶段 3-5 使用并行分支；
3. 在每个阶段验收功能是否保持一致；
4. 对 `PHILOSOPHY.md` 的内容拥有最终编辑和确认权；
5. 发现交互或行为变化时提供可复现步骤。
