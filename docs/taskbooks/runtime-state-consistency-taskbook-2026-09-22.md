...
本清单合并原“持久记忆写入路径任务书”的 MW-01～05：方向选择在本轮明确，入口/写入语义由 RS-05/06 承接，已有记忆的用户纠正/撤销由 RS-06B 承接，缓存与验收由 RS-07/08 承接。避免两份清单重复排期。当前代码仍是压缩唯一写入方，“明确要求或必要时写入”和 Agent 修改已有记忆均尚未实现；现行事实文档不能提前写成已支持。
...
2026-09-24 进一步核对：`packages/runner/src/infra.ts` 只注册 Agent 可用的只读 `memory_tree`，没有写入或修改已有 Atom 的工具；`packages/memory-tree/src/memory-repository/v3-atom-management.ts` 已有按 revision 撤销/恢复及有条件的替代/修订底层操作，`memory-correction.ts` 也有双 Atom、关系与证据校验，但这些高级服务只有 API/测试，没有 Agent Runtime 调用方。`packages/app/src/main/memory-atom-control.ts` 的管理入口不等于 Agent 对话能力；`packages/runner/src/session-summary-revocation.ts` 与 `packages/harness/src/context.ts` 已有撤销后阻止旧摘要注入的边界，需要接入新操作并验证。当前用户说“忘记”“刚才记错了”时，Agent 无法可靠修改现有持久记忆，RS-06 的新增写入不能代替这一能力。
...
- [ ] 保持 HC-12 撤销、冲突/替代及来源边界；现有撤销机制必须能约束新记录。用户纠正/忘记现有 Atom 由 RS-06B 接入；该任务未完成前不得假称支持，也不能把纠正请求静默存成一条矛盾事实。

**进行中（2026-09-27）**：`memory_write` 已落地并注册（`packages/memory-tree/src/memory-write-tool.ts` + `packages/runner/src/infra.ts`，`requiresApproval: true`，每 run 上限 4 条）。已完成的规则：两种有记录理由（`user-request` 必须引用**真的写了记忆指令**的用户消息，Runtime 读原文核对，模型不能自授权；`necessary` 需要说明用途与损失的实质理由）；引用的来源必须存在于本会话，并据此生成不可变 `conversation-source:` 引用；作用域规则（只有 project 分支能用 workspace/project 作用域）；写入身份由会话+分支+作用域+规范化内容+来源哈希而来（重试同一操作、内容不同即另一操作）；认识状态走既有的 `resolveMemoryWriteEpistemic`（新增中性 `tool` stage，状态按 `capture` 的谨慎口径）；模型只有在拿到 `created/merged/reinforced` 时才被允许说"已记住"，`rejected/queued` 一律如实报告。**反自动合并**在两个写入路径落地（`merge-guard.ts`）：值与否定条件不同或实体不相交的相似记忆拒绝合并、写审计、旧记录不动——§2.1 复现的 5432→6432 缺陷已有常驻回归（`v3-backend.test.ts`）。测试：`memory-write-tool.test.ts` 11 例、`merge-guard.test.ts` 5 例、`runner.test.ts` 新增注册与授权用例。**尚未完成**：证据完整/截断的显式拒绝（引用被截断工具结果的写入仍需专门判定）、批准后的写入理由与权限重验、按模型可见证据完整性降级，以及 RS-07 的"重启→新会话召回"实机验收。
...
范围：沿现有 `memory_tree` 导航确定具体 Atom 与当前 revision；经既有工具注册、权限、执行记录入口，复用 `MemoryRepository.management`、`memory-correction.ts`、`memory-revision.ts` 及来源/摘要撤销边界。首版只承接用户明确要求的“忘记这条”“刚才记错了，应为……”及有可靠新证据的同一事实纠正；不恢复自动演化或给模型开放 `move`/`merge`/任意 `revise` 等通用管理能力。仅在底层约束被证明不足以完成用户明确纠正时，补最小受控的替代提交规则。

- [ ] 增加受控的 `memory_manage`（或与 `memory_write` 同一工具的明确操作）入口，先通过索引/展开定位目标，再由 Runtime 核对本轮已见目标 ID、revision、scope、所属主体、来源与状态；含糊指代、多条同名、跨项目或已变更版本时不猜目标，返回具体阻断原因并允许重读后再试。用户原话与模型自填的 `atomId`、`expectedRevision`、`authorized=true` 不能单独证明授权。
- [ ] “忘记”只对明确目标执行受控 `invalidate`，保留不可改写的原始对话、操作审计和恢复所需历史；使该 Atom 立即退出普通检索、索引和当前 working set，并复用会话摘要撤销标记阻止旧摘要把它重新当作当前事实。后续压缩、重启或重试不得从同一旧来源重新沉淀；若用户要求删除原始数据而不仅是停止使用这条记忆，明确报告该操作的实际边界，不把撤销说成物理擦除。
- [ ] “纠正”把用户新陈述或经验证的新证据写成有来源的新版本，再在同 branch/scope/parent/认识类别内让旧 Atom 失效或由新 Atom 替代；旧正文与来源保留供审计，新旧事实不能同时作为当前结论注入。既有 `supersede` 要求 active/resolved 且有证据的关系：若单次明确用户纠正尚无此关系，设计最小受控的关系创建/校验或等价提交边界，不能以此为由把纠正静默退化成普通新增写入；不能用相似度自动决定替代对象。
- [ ] 用既有权限模式和工具执行记录承接批准、拒绝及副作用结算；按目标当前 revision、用户/证据来源和稳定操作身份实现重试幂等。新版本提交与旧版本退出的中断点必须有原子结算或可恢复的持久操作记录，失败时明确给出哪一步已提交，恢复不能留下两个都被当作当前事实的 Atom；成功后更新索引/缓存、撤销旧会话摘要，并只在 Runtime 确认结算后允许模型说“已改正/已忘记”。
- [ ] 不对普通闲聊、网页指令、工具输出里的命令或未经验证的模型推测自动修改现有记忆；用户明确纠正优先于低权威旧事实，事实冲突且无法验证或定位时保留来源并说明待决，而非悄悄覆盖。阶段性不支持的管理动作返回具体能力边界，不伪装成功。

验收：隔离数据根中，先存“端口 5432”，再由用户说“刚才记错了，是 6432”：跨会话检索只把 6432 作为当前事实，旧来源和替代审计仍可查；“忘记这条偏好”后同会话、重启及再次压缩均不注入旧偏好。覆盖含糊目标、跨 scope、权限拒绝、stale revision、相同请求重试、响应丢失、新版本提交后中断、旧摘要与旧来源回流；每种失败均有可核对的持久状态和面向用户的准确结果。真实模型/Electron 检验归 RS-07，不以底层 API 测试冒充 Agent 已可用。

### RS-07｜定向故障与真实流程验收（P0，依赖 RS-00～06、RS-06A/06B）
...
- [ ] Memory 用隔离数据根和真实模型验证明确记住、必要决定按需写入、用户自然语言纠正/忘记、普通闲聊不默认写、正常压力压缩不学习、升级 pending 候选、重启幂等、跨会话召回及撤销；失败、权限拒绝和不支持情形单独保留。核对 Agent 实际可调用的工具目录与真实操作结果，不以 App 管理 API 存在证明对话能力。
...
| “忘记这条”与“5432 改成 6432” | Agent 先定位正确 Atom；忘记后旧事实不再注入，纠正后仅新事实是当前结论；会话摘要、跨会话召回、重启与再次压缩不复活旧事实，原始来源和变更审计仍可查 |
| 含糊目标、跨 scope、权限拒绝、stale revision 与中断重试 | 无未授权或过期修改；部分结算明确可恢复，同操作重试不重复修改，不得把仅新增一条矛盾事实报告为“已纠正” |
...
顺序：**RS-00 小范围收口；RS-01→02→03/04 文件一致性；RS-05/06 与依赖它的 RS-06B 同一发布批次交付按需写入及用户纠正/忘记；RS-06A 可独立修复并在 RS-07 前完成；RS-07 真实验收，RS-08 随各批同步。** 文件一致性优先于绝大多数新功能，不与 UI 扩展、MCP 或新规划体系绑定。

接受的代价：过期/未完整观察需重读；exec 后可能多读一次；首版不覆盖不受支持的链接/特殊文件；文档同名覆盖收缩为新建；普通聊天不默认学习，压缩不再触发长期事实写入。既有记忆与对话仍保留；明确要求和必要写入均不依赖会话长度或压缩时机。

不扩大为：通用文件数据库、OS 沙箱、分布式锁平台、持续 watcher、自动记忆治理、新的阶段/规划/调度体系。RS-06B 只开放有目标、有来源的纠正/忘记，不让“Agent 可修改记忆”变成任意 Atom 管理或后台自动演化。优先删除和复用；若保证需要超出本范围的机制，先记录具体无法满足的条件并收缩支持范围，不以更多抽象掩盖缺口。
