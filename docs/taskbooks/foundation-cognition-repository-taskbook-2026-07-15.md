...
- 最终质量门通过：`check:repo` 31/31、全量 119 个测试文件（996 项通过、1 项跳过）、26 个 workspace typecheck、全工作区 build 和恢复源检查；桌面快捷方式已刷新，最新 `LittleSheep` 窗口可见且响应正常。
...
- 所有 300 行以上生产文件必须登记；当前 5 个超过 600 行的文件进入包含原因、所有者、上限和 2026-08-15 复查日期的受控清单；
- 新增 workspace 运行时依赖环检查、跨包深层 import 检查和核心协议唯一来源检查；
- 当前 `pnpm.cmd run check:repo` 为 31 项通过、0 项失败。
