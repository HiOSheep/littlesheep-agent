...
进行中但阶段 5 已暂停。阶段 0、阶段 1、阶段 2、阶段 3、阶段 4 已完成并分别形成独立提交且推送到 GitHub；阶段 5A、阶段 5B、阶段 5C、阶段 5D、阶段 5E、阶段 5F、阶段 5G、阶段 5H、阶段 5I、阶段 5J、阶段 5K、阶段 5L、阶段 5M、阶段 5N、阶段 5O、阶段 5P 的实现、质量门、提交和推送已完成。本文是一个可单独设置为阶段目标的执行基线；完成任一阶段后，必须更新本文的状态、证据和实际边界，并提交、推送一次，再决定是否进入下一阶段。
...
状态：暂停。阶段 5A、阶段 5B、阶段 5C、阶段 5D、阶段 5E、阶段 5F、阶段 5G、阶段 5H、阶段 5I、阶段 5J、阶段 5K、阶段 5L、阶段 5M、阶段 5N、阶段 5O、阶段 5P 已完成；本轮审计没有发现值得继续拆分的新跨模块状态阶段，后续优先回到开发反馈环提速问题。
...
状态：已完成。本阶段只收敛活动路由、需求评估和澄清请求/响应四类决策状态；`lastError`、`recoveryAttempts` 以及执行证据字段保留到后续阶段，避免把失败恢复控制与决策语义混成一个入口。

- `run-context-contract.ts` 新增 `decision` 组，登记 `classification`、`needAssessment`、`clarificationRequest`、`clarificationResponse` 的 owner、读写阶段、生命周期和 purpose。
- 新增 `packages/harness/src/decision-state.ts`，提供 `writeDecisionState()` 和 `updateClarificationRequest()`；先验证整批字段，再一次性提交。澄清文本更新使用复制后的 request，避免原对象在校验前被半途修改。
- CLASSIFY、DECIDE adoption、ASK_USER、RECOVER、VERIFY escalation、Context 初始化和 Runner checkpoint restore 均已接入统一入口；生产源码中本阶段四个字段不再直接赋值。测试夹具仍可直接构造 `RunContext`。

阶段 5H 定向证据（2026-08-10）：`decision-state.test.ts`、`run-context-contract.test.ts`、`stages/classify.test.ts`、`stages/decide.test.ts`、`stages/ask_user.test.ts`、`stages/recover.test.ts`、`stages/verify.test.ts` 共 6 个测试文件、77 项通过；Harness build 与 Runner typecheck 通过；生产源码扫描未发现本阶段四个字段的直接赋值。`verify:core` 通过：仓库卫生 33/33、TypeScript project references 27/27、核心集合 127 项通过，`failedStage=null`，耗时约 18.86 秒。`verify:full` 通过：287 个测试文件、1997 passed、1 skipped，workspace typecheck、App build 和 recovery 均通过，`failedStage=null`，耗时约 206.94 秒；recovery 仍只有既有 sampled runIds 缺少 execution log 和可选 `workspace/artifacts.json` 警告。阶段 5H 已独立提交并推送；用户已有 `test/e2e-webhook.test.ts` 改动不纳入本阶段。

阶段 5I：失败与恢复状态写入边界

状态：已完成。本阶段只收敛 `lastError` 与 `recoveryAttempts` 两个跨阶段失败/恢复字段；`modelCallCount` 属于模型观测/预算，`toolResults`、`sideEffects`、`toolInvocations`、`toolInvocationsTruncated` 属于执行证据，均保留到后续独立阶段，避免把不同生命周期和验证责任混成一个入口。

- `run-context-contract.ts` 新增 `failure` 组：`lastError` 由 Core Flow 各 stage、`runner-restore`、`post-run` 写入，`recoveryAttempts` 只由 `runner-init`、`recover`、`runner-restore` 写入；前者为 `run-local`，后者为 `checkpoint-carried`。
- 新增 `packages/harness/src/failure-state.ts`，提供 `writeFailureState()`、`recordFailure()`、`clearFailure()` 和 `incrementRecoveryAttempts()`；所有入口先校验当前写入边界，再一次性提交，失败消息保留产生错误的 stage 和可选 cause。
- Context 初始化、Core Flow stage、TaskBook/Runner 执行、RECOVER、VERIFY/REPLY/ASK_USER、checkpoint restore 和 post-run 持久化失败均已接入统一入口；生产源码中不再直接赋值 `lastError` 或 `recoveryAttempts`。checkpoint schema 和 `RunContext` 公共字段形状保持兼容，checkpoint 仅继续携带恢复计数。

阶段 5I 定向证据（2026-08-10）：`failure-state.test.ts`、`run-context-contract.test.ts`、`stages/ask_user.test.ts`、`stages/decide.test.ts`、`stages/execute.test.ts`、`stages/recover.test.ts`、`stages/reply.test.ts`、`stages/verify.test.ts`、`runner.test.ts`、`runner-continuation.test.ts` 共 10 个测试文件、139 项通过；Harness build 与 Runner typecheck 通过；生产源码扫描未发现 `lastError =` 或 `recoveryAttempts =` 直接赋值。最新质量门：`check:repo` 为 33/33、27/27 个 TypeScript project references 通过；`verify:core` 退出码为 0，核心集合 127 项通过，`failedStage=null`；`verify:full` 退出码为 0，288 个测试文件、2000 passed、1 skipped，workspace typecheck、App build 和 recovery 均通过，`failedStage=null`，总耗时约 420.64 秒。recovery 仍只有既有 sampled runIds 缺少 execution log 和可选 `workspace/artifacts.json` 警告。阶段 5I 完成后，下一候选是执行证据状态边界。

阶段 5J：执行证据状态写入边界

状态：已完成。本阶段只收敛四个顶层执行证据字段：`toolResults`、`toolInvocations`、`toolInvocationsTruncated` 和 `sideEffects`。TaskBook 内部 `stageResults[*].toolResults` 继续由 `taskExecution`/EXECUTE 嵌套证据边界拥有；`modelCallCount`、`modelRequests` 和 `contextSnapshots` 继续由模型观测/预算领域拥有。

- `run-context-contract.ts` 新增 `executionEvidence` 组，登记四个字段的 owner、读写阶段、生命周期和 purpose；ownership manifest 现覆盖 37 个字段、7 组高频共享状态。
- 新增 `packages/harness/src/execution-evidence-state.ts`，提供 `writeExecutionEvidenceState()`、`replaceToolResults()`、`upsertToolInvocationEvidence()` 和 `replaceSideEffectEvidence()`；所有字段先完成 stage 校验再一次性写入，调用记录保持有界 upsert，副作用账本更新使用 detached immutable replacement。
- Context 初始化、legacy EXECUTE、TaskBook EXECUTE、ToolExecutionService 调用回调、side-effect ledger begin/finish/uncertain 和 Runner checkpoint restore 均已接入统一入口；生产源码中不再直接赋值顶层四个字段或原地修改副作用账本记录。checkpoint schema、幂等键、容量上限和 `RunnerResult` 公共形状保持兼容。
...
阶段 5M：Context snapshot 生命周期与截断一致性边界

状态：已完成。本阶段审计发现模型观测入口、checkpoint 引用窗口和 execution log 持久化窗口此前分别使用 64、128 和未再次限制的边界，长 run 会让恢复引用与日志内容的生命周期难以相互验证。现统一以 `MAX_MODEL_REQUEST_SNAPSHOTS_PER_RUN = 64` 作为有效窗口：checkpoint 只引用最近 64 个 `contextSnapshotId`，execution log 只持久化最近 64 个 `modelRequests` 与 `contextSnapshots`，资源索引从截断后的 snapshot 集合生成。公共 `RunContext` 字段形状、checkpoint schema、`contextSnapshots[].providerUsage` 的嵌套位置和 ownership group 均保持不变。

- 变更集中在 `packages/runner/src/run-checkpoint.ts` 与 `packages/runner/src/execution-log.ts`；测试覆盖超窗 tail 保留、request/context snapshot 关联和 checkpoint 引用窗口。
- 旧 checkpoint 的读取兼容边界保持原样，由 `run-checkpoint-store.ts` 继续处理历史数据，不对既有持久化 schema 做迁移。

阶段 5M 定向证据（2026-08-10）：`run-checkpoint.test.ts`、`execution-log.test.ts`、`runner-continuation.test.ts`、`memory-workload-observability.test.ts`、`model-observability.test.ts`、`model-observability-state.test.ts` 共 6 个文件、37 项通过；`check:repo` 为 33/33，27/27 个 TypeScript project references 通过；`verify:core` 退出码为 0，核心集合 7 个测试文件、127 项通过，`failedStage=null`。`verify:full` 两次均在同一既有 `packages/app/src/main/local-app-api/workspace-git-review-edge-cases.test.ts` 用例上发生 30 秒超时，报告为 290 个测试文件、2008 passed、1 skipped、`failedStage=tests`，总耗时约 337.14 秒；该文件单独重跑通过，表明是全量运行时资源竞争/波动，且本阶段没有修改 App 路径，因此不纳入本阶段修复范围。完整门因测试阶段失败而未执行 typecheck、build、recovery；核心门已覆盖本阶段公共 Runner/Context 契约。阶段 5M 完成后，每个阶段独立提交并推送 GitHub，用户已有的 `test/e2e-webhook.test.ts` 改动不纳入提交。

阶段 5N：checkpoint store 新写入与历史读取边界

状态：已完成。本阶段修复 5M 审计留下的持久化绕过路径：`buildRunCheckpoint()` 已只生成最近 64 个 snapshot ID，但 `RunCheckpointStore.write()` 仍使用旧的 128 条校验上限，直接写入 store 的新 checkpoint 可能重新扩大窗口。现将校验函数的 snapshot 上限作为显式参数传入：新写入使用 64，读取、列表和诊断使用 128，以保持历史 checkpoint 可检查兼容；不改变 checkpoint schema、不裁剪旧文件、不新增 ownership group。

- `run-checkpoint-store.test.ts` 新增新写入超限拒绝和 128 条历史 checkpoint 读取回归。
- `run-checkpoint.ts`、execution log 和 Context 的 64 条生产窗口保持不变；store 只补齐持久化闸门。

阶段 5N 定向证据（2026-08-10）：`run-checkpoint-store.test.ts`、`run-checkpoint.test.ts`、`runner-continuation.test.ts` 共 3 个文件、15 项通过；`check:repo` 为 33/33，27/27 个 TypeScript project references 通过；`verify:core` 退出码为 0，核心集合 7 个测试文件、127 项通过，`failedStage=null`。`verify:full` 在同一既有 `packages/app/src/main/local-app-api/workspace-git-review-edge-cases.test.ts` 用例上发生 30 秒超时，报告为 290 个测试文件、2010 passed、1 skipped、`failedStage=tests`，总耗时约 484.60 秒；该文件单独重跑此前通过，表明是全量运行时资源竞争/波动，且本阶段没有修改 App 路径，因此不纳入本阶段修复范围。完整门因测试阶段失败而未执行 typecheck、build、recovery；核心门已覆盖本阶段公共 Runner/Context 契约。用户已有的 `test/e2e-webhook.test.ts` 改动不纳入提交。

阶段 5O：模型观测入口的共享窗口防绕过边界

状态：已完成。本阶段审计发现 `appendModelObservations()` 作为公开 Harness 状态入口接受调用方传入的 `maxSnapshots`，生产调用虽然固定使用 64，但外部/测试调用可以传入更大值，使 `RunContext.modelRequests` 与 `contextSnapshots` 超过统一 retention 窗口。现由入口内部将参数归一化到 `1..MAX_MODEL_REQUEST_SNAPSHOTS_PER_RUN`，非有限值回退到 64，两个数组共享同一归一化上限；不改变函数签名、公共字段形状、checkpoint schema 或 ownership group。

- 新增特征测试证明传入 `Number.MAX_SAFE_INTEGER` 仍只保留最近 64 个 request/context snapshot。
...
阶段 5 暂停审计（2026-08-10）

本轮不机械启动 5Q/5R/5S，按四项判定标准检查当前状态边界：

1. **是否还有可证明的跨模块状态漏洞？** 当前没有发现已登记 ownership 字段的生产绕过。静态扫描覆盖 Harness/Runner 生产 TypeScript 128 个文件，manifest 的 40 个字段没有 `ctx.field =` 或原地数组写入命中；阶段 5A–5P 已覆盖状态写入、checkpoint reconcile、snapshot retention、store 新写入校验和 request/snapshot 关联。`verificationHistory` 仍由 VERIFY 记录函数追加、由 Runner restore 恢复，是未纳入八组 manifest 的观察项，但本轮没有发现历史丢失、恢复引用断裂或非法写入阶段的可复现失败。
2. **是否影响开发定位、恢复正确性或任务执行效率？** 当前证据不足以证明它已成为瓶颈。checkpoint 对 `verificationHistory` 有 32 条输入校验和恢复边界，默认 replan 为 2、默认 recovery 为 3、模型调用为 32；execution log 继续保留完整验证记录供审计，Memory workload projection 另有 512 条分析封套。现有 checkpoint continuation、execution log、VERIFY 和 core 回归均通过。
3. **是否可以由现有 owner 和边界解决，而不需要新增抽象？** 可以。`verificationHistory` 的产生语义仍集中在 `packages/harness/src/stages/verify/routing.ts`，Runner 只负责 continuation 装配，checkpoint/store 和 execution log 继续分别拥有持久化边界；没有证据要求新增第九个 ownership group 或跨域 facade。
4. **是否值得承担一次独立阶段的验证和提交成本？** 当前不值得。新增阶段会带来核心协议 manifest、checkpoint 兼容和多模块回归成本，却不能对应一个已复现的用户可见错误或可测量的反馈环收益。

**决策：阶段 5 暂停。** 后续阶段不以状态字段数量为目标；优先恢复任务级反馈耗时、affected 选择器可信度、重复构建消除和 full gate 资源竞争的测量。只有出现可复现状态错误、现有 owner 无法处理且收益足以覆盖独立阶段成本时，才重新开启新的状态治理阶段。

阶段 5 暂停审计证据（2026-08-10）：`check:repo` 33/33、27/27 个 TypeScript project references 通过；`verify:core` 7 个测试文件、127 项通过，`failedStage=null`。用户已有的 `test/e2e-webhook.test.ts` 改动不纳入本次文档提交。
...
- 阶段 1 的任务级入口和直接测试优先策略已完成，阶段 4 的代码实现、定向回归、`verify:changed`、`verify:core` 和 `verify:full` 已完成；阶段 5A 的状态契约第一里程碑、阶段 5B 的 Runner coordinator 最小抽取、阶段 5C 的 replan-state 写入边界、阶段 5D 的 reply/replyProvenance 写入边界、阶段 5E 的 runtime state 写入边界、阶段 5F 的 Memory 顶层 RunContext 写入边界、阶段 5G 的 usage 顶层快照写入边界、阶段 5H 的决策与澄清状态写入边界、阶段 5I 的失败与恢复状态写入边界、阶段 5J 的执行证据状态写入边界、阶段 5K 的模型观测与预算状态写入边界、阶段 5L 的 checkpoint 预算一致性边界、阶段 5M 的 Context snapshot 生命周期与截断一致性边界、阶段 5N 的 checkpoint store 新写入与历史读取边界、阶段 5O 的模型观测入口共享窗口防绕过边界和阶段 5P 的 execution log request/context snapshot 关联保留边界已完成，质量门、提交和推送按阶段独立执行；阶段 5 暂停审计已完成，后续优先回到开发反馈环提速。本阶段提交时必须排除用户已有的 `test/e2e-webhook.test.ts` 改动。
