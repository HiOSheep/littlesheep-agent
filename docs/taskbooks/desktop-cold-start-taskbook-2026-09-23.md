...
- CS-08 仍未完成的部分：**改选目录**（走系统目录选择对话框，无法在无头环境驱动，需人工或改走可注入的选择器）；浏览器标签在未就绪期间的显式断言也还没做。复选框保持未勾选。
- **切换会话与关闭/恢复窗口已实机取证（发现一处真实缺陷，未修）**：`scripts/verify-desktop-cold-start-interaction.mjs` 新增工作区健壮性用例，夹具两份文件各带唯一标记，于是"面板说自己在显示哪个文件"（面包屑）与"正文到底是哪个文件"（标记）可以互校——这正是"迟到的旧响应覆盖新响应"会露馅的地方。实测四件事：①**控制组通过**：就绪后在某个会话里打开文件，切走再切回，正文逐字恢复（面板已开、正文 753 字符、标记一致），说明"按会话保存布局"这条路径是通的；②**竞争通过**：点第二个文件后立刻切走再切回，面板显示 `notes.md` 且正文就是 notes 的标记，新响应胜出、没有串味；③**关闭/恢复窗口与就绪交接通过**：隐藏再显示、以及就绪到达后，正文一字不变；④**发现缺陷**：**启动期间**（会话列表刚出现、窗口仍是草稿会话时）打开的文件被记进 `__draft__` 布局桶，用户在侧栏点进任何会话后，当初被高亮的那一行再点回来时拿到的是它自己的空桶（`session:<id>=[review]`），**启动时打开的文件不再回来**——Main 的 `workspace/layout.json` 与渲染器镜像都记进了证据；不修的原因是先要定"启动期草稿布局归属哪一段会话"这条产品规则。因此该用例目前**故意红**：失败项名就是这条缺陷，不要把它读成 CS-09 的回归。
- **产品规则已定，按该规则的修复尝试未生效并已回滚**：决定是"启动期草稿布局并入用户进入的第一段会话"。据此写了两条纯规则（`adoptStartupDraftSessionLayout` / `hasWorkspaceLayoutContent`：空草稿不带、目标会话已有自己的内容不覆盖）并单测通过，同时把认领从"只在 `__draft__ → session` 那一次键变化"改为"会话键未被用户碰过期间持续认领"。**实机无效**：镜像与改动前逐字节相同（`__draft__=[review+file]`、`session:<id>=[review]`），持续认领同样没有触发，说明文件标签写进 `__draft__` 桶的时机/路径不在这个 hook 的观察范围内；改动已 `git checkout` 回滚，不留未证实的行为变更。下一步是在 `openWorkspaceFile`（`setWorkspaceOpenRequest` → `updateActiveLayout`）与 `commitLayout` 上加一次性观测，记录每次写入时的 `activeWorkspaceSessionKey`/`currentSession`/`touchedKeysRef`，看清"哪一帧、以哪个键写进去的"再决定认领挂在哪一级；详细记录见基线文档的 CS-08 补充一节。
