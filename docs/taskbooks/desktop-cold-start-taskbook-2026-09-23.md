...
最后更新：2026-09-24 20:25:23

## 0. 关闭记录（2026-09-24）

**CS-01～CS-10 的全部复选框已勾选，验收完成。** 依据分两类，逐项事实都落在常驻文档（[桌面冷启动基线](../reference/cold-start-baseline/README.md)、[项目状态](../decision/project-status.md) 与各 package / 领域 README）：

- **自动化实机证据**（真实 Electron 窗口 + 账本）：五时间点基线与冷/稳态护栏（`desktop-cold-start-*.json`、`budgets.json`）；三种窗口宽度、最大化/还原、启动页与启动失败页的真实像素（`cold-start-visuals*.json` + 截图）；未就绪期间的输入/切换/发送门禁（`cold-start-interaction.json`）；失败态与有界重试（`cold-start-readiness-failure.json`）；恢复归属（`cold-start-recovery.json`，真实 Provider 凭据）；CS-08 右侧可用性三指标与切换/竞争/窗口隐藏健壮性（`desktop-workspace-availability-2026-09-24.json`、`cold-start-interaction.json`）；CS-09 三档启动的摆放实拍（`cold-start-readiness-placement.json`）；CS-10 大历史数据根与缓存复用（`desktop-large-history-startup-2026-09-24{,-large,-packaged}.json`：400/1200 分区下执行就绪 1191/1233 ms，缓存首次 1 次请求、重访 0 次、0 取消，打包版 1497 ms 同样 0 失败）。
- **人工验收（用户 2026-09-24 确认全部无问题）**：CS-02 的 125% / 150% / 200% 显示缩放、明暗桌面背景、启动页→渲染器交接瞬间、失焦与最小化/还原观感；系统重启后的真正冷启动；CS-07 的安装包实机与异常路径（大历史、离线、无模型配置、迁移失败）；CS-06/CS-07 的真实续接执行（在自有数据上点"继续"）；CS-08 的改选目录。

仍然成立的边界（不因勾选而消失，已写在基线文档的对应小节）：合成数据根只证明机制、不证明真实磁盘规模；缓存用例的夹具会话没有消息，证明的是请求行为而非真实负载收益；DPR 代理不能替代真实缩放目视；后台兼容扫描没有完成信号；安装包未签名，签名与干净机器安装仍是发布门（`package:win` 报告 `signed: false`）。

本任务书已按仓库文档生命周期退役：以上事实由常驻文档拥有，原文保留在 git 历史（`git log --follow -- docs/taskbooks/desktop-cold-start-taskbook-2026-09-23.md`）。
...
| 状态 | ID | 优先级 | 任务 | 前置依赖 |
| --- | --- | --- | --- | --- |
| [x] | CS-08 | P0 | 右侧拓展工作区进入即能预览和操作 | CS-01、CS-03；与 CS-07 共用实机验收 |
| [x] | CS-09 | P0 | 正常加载信息不占据整窗顶部 | CS-02、CS-03；失败状态保留全局可见 |
| [x] | CS-01 | P0 | 建立可重复的启动基线 | 无 |
| [x] | CS-02 | P0 | 统一启动页、标题栏与按钮区域视觉 | 无；使用 CS-01 记录前后表现 |
| [x] | CS-03 | P1 | 提前呈现可交互界面并建立就绪契约 | CS-01 |
| [x] | CS-04 | P1 | 缩短首次可执行的关键路径 | CS-01；与 CS-03 对齐能力边界 |
| [x] | CS-05 | P1 | 减少首屏依赖及后台资源争用 | CS-01；与 CS-03 对齐页面加载 |
| [x] | CS-06 | P1 | 保护续接、草稿和初始化失败体验 | CS-03；贯穿 CS-04、CS-05 |
| [x] | CS-07 | P2 | 完成真实 Electron 启动回归与文档收口 | CS-01～CS-06 |
| [x] | CS-10 | P0 | 按选中会话加载与后台续载；新会话直接进入输入 | CS-03、CS-04；需真实窗口快速切换验收 |
...
- [x] 记录进程启动、真实首帧、输入区可用、当前会话可读、首次可执行五个时间点，并注明各时钟来源与关联方式。
- [x] 在已有计时基础上细分主要阶段，覆盖首条主进程业务日志之前的模块加载时间以及 Renderer 首次挂载。
- [x] 分别测量开发版与打包版；区分进程冷启动、系统重启后的首次启动、重复启动，不把热缓存样本称为完全冷启动。
- [x] 使用隔离数据覆盖空数据、普通数据量、大历史会话与待恢复任务，记录机器、版本、数据规模、运行次数和耗时分布。
- [x] 基于基线为五项指标确定验收预算；保留逐次原始数据，样本不足时不声称获得稳定的高分位统计。

验收：能重复定位耗时所在阶段；计时不包含密钥与用户正文；启动画面出现、界面可操作和任务可执行被分别度量。

### CS-02｜统一启动视觉

- [x] 为启动页、标题栏、原生按钮覆盖区和正式界面交接定义一致的底色、标题栏高度及材质策略，优先验证统一实色方案。
- [x] 保留小羊品牌元素，使其位置和尺寸不因交接发生明显跳动；避免大范围明暗闪烁。
- [x] 检查正常、失焦、最大化、还原、最小化恢复以及错误页状态。
- [x] 在明暗桌面背景和 100% / 125% / 150% / 200% 缩放下截图对比，检查按钮区域接缝、边框和拖动区域。

验收：启动页与按钮区域无明显色块断层；原生窗口行为正常；进入正式界面无白闪、黑闪或明显布局跳变。真实 Electron 截图是完成依据，不能仅以 CSS 或 HTML 测试代替。

### CS-03｜提前呈现可交互界面

- [x] 拆分轻量界面挂载与完整业务初始化，保留最小失败兜底页。
- [x] 为界面、数据读取、运行执行定义最少必要的就绪状态与失败状态；避免新增通用调度框架。
- [x] 改造 preload 的固定端口快照：提供受控状态查询与就绪通知，订阅后可补读当前状态，防止错过通知；卸载时释放订阅。
- [x] 将仅需本地元数据的读取与 Runner 依赖隔离；迁移期间禁止访问尚不一致的数据。Main 继续校验接口及权限。
- [x] 等待期间允许草稿输入和窗口操作；就绪后原地启用发送。未就绪时不可向端口 `0` 发请求，不得假报已发送。

验收：慢初始化期间可连续输入，草稿和焦点在就绪交接后保持不变；会话数据按真实就绪状态显示；不通过整页导航实现业务能力启用。

### CS-04｜缩短首次可执行路径

- [x] 依据 CS-01 列出初始化依赖图，区分执行必需、当前页面必需和非必要维护工作。
- [x] 检查 Runner 中版本管理、检查点、记忆仓库、恢复队列与 Skill 资源等初始化成本，仅调整测得有收益且依赖允许的部分。
- [x] 数据迁移、持久化一致性和权限前置检查保持顺序；只对互不依赖的准备工作使用有限并行。
- [x] 可后移的维护使用单次初始化与共享进行中结果，防止首次访问、后台预热和重试同时启动同一工作。
- [x] 同时记录首次发送的准备耗时，防止将原有启动等待全部转移到用户点击发送之后。

验收：首次可执行与首次发送指标符合 CS-01 确定的预算；迁移失败不放行执行；无重复初始化、数据库竞争或已结算副作用重放。

### CS-05｜减少首屏依赖与资源争用

- [x] 测量首屏依赖和解析成本，检查设置页、终端、预览、Git 等静态导入链；未打开的功能按需加载。
- [x] 保留现有 Monaco 懒加载；加载失败有局部重试，不使整个应用不可用。
- [x] 优先加载当前会话和可见列表；若引入只读缓存，明确数据归属、失效和更新规则，不复制第二份权威存储。
- [x] 对后台 I/O 限并发、长列表分批处理；仅在测量确认 CPU 阻塞后引入 Worker 或独立进程。
- [x] 在后台工作进行时测量输入延迟、滚动及窗口拖动表现；用户关闭应用后及时停止待执行工作。

验收：首屏不等待未使用功能；后台初始化不导致明显输入和窗口卡顿；按需加载不会使首次使用长期无反馈。

### CS-06｜保护续接与故障体验

- [x] 恢复发现等待所需依赖就绪，再自动处理可安全续接且不需用户输入的检查点。
- [x] 恢复结果回写原会话；用户已切换会话或开始输入时，不自动抢回页面、不覆盖草稿。
- [x] 为初始化失败、超时、局部能力失败提供真实状态与有界重试；避免永久停留在品牌页。
- [x] 重试复用已完成证据，避免重复迁移、重复恢复、重复发送和重复发布回复。
- [x] 验证启动期间关闭、最小化、重新激活与重试并发，不出现窗口复活、资源泄漏或已销毁窗口访问。

验收：安全恢复不弹出阻断对话框；需介入事项仍可找到；局部故障不破坏已可用的输入和浏览；失败不会被表达为成功。

### CS-07｜真实回归与文档收口

- [x] 执行打包版与开发版的启动回归，覆盖大历史会话、离线、无模型配置、待恢复任务和迁移失败。
- [x] 覆盖启动中输入、切换会话、关闭窗口，以及各 DPI、最大化和窗口交接场景。
- [x] 对涉及就绪竞态、幂等初始化、恢复归属和失败重试的逻辑运行针对性测试，并完成仓库要求的检查。
- [x] 保存前后耗时、配置、样本量及截图/录像；区分真实改进、样本波动和未验证场景。
- [x] 修改行为或依赖时同步更新对应 package / 领域 README，并按系统时钟填写更新时间；同一变更提交中包含实现与说明。
- [x] 完成后将稳定事实归入常驻文档，再按文档生命周期规则退役任务书。

验收：CS-01～CS-06、CS-08、CS-09 的证据齐备，真实窗口交互无回归；无法验证的场景保持未勾选并说明缺口，不以单元测试替代实机结论。

### CS-08｜右侧拓展工作区进入即能预览和操作

- [x] 为“右侧真正可用”建立独立指标：进程启动 → 首个目录条目出现、进程启动 → 恢复的文件内容可见、点击文件 → 预览内容可见。由真实 Renderer DOM 与成功的 Local App API 响应共同确认，不把壳体绘制或请求发出当作完成。
- [x] 在保留默认工作区、最近文件标签、展开目录和普通会话布局的隔离数据根中实测。分别覆盖首次启动和同一数据根的后续启动；记录目录规模、文件类型与预览大小。
- [x] 检查 `runtime` 初值为空、默认根路径恢复、布局镜像读取、目录预热和文件预览请求的时间顺序。只有测得的关键等待才进一步调整，保持 Main 对当前真实根路径和目标路径的校验。
- [x] 验证 Runner 尚未就绪时，目录、普通文件预览与浏览器标签能使用；发送和 Runner 依赖动作仍由原有就绪门管理。终端、Git、产物等各自依赖另列，不把“右侧已可用”泛化为全部功能已就绪。
- [x] 验证启动中点击文件、切换会话、选择新目录、关闭/恢复窗口均不会把预览复位到空白，也不会把旧请求结果写进新会话。

验收：用户进入右侧后可以看到真实目录与文件内容并执行只读浏览；正常路径不等待 Runner，失效路径给出局部可恢复状态。前后耗时应配对测量并记录原始证据；未完成此项实机验收前不得声称“右侧即刻可用”。

### CS-09｜收敛正常启动提示的视觉占用

- [x] 正常启动的阶段文字只在发送能力相关位置就近显示，不在标题栏下方横跨整窗；状态由 Main 的真实就绪事实提供。
- [x] 启动失败和可重试状态保持明显可见；重试预算与失败原因仍由 Main 决定。
- [x] 在正常、故意拉慢和故障启动中实拍，检查输入区焦点、草稿、右侧预览、窄窗和高缩放下的布局。

验收：正常启动没有占据视野的全局加载条；用户仍能理解何时可以发送；失败状态可见、可按既有规则重试。

### CS-10｜按选中会话加载与后台续载

- [x] 未选中会话不读取首屏历史；只有选中后才发起该会话的读取，读取属于该会话本身——切走不取消，切回可复用进行中或已完成的结果；缓存按会话时间戳校验、总数有界（当前最多 6 个会话，消息条数仍受 `SESSION_HISTORY_MEMORY_MAX` 约束），陈旧响应不得写进当前视图。
- [x] 新会话不依赖任何旧历史：新建会话直接进入可输入状态，首屏为空而不是上一段会话的内容，草稿与焦点不被抢占。
- [x] 会话选择只改变**当前视图的有效工作区**与本次 run 的请求工作区，不得因此改写全局默认工作目录或重建 Runner；切回独立/新建会话恢复持久化默认工作区，避免发送落到上一项目。
- [x] Local App API 的请求入口与 RunRouter 恢复解耦：元数据、Runtime、会话索引与工作区路由在恢复期立即应答；`/run`、`/run-stream`、`/run-checkpoints`（含子路径）、审批与应用生命周期在路由未就绪时明确返回 503，不静默排队、不假装成功。
- [x] 历史事件分区扫描不阻塞执行就绪：现代租约与收件箱中需要恢复的工作仍在就绪前处理，历史分区只作后台兼容扫描；分区在被读写或扫描时严格校验，损坏分区必须报错，不能被当成"没有待恢复工作"。
- [x] 在保留大量历史事件分区的真实或合成数据根上做真实窗口回归，记录进程启动 → 首帧、→ 工作区目录条目、→ 执行就绪，所选会话首屏历史的应答耗时与缓存复用；覆盖快速连续切换、后台扫描结束后继续交互，以及打包版复验。
