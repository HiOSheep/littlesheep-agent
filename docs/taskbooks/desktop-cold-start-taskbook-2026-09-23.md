...
- 剩余缺口：启动页到渲染器的交接瞬间（文档像素已覆盖，时序未覆盖）、失焦、最小化、还原、最大化恢复、125%/150%/200% 缩放、明暗桌面、打包版。**因此 CS-02 复选框保持未勾选。**
...
- 剩余缺口：初始化失败/超时的有界重试入口、启动期间关闭/最小化/重新激活/重试并发的实机场景未验证；**续接执行本身**（真正点"继续"并完成一次恢复）需要用户在真实数据根上决定。
