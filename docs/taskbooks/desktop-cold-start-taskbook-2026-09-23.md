...
- bootstrap 文件注册的下一处线索（只记录证据，未优化）：跨 profile 测量（各 3 次）显示该阶段 normal 27.9–41.3 ms、large 38.5–51.4 ms，**不随数据根规模增长**；代码阅读表明测量每次都新建数据根（首次注册必须落盘），而 `sameResourceRegistration` 忽略 `registeredAt`/`updatedAt`、`commitMutation` 在无增删改时提前返回，因此同一数据根的第二次启动应跳过这些写入。该推论需要"同一数据根连续启动两次"的对照才能成立，当前脚本不支持，故本轮不动，也不声称收益。
- 首次发送准备耗时（13–34 ms，与启动前的基线同量级）说明启动等待没有被转移到发送之后。
- 剩余缺口：预算已确定，Runner 构建已取得阶段级净收益（约 14 ms）但**未**在"首次可执行"上测出稳定改善；bootstrap 文件注册的写入成本尚未单独设计（当前证据指向"首次注册"，稳态行为待对照测量）。
