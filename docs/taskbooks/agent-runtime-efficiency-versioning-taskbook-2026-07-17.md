...
> 2026-08-04 09:31 当前效率复跑：自主单只读 `glob / grep / read` 均为 2 次 API、1 次工具、`exact_match`、结构 VERIFY 通过，Prompt 分别为 `1,009 / 979 / 924`；这些点值随步骤摘要小幅波动，回归契约为 `2 API / 1 tool / 1,200 total prompt`。

状态：已完成统一 Tool Execution Service、工具调用级与 TaskBook 步骤级有界并行、数据与工作区 shadow Git 检查点、退出冻结、有界 `RuntimeEventQueue`、活动 run ingress、Harness 安全边界、确定性 `TaskBookPatch`、延迟事件重规划、Renderer 事件生产/反馈入口、持久 RunCheckpoint、Runner 显式续跑、应用启动恢复控制面、活动任务控制、后台托盘、三档关闭策略和设置页“应用与后台”的工程基线。确定性 Electron 七场景，以及真实 DeepSeek 后台九场景、暂停/继续/中断、强制终止、跨重启恢复、回答级记忆连续性、一次有界断档纠偏、主动断线恢复、多轮五字段摘要连续性、当前 120 秒诊断门、正式 2 小时持续负载和短时并行压力均已通过；DeepSeek V4 Flash 普通请求与工具协议已完成 `15/15 exact_match`，显式单工具路径 Prompt 为 `796`，LLM 自主选读工具矩阵固定 2 次 API、1 次工具，最新 Prompt 为 `glob 1,009 / grep 979 / read 924`，回归契约为 `2 API / 1 tool / 1,200 total prompt`。Pro 工具协议、非字段事实和真实外部系统副作用仍待验收。
...
- `REPLY`、DECIDE 产生的任务说明、执行步骤结果、VERIFY 说明和多步骤 `execute_final_reply` 都必须在当次 run 中实时调用当前 Provider API 生成；运行时生成的澄清事实只能作为 `ASK_USER` 的输入，最终文本仍由 LLM 现场组织，不能直接发送，也不能从模板库、预备文案池或历史回答选取。这里不存在“LLM 生成候选文案后由 Runtime 挑选”的前台流程：Runtime 只校验真实 API 返回的来源、事实边界和唯一性。所有前台自然语言调用均注入运行时 `SOUL.md`，保证用户配置的人格、语气和渐进式披露风格；`ReplyProvenance` 绑定真实 model request，API 返回在发布前由持久化会话级注册表原子占用规范化指纹，完全重复时最多重新实时调用两次当前 Provider API，仍重复、注册表失败或生成失败只呈现 Runtime 错误/状态。直接续答在指纹占用前复用回答级连续性检查：普通回答不增加调用，只有首个实时回答被判为 `discontinuous` 时才允许一次实时 API 纠偏，纠偏仍断档则失败关闭。按钮、状态、权限、路径和进度由 Runtime 固定提供，`FINALIZE` 禁止新增模型调用，只负责校验并持久化已经生成的回复。
- 显式工具任务已建立有界提议路径：DECIDE 只接收用户明确点名的最多 4 种工具 schema，为每个可分别验证的步骤生成一个 `toolProposal`；单 schema 最多 12,000 字符、总计最多 24,000 字符，每个 TaskBook 最多采用 8 个提议。Runtime 再校验名称、schema、依赖、资源、副作用、权限和审批；直接执行接受可证明且无需审批的 `read/write`，以及完全访问模式下来源明确为内置、参数完整、非 Checkpoint 恢复态的单次 `exec`，随后只调用一次 `execute_final_reply`。续接、记忆追问、需审批、参数不完整或边界不可证明时自动回退普通执行链；Runtime 不从自然语言猜参数。2026-08-04 最新真实显式 `glob` 验收为 2 次 API、1 次工具调用，DECIDE/final prompt `417/379`、Provider total `826`；最新真实两步 `write -> read` 恢复任务同样只有 `decide -> execute_final_reply` 两次 API，耗时 `6.863s`，两次工具各成功一次。具有权威 usage 的请求合计 Provider prompt `5,477`、completion `934`、total `6,411`；中断分支的未完成请求不伪造 Provider usage。6 分钟历史门、当前 120 秒诊断门和正式 2 小时门均确认单次内置 `exec` 只执行一次、暂停在工具完成后生效且恢复不重放副作用。
...
- 2026-08-04 09:31:50 的最新“用户只表达目标”自主只读矩阵：只有请求新鲜、自包含、无附件、无续接、无实际记忆介入、无恢复态且不含写入/执行语义时，DECIDE 才在 builtin `glob / grep / read` 中选择一个最小工具，并一次返回满足有界 schema 的参数、简短步骤摘要和验收标准；Runtime 复核工具来源、schema、路径、权限和副作用后直接执行，不要求用户知道或点名工具，也不再开启 Provider 工具协议循环。只有 `excluded` 的记忆候选不算真实 Context 依赖；`adopted/conflicted`、活动 working set 或实际注入记忆会失败关闭。真实 `glob / grep / read` 均固定 2 次 API、1 次对应工具，DECIDE/final prompt 分别为 `630/379`、`646/333`、`632/292`，合计 `1,009 / 979 / 924`；六次请求均为 `exact_match`，Provider 工具协议请求数为 0，工作区哈希前后一致。回归上限保持为 `750/450/1,200`。Runtime 继续保留工具结果、SSE、结构 VERIFY 与调用审计；提议无法安全采用时失败关闭或回退既有工具循环，不猜参数。
...
pnpm.cmd run verify:electron-deepseek-autonomous-read
...
保持 `respond / execute / clarify`、直接回应 Context、统一 Tool Execution Service、TaskBook 步骤级并行、Renderer 运行时事件入口、应用启动恢复、回答级连续性、后台控制面、Flash 工具协议 `15/15` 校准和正式 2 小时持续任务的完整质量门；下一步校准 Pro/其他实际启用 Provider，并推进非字段事实、真实外部副作用恢复、版本治理 UI 和严格效率对比基线。自主只读路径仅在权限、schema、证据、结构 VERIFY 和实时最终回复均保持时继续缩短 Prompt。
