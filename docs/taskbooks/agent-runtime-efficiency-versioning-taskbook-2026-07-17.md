...
- 显式工具任务已建立有界提议路径：DECIDE 只接收用户明确点名的最多 4 种工具 schema，为每个可分别验证的步骤生成一个 `toolProposal`；单 schema 最多 12,000 字符、总计最多 24,000 字符，每个 TaskBook 最多采用 8 个提议。Runtime 再校验名称、schema、依赖、资源、副作用、权限和审批；直接执行接受可证明且无需审批的 `read/write`，以及完全访问模式下来源明确为内置、参数完整、非 Checkpoint 恢复态的单次 `exec`，随后只调用一次 `execute_final_reply`。续接、记忆追问、需审批、参数不完整或边界不可证明时自动回退普通执行链；Runtime 不从自然语言猜参数。2026-08-04 最新真实显式 `glob` 验收为 2 次 API、1 次工具调用，DECIDE/final prompt `417/379`、Provider total `826`；最新真实两步 `write -> read` 恢复任务同样只有 `decide -> execute_final_reply` 两次 API，耗时 `7.943s`，两次工具各成功一次。本轮有权威 usage 的请求合计 Provider total `7,098`；中断分支不伪造 Provider usage。6 分钟历史门、当前 120 秒诊断门和正式 2 小时门均确认单次内置 `exec` 只执行一次、暂停在工具完成后生效且恢复不重放副作用。
...
- 2026-08-04 05:16:39 的最新“用户只表达目标”自主只读路径：只有请求新鲜、自包含、无附件、无续接、无实际记忆介入、无恢复态且不含写入/执行语义时，DECIDE 才在 builtin `glob / grep / read` 中选择一个最小工具，并一次返回满足有界 schema 的参数、简短步骤摘要和验收标准；Runtime 复核工具来源、schema、路径、权限和副作用后直接执行，不要求用户知道或点名工具，也不再开启 Provider 工具协议循环。只有 `excluded` 的记忆候选不算真实 Context 依赖；`adopted/conflicted`、活动 working set 或实际注入记忆会失败关闭。最新真实 `glob` 固定 2 次 API、1 次工具，DECIDE/final prompt `744/376`、合计 `1,120`、Provider total `1,185`，相对原始完整路径 `11,050` 下降约 `89.9%`，相对上一版 `2,325` 再下降约 `51.8%`；两次均为 `exact_match`，回归上限 `900/450`、总上限 `1,400`。Runtime 继续保留工具结果、SSE、结构 VERIFY 与调用审计；提议无法安全采用时失败关闭或回退既有工具循环，不猜参数。
