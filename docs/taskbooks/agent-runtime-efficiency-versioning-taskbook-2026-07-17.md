...
- 2026-08-03 13:45:32 的最新单只读工具边界：自包含且来源可证明为内置的单个 `glob / grep / read` 进入独立 `decide_explicit_tool` 契约，不加载历史、附件、记忆、完整 Workflow 或 bootstrap；模型只生成最小需求字段和一个 schema-bound 提议，Runtime 再展开为既有 TaskBook 并复核来源、名称、schema、路径边界、权限和副作用。附件、历史指代、记忆介入、恢复态、运行时事件、同名插件/run-scoped 工具、多工具、写入、执行或边界不明请求自动回退完整 `decide`。最新真实 `glob` 为 2 次 API、1 次工具、`2.901s`；紧凑决策 prompt `629`，全程 prompt `1,479`、Provider total `1,702`，两次本地 tokenizer 均为 `exact_match`。该结果替代“当前单工具基线”，但不改写上句 `5.642s` 的历史验收记录，也不外推到多工具或写入任务。
