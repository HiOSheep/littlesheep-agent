...
## 10.219 S1 第二样本（长会话）确认：**两负载均为正向，代码已推送**（2026-09-18）

| 指标 | 长会话基线 | **S1 后** |
| --- | --- | --- |
| `failedRuns` / `publishedRuns` / `silentRuns` / `semanticFailures` | 0 / 120 / 0 / 0 | **0 / 120 / 0 / 0** ✓ |
| 主对话 hit | 73.7 / 74.0% | **74.2 / 74.7%** |
| 总体 hit（分析器） | 74.1% | **74.7%** |
| 总体 miss/调用 | 1,092–1,107 | **1,077** |
| **`execute_tool_loop` miss/调用** | **2,223–2,447** | **1,900.5（−14%）** ✓ |
| `reply` miss/调用 | 990–1,058 | **959.0** |
| `decide` | 1,351 | 1,331.6 |
| `execute_final_reply` | 870 | 855.5 |

**两负载汇总（S1 = 工具循环不再重复发送原生已覆盖的工具文本）**：

| 负载 | hit | miss/调用 | 结论 |
| --- | --- | --- | --- |
| 短会话 8×5 | 74.0 / 74.4% | **685.3 / 696.9（<700 ✓）** | **判据 ① 的 miss 达标** |
| 长会话 8×15 | 74.2 / 74.7% | **1,026.6 / 1,054.5** | **正向（+0.5–0.7pt）** |

**正确性/质量**：两次样本 `failedRuns=0`、`silentRuns=0`、`publishedRuns` 满额、`semanticFailures=0` ⇒ **未出现能力退化** ✓
**代码**：`fdfb0b5`（首次推送遇 TLS 临时错误，已重试推送）。

**归因**：工具循环单次 prompt 减少约 4,100 字符（≈1,100 token），长会话中工具循环占 23/263 次调用 ⇒ 摊薄后总体 miss 下降约 15–30/次；**工具循环自身的 miss 下降 14%**，与"被删文本此前位于 system 尾部、未被缓存"的判断一致 ✓

**⇒ 判据现状**：
- **①**：hit **74.0–74.7%**（距 75% 差 0.3–1.0pt）；**miss/调用短会话 685–697（<700 ✓ 达标）**、长会话 1,027–1,055；
...
## 10.226 **S3 已生效**（reply system 6,616 → 5,759）+ **策略结论：ratio 只能靠减少"新增"取得**（2026-09-18）

**直接证据（逐 purpose 首条 system 消息长度，同一脚本）**：

| 样本 | `reply` 首条 system 长度 |
| --- | --- |
| S1 后（`live-notooltext-short`） | **6,616 字符**（49 次调用，单一值） |
| **S3 后（`live-s3c-short`）** | **5,759 字符**（52 次调用，单一值） |

⇒ **S3 每次 `reply` 调用省 857 字符**，与预测（−870）**完全一致** ✓ staged 渲染已生效（此前"未生效"的判断是**被样本均值误导**：两次样本的 run 构成不同，均值/prompt 不可直接比较）。

**但 ratio 未改善（73.5% vs 74.4%）—— 由此得到关键策略结论**：
- 命中率 = **cached / total**。S3 删除的 `core-flow` 文本**原本位于被缓存的前缀内** ⇒ 删除它使**分子与分母同时变小** ⇒ **ratio 不升反可能微降** ✓ 与实测一致；
- ⇒ **凡"删除/后置已缓存内容"的改动都只降低 *成本*（prompt 更短），不提升 *ratio***（这解释了 S、`final_reply` 契约、B1 单侧的中性，以及 S3 的 +0 效果）；
- ⇒ **提升 ratio 的唯一方向是减少"每次新增的内容"**（前缀缓存无法复用的部分），即调用**尾部**的逐轮变化量：
  | 尾部项 | 每轮字符 | 估计影响 |
  | --- | --- | --- |
  | **`runtime-awareness`** | **706** | 若压到 ~250 ⇒ 每次少 ~450 字符（≈115 token）新内容 ⇒ ratio **+1.5–3pt** |
  | `user` 入站消息 | ~30 | 必要，不可减 |
  | 助手新回复 | ~15–60 | 必要 |

**⇒ 结论：为达成判据 ① 的 hit ≥75%，应做 S4（`runtime-awareness` 紧缩）**，而不是继续做"缩短已缓存部分"的改动（S1/S3 的价值在于**成本与提示简化**，符合用户"简化提示词"的目标，予以保留）。
...
## 10.233 S4 第二样本（长会话）：**噪声量级、略偏负** ⇒ 诚实结论与取舍（2026-09-18）

| 负载 | S1 后 | S3 后 | **S4 后** |
| --- | --- | --- | --- |
| 短会话 hit | 74.4 / 74.0% | 72.5 / 73.5% | **73.4 / 73.1%** |
| 短会话 miss/调用 | 685.3 / 696.9 | 730.1 / 710.6 | **674.9 / 711.3** |
| 长会话 hit | 74.2 / 74.7% | — | **73.7 / 72.8%** |
| 长会话 miss/调用 | 1,026.6 / 1,054.5 | — | **1,064.9 / 1,111.1** |
| 正确性（两负载） | 全绿 | 全绿 | **全绿**（`failedRuns=0`/`silentRuns=0`/`publishedRuns` 满额/`semanticFailures=0`） |

⇒ **S4 的 ratio 效果为噪声量级（长会话略偏负 −0.5pt）**；它**确实缩短了尾部**（约 140 字符/次 ≈ 35 token）⇒ 属**成本/简化**收益，而非 ratio 收益。

**取舍决定：保留 S4**（理由：① 它**不减少任何事实**、不动能力；② 与 S1/S3 同属"提示简化"，符合用户第二目标；③ 其 +/−0.5pt 在多次取样噪声范围内，而成本方向恒为正）。**如实标注其 ratio 增益为 0**。

**⇒ 本会话最终判据（据全部 6+ 次短会话与 4 次长会话样本）**：
- **判据 ①**：**miss/调用 短会话 674.9–730（<700 多数样本达标 ✓）**；**hit 72.5–74.6%**（多次集中在 **74.0–74.7%**，差 0.4–2.5pt）；
- **判据 ②**：长会话 hit **72.8–74.7%** ⇒ **不可达**（结构性，10.203）；
...
## 10.240 **taskbook-as-skill 验收通过** + 本会话最终交付汇总（2026-09-18）

### 长会话第二样本（8×15，120 run）

| 指标 | S4 后 | **taskbook-as-skill 后** |
| --- | --- | --- |
| `failedRuns` / `publishedRuns` / `silentRuns` / `semanticFailures` | 0/120/0/0 | **0/120/0/0** ✓ |
| 主对话 hit | 73.7 / 72.8% | **73.5 / 73.0%** |
| miss/调用 | 1,064.9 / 1,111.1 | **1,058.8 / 1,017.9** |
| `reply` miss/调用 | 1,034.7 | **948.2** |
| `decide` | — | 1,313.1 |

⇒ **无退化**（正确性与发布数全绿；path2 的 miss/调用为本会话长样本最低），模型经 `use_skill` 拉取 taskbook ⇒ **按需通道成立**。

### 本会话全部落地项（均已在远端 `main`，均未裁剪能力）
...
| `final_reply` 契约移出 system | `9502d3b` | 中性 |
| S：`memory-root-index` 归稳定区 | `f9231e6` | 中性 |
| **S1 工具文本去重** | `fdfb0b5` | 短会话 miss **<700**、工具循环 **−14%** |
| **S3 按 stage 渲染流程段** | `9faada0`+`09f41ee` | `reply` system **−857 字符/次** |
| S4 尾部标签压缩 | `d15627e` | 成本/简化（ratio 增益为 0） |
| **taskbook-as-skill（四步）** | `485d58c`、`75a29bc`、`a780b7a`、`4f7abe8`、`b7aa807` | execute 调用卸下 ≈1,630 字符/次；两负载均无退化 |
| 诊断工具入仓 | `268a760`、`49be732` | `scripts/analyze-prompt-cache.mjs`、`scripts/analyze-cache-shapes.mjs` |

**已如实入档的中性/负面**：`c692dcc`、`8827b90`、B1×2（全局 −14pt、单侧中性）、跨 purpose system 统一（−14pt 已回退）、D1（前提被数据否证）。

### 判据（据全部样本）

| 判据 | 现状 | 结论 |
| --- | --- | --- |
| **① miss/调用 <700** | 短会话 **674.9–730**（多数样本达标） | **基本达标** |
| **① hit ≥75%** | **72.5–74.6%**（多次集中 74.0–74.7%） | 差 **0.4–2.5pt** |
| **② 长会话 ≥95%** | **72.8–74.7%** | **结构性不可达**（§10.203：每轮真实新增不可消除；各 purpose 上下文差异属设计取舍） |
| **硬约束** | `failedRuns=0`、`silentRuns=0`、**未裁剪能力** | ✓ 全程满足 |
...
## 10.246 **有界历史窗口：短会话首次两项判据同时达标**（2026-09-18）

**改动**（`7dfb307`，3 文件 7 处）：能力回复（`reply.ts:101/348`）、紧凑决策（`decide/request.ts:177`）、紧凑只读执行（`task-step-runner.ts:151`）由"**不发历史**"改为 **`recentHistoryForModel(ctx.history)`**（既有辅助，**8 条 / 6,000 字符**）⇒ 这些路径的请求前缀**与主路径一致** ✓
**`verify` 不在其内**（10.244：它在**任何**窗口下都破坏 runner 的 continuation/checkpoint，9 例固定失败）。

**门禁**：`typecheck` clean、**全量 3,287 通过（0 失败）** ✓、`check:repo` 33/33、continuity + UI 门 ok。
...
| --- | --- | --- |
| `failedRuns` / `publishedRuns` / `silentRuns` / `semanticFailures` | 0/40/0/0 | **0/40/0/0** ✓ |
| **主对话 hit** | 74.4 / 74.0% | **76.3 / 75.1%（双双 ≥75% ✓）** |
| **miss/调用** | 685.3 / 696.9 | **698.3 / 676.1（双双 <700 ✓）** |
| `decide` miss/调用 | 1,225.6 | **1,148.8** |
| **`execute_tool_loop`** | 1,876–2,259 | **1,471.3** |
| `execute_final_reply` | 820–855 | **790.0** |
| `reply` | 494.8–509.7 | **490.9** |

⇒ **判据①的两项首次同时达标**（hit ≥75% **且** miss/调用 <700），且**每个 purpose 的 miss 都在下降** —— 正是"各 purpose 前缀一致 ⇒ 跨 purpose 复用"的预期机制 ✓

**待办**：长会话第二样本（正在跑）⇒ 若 `publishedRuns` 满额、hit 不降，即确认保留并推送；随后做**授权②**（`core-flow` 仅保留相邻转移）。

## 10.247 有界历史窗口：长会话第二样本（噪声内略偏负）⇒ **保留**，判据①达成（2026-09-18）

**长会话（8×15，120 run）**：

| 指标 | taskbook-as-skill 后 | **有界窗口后** |
| --- | --- | --- |
| `failedRuns` / `publishedRuns` / `silentRuns` / `semanticFailures` | 0/120/0/0 | **0/120/0/0** ✓ |
| 主对话 hit | 73.5 / 73.0% | **72.7 / 73.1%** |
| miss/调用 | 1,058.8 / 1,017.9 | **1,136.1 / 1,044.0** |
| `reply` miss/调用 | 948.2 | 989.4 |
| `decide` | 1,313.1 | 1,331.6 |
| `execute_final_reply` | — | 825.7 |

**⇒ 长会话为噪声范围内（−0.8pt / +0.1pt），略偏负；短会话则明显改善**：

| 负载 | 改前最好 | **改后** |
| --- | --- | --- |
| **短会话 hit** | 74.4 / 74.0% | **76.3 / 75.1%（≥75% ✓）** |
| **短会话 miss/调用** | 685.3 / 696.9 | **698.3 / 676.1（<700 ✓）** |
| 长会话 hit | 73.5 / 73.0% | 72.7 / 73.1% |

**取舍决定：保留**。理由：① **判据①（短会话两项）首次同时达标**，且该判据本就针对短会话；② 长会话变化在多次取样的噪声带内（±0.8pt）；③ **未裁剪任何信息**（这 3 处由"不发历史"改为"8 条/6k 的有界历史"，信息**只增不减**）；④ 正确性两负载全绿。
**如实标注**：本改动对**长会话无正收益**（略偏负），其价值集中在短会话判据①与"各 purpose 前缀一致"的结构简化。
...
## 10.250 授权②实测：**hit −3pt（跌破 75%）、miss/调用却创新低** ⇒ **按判据①回退**（2026-09-18）

**改动**（`adb3a4a`，随后 **`git reset --hard` 移除**）：staged 流程段由「5 行图 + 代码围栏」改为**一行纯 ASCII 路线**，并用正则删除无用的 `CORE_FLOW_DIAGRAM` 常量（**不触碰字形**）。门禁：`typecheck` clean、**全量 3,286 通过**（1 例为 `development-environments.test.ts` 的**环境性抖动**，单独运行 6/6 通过）、`check:repo` 33/33、continuity + UI 门 ok。
...
| --- | --- | --- |
| `failedRuns`/`publishedRuns`/`silentRuns`/`semanticFailures` | 0/40/0/0 | 0/40/0/0 ✓ |
| **主对话 hit** | **76.3 / 75.1%** | **73.2 / 72.9%（−3.1pt，跌破 75%）** |
| **miss/调用** | 698.3 / 676.1 | **658.2 / 673.3（为本会话最低 ✓）** |
| `reply` 首条 system | 5,759 字符 | **5,578（−181）** ✓ |

**⇒ 结论（与 10.226 完全一致）**：②**削减了"已被缓存的前缀"**（`core-flow` 在稳定头内、本就被复用）⇒ **分子下降、分母不变** ⇒ **ratio 下降**；而 miss/调用 的改善来自总 prompt 变小。
**⇒ 按判据①（需 hit ≥75% **且** miss/调用 <700）判定：② 使 hit 跌破 75% ⇒ 不采用，已回退。**
...
## 10.260 **R1 落地并实测：调用数达标（−10%）、重写几乎消失，但 ratio −3pt、uncached/run +6%**（2026-09-18）

**实现**（`d91169c`，**零测试改动**）：`reply.ts` 首答新增一条**尾随 `system` 消息**，内容 = 重写轮本会用的"需避免逐字重复的近期答复"清单（`collectRecentAssistantReplies` + `MAX_AVOID_REPLY_COUNT/CHARS`）；**能力回复跳过**（语义自包含）；尾随 `system` 角色使其**不触碰被缓存前缀**、也不破坏 `model-request-characterization` 的"前 5 条之后均为 system"不变量 ✓
**门禁**：`typecheck` clean、**全量 3,287 通过（0 失败，零测试改动）**、`check:repo` 33/33、continuity + UI 门 ok。
...
| --- | --- | --- | --- |
| `failedRuns`/`publishedRuns`/`silentRuns`/`semanticFailures` | 0/120/0/0 | **0/120/0/0** | ✓ |
| **模型调用/run** | **2.14** | **1.93** | ✅ **−10%** |
| **`rewriteCount` 分布** | 0→67、1→29、2→24 | **0→91、1→6、2→23** | ✅ **重写几乎消失** |
| 单 `reply` run 数 | 65 | **89** | ✅ |
| **主对话 hit** | 72.7–73.5% | **70.0 / 69.2%** | ⚠️ **−3pt** |
| **uncached / 调用** | 1,136 / 1,044 | **1,188.6 / 1,261.9** | ⚠️ 变差 |
| **uncached / run**（`run-shapes`） | 2,287 | **2,418** | ⚠️ **+6%** |

**⇒ 这是一个明确的权衡，不是单边胜利**：
- **赢**：**少 10% 的模型调用**、**重写循环几乎消失**（44% → 24%）、单次 `reply` 的 run 从 65 → 89；
- **输**：**ratio −3pt**、**uncached tokens/run +6%**。
- **机制**：被省掉的"重写调用"恰恰是**高命中**的（它们复用同一前缀）⇒ 去掉它们会**拉低平均命中率**；而新增的回避清单让首答 prompt 变大 ⇒ uncached 略升。

**⇒ 决策依据（用户原则 4）**："99% 命中不一定优于 95%，若后者调用 3 次而非 8 次" ⇒ 本项**减少了调用**（更贵的维度）却**增加了 uncached tokens**（也重要）⇒ **两者相抵**。

**重要 caveat（影响结论）**：本基准用 `UNIQUE_TURNS=1` + 8 任务 × 15 轮 ⇒ **同题反复**，重写循环被**放大**；真实使用中重写更少 ⇒ **R1 的调用节省会更小，而其 prompt 成本不变**。⇒ 建议：**保留**（调用数是用户强调的维度，且重写消失对体验/延迟有利），但**在长会话 ratio 判据上如实标注 −3pt**；若用户更看重 ratio 与 uncached，则应**回退 R1**。

**下一步**：跑**短会话（8×5）第二样本**确认（判据①是否仍达标）⇒ 再据上表决定保留/回退。

## 10.261 **R1 决定：回退**（短会话判据①被破坏）（2026-09-18）

**短会话（8×5）第二样本（R1 之后）**：

| 指标 | R1 之前（达标） | **R1 之后** |
| --- | --- | --- |
| 正确性 | 0/40/0/0 | 0/40/0/0 ✓ |
| **主对话 hit** | **76.3 / 75.1%** | **72.6 / 70.5%（−3.7pt，跌破 75%）** |
| **miss/调用** | **698.3 / 676.1** | **809.9 / 830.0（>700）** |
| `rewriteCount` | — | 0→30、1→5、2→5（重写确已减少） |

**⇒ 判定：回退**。理由：**判据①（hit ≥75% 且 miss/调用 <700）是目标的头部判据**，而 R1 用 **−3.7pt hit / +130 miss per call** 换取了 **−10% 调用数**；且本基准（`UNIQUE_TURNS=1`、同题反复）**放大**了重写循环 ⇒ 真实使用中调用节省更小、而 prompt 成本不变 ⇒ **代价/收益不成立**。
**处置**：`git revert d91169c`（保留历史），全量套件复跑确认绿。
**保留的记录**：R1 的完整数据（§10.260/10.261）与"调用数 vs uncached/ratio 的权衡"结论 —— 它验证了用户原则 4 的**反面**：**若收益来自"删掉高命中的调用"，则 ratio 与 uncached 会同时变差**，此时应以 ratio/uncached 为准。

**当前保留状态（判据①达标）**：`7dfb307`（有界历史窗口）⇒ 短会话 hit **76.3 / 75.1%**、miss **698.3 / 676.1**、`failedRuns=0`、`silentRuns=0`。

**下一轮回到用户优先级 #3/#5**：继续统一稳定缓存前缀，或清理隐式 Context 依赖（D1：runner 的 continuation/checkpoint 验证路径）。

## 10.262 优先级 #3 现状测量：**各 purpose 各自稳定，但跨 purpose 差异极大**（2026-09-18）

**脚本**：工作区 `syslen.mjs <dataDir>`（按 purpose 统计首条 system 消息长度分布）。**样本**：`live-bounded-long`（8×15，120 run）。

| purpose | 调用数 | 首条 system 长度 |
| --- | --- | --- |
| `reply` | 154 | **5,759**（单一值 ✓） |
| `execute_final_reply` | 42 | **1,646**（单一值 ✓） |
| `decide` | 30 | **15,520**（单一值 ✓） |
| `execute_tool_loop` | 15 | **12,606**（单一值 ✓） |
| `verify` | 15 | **3,264**（单一值 ✓） |
| `classify` | 1 | **942** |

**两点结论**：
1. **同 purpose 连续调用已完全稳定**（每个 purpose 只有一个取值）⇒ 同 purpose 复用良好 ✓（这是本会话多次改动的结果）；
2. **跨 purpose 差异巨大**（942 → 15,520）⇒ **跨 purpose 只能复用公共头（≈2.9k 字符）**，其后整段历史仍会重算。

**⇒ 新的机会窗口（10.200 的结论需重新评估）**：
- 10.199 曾试过"全局把边界之下段移到尾部"⇒ **回归 −14pt**，当时诊断的**主因是"各 purpose 的历史窗口不同"**（10.200：`verify` 传 `history: []`、能力回复清空、紧凑路径清空）；
- **而这一层已被 `7dfb307`（有界历史窗口）修掉**（除 `verify` 外，各路径已共用同一有界窗口）⇒ **"统一 system 头"的前提已部分具备** ⇒ 值得**重新测试**（此前失败可能是被历史差异掩盖）。
...
## 10.275 **D1 决定性结论：`verify` 带历史使续跑调用数近乎翻倍（+4 次）⇒ 不应作为缓存优化实施**（2026-09-18）

**实验（第 1–3 步已执行，插桩与改动均已回退，树干净）**：
- `verify/model-call.ts`：`history: []` → `history: ctx.history.slice(-8)`；
- 测试在该用例断言前打印 `requests.map((r) => r.callContract?.purpose)` 的**调用序列**；
- 同一用例（`Promise.all([runner.run(continuationInput), runner.run(continuationInput)])` ⇒ 两个 run 共用 `requests`）：

| 情形 | 调用序列长度 | 用例结果 |
| --- | --- | --- |
| **带 `verify` 历史** | **9 次调用**（8 个 ` > ` 分隔） | **1 failed** |
| **不带（基线）** | **5 次调用**（4 个 ` > `） | **1 passed** |

（`callContract?.purpose` 在该路径**未填充** ⇒ 序列为空串，但**次数是决定性的**。）

**⇒ 结论**：
1. **`verify` 看到历史后，续跑流程的模型调用数从 5 → 9**（按 run 计约 2.5 → 4.5，近乎翻倍）⇒ **这是行为变化，不是缓存问题**；
2. 结合 10.273（替身是队列、取空即抛）⇒ **多出的 4 次调用**极可能是 **verify 判定改变**（历史中含既有目标/答复 ⇒ 它给出 `needs_replan`/`fail` 而非 `pass`）⇒ 流程进入**重规划/恢复**分支 ⇒ 调用激增并在测试中耗尽脚本 ⇒ `status='error'` ✓
3. ⇒ **按用户指标 #4（模型调用数）与硬约束（不裁剪能力/不降质量），本项应被拒绝**：它不是"让 `verify` 共享前缀"的免费收益，而是**改变 verify 判定语义** ⇒ 需**端到端行为验证**才能评估，收益（长会话 ratio）与代价（调用数翻倍、行为改变）明显不划算。

**⇒ D1 处置：收束（不作为缓存优化实施）**。
- 保留已完成的 **#1**（`response-continuity-exposure` 纯身份匹配，`7bc05b5`）—— 它是**独立的架构债改善**、零回归 ✓；
- **不再推进**"让 `verify` 携带历史"；`verify` 的 `history: []` 维持现状（它是**设计选择**，不是待偿债务 —— 本会话的实证改正了 10.265 的定性）；
- ⇒ **长会话可达上限维持 ≈74–76%**（判据② 的收束依据）。

**⇒ 本会话对 D1 的最终净结果**：① 推翻了一个错误的架构定性（10.265 → 10.272/10.273/10.275）；② 落地一项真实的架构债改善（#1 身份匹配）；③ 用一个**可复现的量化实验**（5 vs 9 次调用）否掉了一个看似有利的改动 —— 避免了把"调用数翻倍"的行为变化当成缓存收益发布。

## 10.276 **#6 扩新能力候选清单（带预期收益与复杂度预算影响）**（2026-09-18）

**评估口径**（用户原则 9/10）：每项回答"用户获得什么"（更快/更聪明/更稳定/更省），并标注对 **complexity budget** 的影响（模型调用数、系统 prompt 长度、RunContext 字段数、核心 package 数、状态数、恢复路径数）。

| # | 候选能力 | 用户获得 | 预期收益（可量化） | 预算影响 | 风险 | 判定方式 |
| --- | --- | --- | --- | --- | --- | --- |
| **N1** | **缓存回归门**：把本会话两个诊断脚本（`analyze-prompt-cache.mjs`、`analyze-cache-shapes.mjs`）与 `syslen.mjs` 变成**常驻入口**（如 `pnpm run audit:cache <dataDir>`）+ 文档化阈值 | **更稳定**（防回退） | 让"hit/miss/调用数"成为每次改动的可复核指标（本会话 4 次负面/中性改动全靠它拦住） | **0 新增运行时复杂度**（只加脚本与文档） | 低（无生产代码改动） | 在两次样本上跑通并输出固定格式 |
| **N2** | **指标采集落地**：补 `Time to first useful action`（日志字段名待确认：`toolCalls`/`toolInvocations` 的时间字段） | **更省/更快**（可观测） | 让"首个有用动作"成为与 `task completion latency`（已有 p50 1.28s / p95 5.66s）并列的指标 | 0 运行时；仅诊断脚本 | 低 | 在 `live-*` 样本上打印 p50/p95 |
| **N3** | **更多知识 Skill 化**（延续 taskbook-as-skill）：把"项目约定/工具使用指南/流程知识"等**可延迟**内容从常驻 prompt 移入 skill（`registerDynamic` 机制已就绪 ✓） | **更快更省**（同时提命中率） | 每移出 1k 常驻字符 ⇒ 所有 purpose 的 prompt 变小（**hit↑、uncached↓**，本会话唯一有效策略） | **系统 prompt 长度 ↓**；skill 数量 ↑（但按需加载） | 中（需确认模型确实会拉取；taskbook-as-skill 已证可行 ✓） | 两次样本：`syslen.mjs` 各 purpose 首条 system 长度下降且 hit 不降 |
| **N4** | **记忆按需化**（原则 3）：`memory-root-index` 等"常驻但可推导/可读取"的内容改为**按需检索** | **更快更省** | 同上（−常量字符）；并让"记忆可见性"从"常驻"变为"可达"（不减少能力） | 系统 prompt 长度 ↓ | 中（须保证模型仍能发现并按需读取） | 两次样本 + 记忆相关套件 |
| **N5** | **Runtime 去 TaskBook 化**（D2 债的**能力版**，原则 2）：让 Runtime 只认 action/effect/result/checkpoint，TaskBook 完全落在 skill 层 | **更稳、更简洁**（可组合） | 消除 D2 架构债；为"新能力以 skill 形式加入"铺路 | **RunContext 字段数 ↓**、状态数 ↓ | 高（跨文件；调度/校验/证据 6 处） | 全量套件 + 两条 Electron 门 |
| **N6** | **Soak/真实负载权重**（原则 8）：2 小时长会话 soak + 崩溃恢复演练（不只断言结构存在） | **更稳** | 暴露"长会话越来越慢/恢复路径失效"类问题 | 0 运行时；+1 脚本 | 低 | soak 报告（失败=0、内存/延迟曲线） |
| **N7** | **`verify` 判定语义改进**（D1 副产物）：不给完整历史，而给**目标与当前答复的摘要**，使 verify 判定更准 | **更聪明** | 可能减少无效 replan（当前带完整历史 ⇒ 调用 5→9 ✗） | 0 字段新增（用既有上下文） | 中高（改判定语义 ⇒ 需行为验证） | 续跑 8 例 + 两次样本（调用数不得上升） |

**建议顺序（按"收益/风险"与用户优先级）**：
1. **N1 + N2**（零运行时风险、立即让后续每步都可度量 —— 也补上本会话唯一缺失的指标）；
2. **N3**（延续已完成的 taskbook-as-skill，**同时**提命中率与降 prompt ⇒ 唯一被本会话三次验证有效的方向）；
3. **N4**（同策略的下一处；需保证"按需可达"）；
4. **N6**（稳定性）；
...
## 10.288 **判据①的如实口径：边界状态**（两份独立样本，2026-09-18）

**第二份独立短会话样本（8×5，当前代码 = `7dfb307` 有界历史窗口后）**：

| 指标 | 样本 A（`live-bounded-short`） | **样本 B（`live-final-short`）** |
| --- | --- | --- |
| `failedRuns` / `publishedRuns` / `silentRuns` / `semanticFailures` | 0/40/0/0 | **0/40/0/0** ✓ |
| **hit** | **76.3 / 75.1%** | **73.3 / 74.2%** |
| **miss/调用** | **698.3 / 676.1** | **681.8 / 700.3** |
| `audit:cache` 总体 | — | hit 72.9%、uncached/call 777.6 |
| 模型调用/run | — | avg **1.98**、p50 **1**、p95 **7** |
| 任务时延 | — | avg 1,495 ms、p50 1,007 ms、p95 4,947 ms |
| **首个有用动作** | — | avg **81 ms**、p50 **78 ms**、p95 **96 ms** |
| `rewriteCount` | — | 0x28 / 1x5 / 2x7 |

**⇒ 如实结论：判据① 处于"边界"状态**，而不是稳定达标：
- **hit**：样本 A 双路径 **≥75%** ✓；样本 B 双路径 **<75%** ✗（73.3 / 74.2）⇒ 阈值 75% **落在抽样离散带（≈73.3–76.3%）之内**；
- **miss/调用**：两份样本的四个读数 **676.1 / 681.8 / 698.3 / 700.3** ⇒ **三个 <700、一个 700.3 恰好越界** ⇒ 同样**贴线**；
- **正确性**：两份样本**全绿** ✓（`failedRuns=0`、`silentRuns=0`、`publishedRuns` 满额、`semanticFailures=0`）。

**⇒ 这一更正很重要**：此前"判据① 达标"的表述**基于样本 A 单次读数**；补第二份样本后应表述为"**hit ≈73–76%、miss/调用 ≈676–700，两者都紧贴阈值**" ⇒ 若要把它变成**稳定达标**，需要**结构性**增益（而非抽样运气），而本会话已穷尽"不裁剪能力"范围内的此类手段（唯一有效的"扩大可复用前缀"已全部落地：共享头、契约后置、有界历史窗口）。

**⇒ 对判据②（长会话 ≥95%）**：无变化 —— 结构性不可达（72.7–73.5%，§10.203）。

**⇒ 阶段收尾的如实交付**：判据① **贴线**（该负载形态下的可达区间约 **73–76%**，`miss/调用` 约 **676–700**）；判据② **不可达**；**硬约束全程满足**（未裁剪工具集/提示信息/记忆可见性，8 项负面/中性改动全部如实入档并回退）。

## 10.289 **阻塞命中率进一步提高的事项清单（供用户取舍）**（2026-09-18）

### A. 结构性不可消除（与能力取舍无关，属负载与协议本质）
| # | 阻塞项 | 机制 | 量化证据 |
| --- | --- | --- | --- |
| A1 | **每轮必然新增内容** | 用户消息 + 助手回复 + 逐 run 运行事实（`runtime-awareness` ≈566 字符）在**尾部**，provider 从 token 0 匹配 ⇒ 每次调用都有一段**不可复用**的新增 | 纯 `reply` 上限 **≈89%**（§10.160/10.170） |
| A2 | **execute 家族的工具块冷启动** | 工具 schema/工具文本位于 system 前段；工具集变化即**整段前缀失效**，每次 execute 冷启动 **≈4.4–5.3k token** | 长会话落地 **66.8–68.4%**（§10.170） |
| A3 | **长会话的历史平移** | 每个新回合都把历史窗口整体后移 ⇒ 后续每条请求的前缀比上一条短 | 长会话 hit **72.7–73.5%**，短会话 **73.3–76.3%**（§10.288） |

### B. 设计取舍（要动必须付出代价；均已实测或已证协议依赖）
| # | 阻塞项 | 为什么不能白拿 | 证据 |
| --- | --- | --- | --- |
| B1 | **`verify` 的 `history: []`** | 已证**正确设计**：它的提示本就自包含（目标+答复+契约+证据）；带历史使续跑调用 **5→9** 且失败 | §10.275 / §10.284 |
| B2 | **`decide` 的 `tooling` 文本（4,179）** | 是 `decide` **唯一**工具通道（它没有 provider 工具）⇒ 移出=模型看不到工具 | §10.264 |
| B3 | **常驻政策与权威事实**（`safety`、`core-flow`、`capabilities`、`output-directives` 1,805） | 移出会削弱保证/诚实护栏，或每次都要加载（更差）；**且实测删除常驻内容会降低 hit** | N4：hit **−1.5～−4.6pt**（§10.281）；S3/core-flow 同类（§10.250） |
| B4 | **各 purpose 的 system 头互不相同**（942 / 1,646 / 3,264 / 5,759 / 12,606 / 15,520） | 跨 purpose 只能复用**公共头 ≈2.9k**；统一它们必然触碰 B2/B3 | §10.262–10.264 |

### C. 口径与抽样（不是代码问题）
| # | 阻塞项 | 说明 |
| --- | --- | --- |
| C1 | **阈值贴着离散带** | hit 抽样 **73.3–76.3%**、miss/调用 **676–700** ⇒ 单次达标 ≠ 稳定达标；稳定达标需**结构性**增益 |
| C2 | **不存在"免费提升"的手段** | 本会话 4 次验证：**删除已缓存内容**或**删除高命中的调用**都会**降低 hit**（S3 / core-flow 精简 / N4 / R1） |

### D. 已被实测否证的手段（勿重犯）
| 手段 | 结果 |
| --- | --- |
| 全局把稳定边界之下内容移到尾部（跨 purpose 统一头） | **−14pt**（已回退） |
| `core-flow` 精简 | hit **−3pt**（已回退） |
| R1（首答告知需回避的近期答复） | 调用 **−10%** 但 hit **−3.7pt**（已 revert） |
| N4（记忆索引上限 2,400→800） | hit **−1.5～−4.6pt**（已 revert） |
| D1（`verify` 带历史） | 续跑调用 **5→9** 且失败（维持现状） |

### E. **只有你能解锁的选项（每条标出代价）**
| 选项 | 换来什么 | 代价 / 风险 |
| --- | --- | --- |
| **E1 授权削减/后置常驻内容**（`output-directives` 1,805、`profile`、`core-flow` 剩余部分） | 每请求变小 ⇒ **uncached↓**；若改为 skill/按需则**不必删除** | 与 B3 实测相反：**hit 可能下降**（内容仍在同一稳定头内）⇒ 只有"移出稳定头 + 每次必然加载"才可能同时降 uncached 且不伤 hit，需实测 |
| **E2 授权改变工具块形态**（如固定工具集、或把 schema 从 system 前段移走） | 消 A2 ⇒ 长会话潜在 **+5–10pt** | 需先验证 **provider 对 `tools` 参数的缓存行为**（`tools` 不在 `messages` 内，位置由 API 决定）；且改变工具可见性=**能力风险** ⇒ 需你明确授权 |
| **E3 授权缩小历史窗口**（8 条/6k → 更小） | 前缀更早稳定 ⇒ hit↑ | 模型上下文减少 ⇒ 属"能力不裁剪"的**边界**，需你认可 |
| **E4 接受当前上限** | 无需改动 | 判据② 记为 **74–76%**；判据① 记为"**贴线达标（可达区间 73–76%）**" |
