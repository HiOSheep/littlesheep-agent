...
最后更新：2026-09-22 02:46:00

日期：2026-09-21。
状态：**清单 41 项全部完成**（SP-01～SP-07 有回归证据；SP-08 已建立结构基线、完成真实 Provider 两组负载读数，并在用户授权下于验收文档末尾追加更正一节，见 SP-08 记录）。**缓存 95% 目标未达成**——三组真实负载总体命中率 78.10/78.18%（对话）、83.69/84.52%（连续工具）、62.83/63.23%（压缩），如实记为实际差距，未改小目标、未剔除冷启动、未挑选热缓存子集。
遗留（**不属于本清单条目**）：仓库自检 `check:repo` 为 32/33，唯一失败是 `packages/harness/src/cache-observability.ts` 超出其受控行数上限，全部来自作者未提交的 +21 行修复；处置已确认为"作者先提交、随后在干净历史上拆分该函数"。
...
- [x] 修正验收文档中过时汇总与未经证实的因果归因；正文保存有界脱敏汇总，原始提示、会话和密钥不进入仓库。
...
- **第 3 条部分完成（本轮补记）：**
  ...
  - **"修正验收文档过时汇总"已补记（2026-09-22）：** 用户授权在**不改动 5.50 节正文**的前提下追加更正，已在 `docs/reference/cache-95-acceptance.md` 末尾新增 **"## 7. 2026-09-22 实测更正"**（`git diff --stat` 为 `74 insertions(+)`，即纯追加、未改任何原有行）：写明两套配置不可互读、给出三组实机负载读数与分项、更正文末状态标记的两句（压缩 54.808% 的旧读数、以及"连续工具工作与重启尚未测量"）、并重申 95% 红线仍未达成。**该文件与用户 5.50 节同属未提交改动，本轮不代为提交**，等作者一并提交。
...
- **仍未修（1 项，属于用户的未提交改动）：** `packages/harness/src/cache-observability.ts` 当前 **696 行 > 受控上限 680**。该文件的未提交改动（`splitRequestForCache` 的"前导 system 块才算可缓存头"修复，+21 行）在 HEAD 上是 675 行、门禁是绿的；本轮**没有改动该文件**（其间一次尝试已完整回滚并逐行还原，`git diff --stat` 仍精确等于 `21 insertions(+)`，其自带测试 14 项通过）。**处置已与用户确认（2026-09-22 第 15 轮）：作者先提交自己的 +21 行修复，之后再在干净历史上把 `splitRequestForCache` 拆到独立模块**（预计降到 600 行以内，受控登记随之移除）；在那之前门禁保持这一项失败，且不由本轮代做，以免覆盖在途编辑。
