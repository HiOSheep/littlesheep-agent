...
版本：v2.0
状态：实施中；阶段 0-5 已完成，阶段 6 的同源读取控制面与只读迁移预检已完成；正式迁移执行、恢复/回滚控制和 atom 写管理仍待停写协议与用户批准；未迁移正式用户数据
...
- 已完成：19 项迁移测试覆盖全部 9 个声明断电点、预检和中途 ENOSPC、损坏 JSON、损坏节点、孤儿 parent、重复 id、快照后 v2 变化、v3 写入后拒绝回滚、模型不可用、重启恢复和实验标记旁路；
- 正式用户数据仍保持 v2，迁移管理器没有接入应用自动启动或设置页，直到用户单独批准。
...
- 已验证：仓库卫生 31/31、27 个 workspace 类型检查、144 个测试文件中的 1127 项通过且 1 项按预期跳过、Electron main/preload/renderer 构建和应用恢复源检查通过。

验收：已通过隔离工程验收。网络被阻断时记忆写入和检索正常，向量搜索不能绕过层级导航；同一作用域内经验证且相关的记忆稳定优先介入，单纯重复访问不能形成错误自增强。该结论不代表正式用户数据已经迁移。

### 阶段 6：管理 UI 与真实迁移

状态：**进行中；同源读取控制面与只读预检已完成，正式迁移执行和写管理尚未开放**。
...
- 待完成：建立覆盖 Agent run、外部渠道、Repository、SQLite 和 Embedding pipeline 的统一停写协议，或采用“登记请求 -> 重启 -> Runner 初始化前迁移”的启动前执行协议；迁移期间必须阻止新写入并等待或中断已有写入者；
- 待完成：在显式用户确认后提供持久化迁移进度、失败恢复、受约束回滚和正式 backend 切换；失败时继续使用 v2；
- 待完成：用户移动、合并、失效、恢复和导出 atom 的写管理能力，以及隔离长时间运行和正式用户场景验收。

验收：读取控制面与预检部分已通过；完整阶段验收仍要求 UI 管理运行时同一份 atom/catalog、不建立展示副本，正式迁移可停写、可恢复、可回滚且只在用户明确批准后执行。
