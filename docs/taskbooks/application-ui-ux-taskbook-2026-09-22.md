...
| 19 | UX-19 | 本任务实施记录 + `pnpm run verify:electron-ui-state-continuity` / `verify:chat-streaming-rendering` / `verify:chat-reading-scenarios` | 顶部/中部/底部阅读 + 流式增量 + 输入增高/展开工具详情/切换会话返回定位 | |
