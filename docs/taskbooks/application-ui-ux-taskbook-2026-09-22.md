...
- 未覆盖项：**变色场景仍未被复现或录屏**（见上），所以第 2 条保持未勾选；`parseStream` 仍以"流结束"为完成条件、异常截断不被识别；复制文本（剪贴板）与普通/紧凑模式、重开会话的一致性未测。
...
| 19 | UX-19 | 本任务实施记录 + `pnpm run verify:electron-ui-state-continuity` / `verify:chat-streaming-rendering` | 顶部/中部/底部阅读 + 流式增量 + 历史加载/输入增高/分栏变化/会话返回定位 | |
