...
> **2026-09-24 归属更新**：本节四项的待办已不再悬空——**大规模文件树虚拟化**与**审阅侧栏独立宽度**折入[应用层 UI / UX 优化与统一任务书](../taskbooks/application-ui-ux-taskbook-2026-09-22.md) 的 UX-17 / UX-18（含基准先行与验收条件）；**行级评论持久性**与**真正的 LS VS Code 扩展**属产品面或数据模型决定，记录在同一任务书第 7 节，当前不排期。本文此后只负责上游证据（commit、许可证、体积、源码路径）与"已直接吸收哪些改动"的记录，不再作为待办清单的持有者。
