...
| `packages/types/src/run-context-contract.ts` | 302 | 四组高频 RunContext 字段的 owner、读写阶段、生命周期和写入查询 | 保持 machine-readable manifest；继续由 `replan-state.ts`、`reply-state.ts` 等领域边界消费，不把具体状态写入逻辑吸回 types | E |
