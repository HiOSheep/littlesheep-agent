...
- **执行**：退役；P1–P7 的机制事实已在项目状态与 `AGENTS.md` 中，工作策略升级通道与等待头兼容路径两条事实新增到 `AGENTS.md`（本地未跟踪的维护说明，不进入版本库，因此不能写成可点击链接）与 [Core Flow 状态契约](core-flow-state-contract.md)；派生缺口转入新建的[记忆写入专项（现合并为 Runtime 状态一致性与必要记忆任务书）](../taskbooks/runtime-state-consistency-taskbook-2026-09-22.md)。
