...
仍未完成：**改选目录**（系统目录选择对话框无法在无头环境驱动，需人工或改走可注入的选择器）。目录、普通文件预览、浏览器标签在未就绪期间的可用性、以及切换会话/关闭恢复窗口都已实测（见下两节）。

### CS-08 补充：未就绪期间的浏览器标签（实测）

同一脚本的 `browser-tab-while-not-ready` 一步把浏览器标签也验到"真的渲染出页面"这一层，而不是只断言标签存在：脚本自己在 127.0.0.1 上起一个只返回 `<h1>MARKER-BROWSER-GUEST-PAGE</h1>` 的页面，在未就绪窗口里通过 `.workspace-add-trigger` 打开浏览器标签、提交地址，再用 webview 的 `getURL()`/`executeJavaScript()` 把客体的 URL 与正文读回来。

| 观测 | 结果 |
| --- | --- |
| 打开浏览器标签时的就绪状态 | `starting`（要点：执行能力还没发布） |
| 提交地址后客体状态 | `url: http://127.0.0.1:2791/`，正文 = `MARKER-BROWSER-GUEST-PAGE` |
...
边界：合成数据根只证明机制，不证明真实磁盘与真实历史规模（任务书里 5.67 s 那批观察值来自真实数据根）；缓存用例的夹具会话**没有真实消息**，因此它证明的是请求次数与往返耗时，不是"有真实历史负载时能省多少毫秒"；本脚本拿不到后台扫描的完成信号（router 不暴露），"扫描结束后继续交互"只由就绪后的响应性采样间接支持；打包版未覆盖。`--partitions` 可调规模，`--keep` 保留数据根以便复查。
