...
**⑤ bootstrap 文件注册的下一处线索（尚未优化，先记录证据）**。跨 profile 测量（empty / normal / large 各 3 次）显示该阶段 normal 27.9–41.3 ms、large 38.5–51.4 ms，**不随数据根规模增长**，因此不是"扫描整张资源注册表"的成本。代码阅读给出的解释是：测量脚本每次都用**新建的数据根**，所以每次启动都是首次注册，六条资源登记与账本事务必须真正落盘；而 `sameResourceRegistration` 忽略 `registeredAt`/`updatedAt`、`commitMutation` 在无增删改时提前返回，因此**同一个数据根的第二次启动应当跳过这些写入**。该推论尚未实测——需要"同一数据根连续启动两次"的对照，而当前脚本每次运行都新建数据根。`empty` profile 不输出 `runner-infra-*` 标点，那一行不能用于跨 profile 比较。因此本轮不动这项，也不据此声称任何收益。
