...
| 应用构建 | `packages/app/out`，本账本记录时的输入摘要 `bd2bc2d5b35b5d7e8730d3f029c6220b16e573b2802fb5f0630f28dd3c060185`（以账本字段为准，不要引用别处的摘要） |
...
以下四项原先只能人工核验，按最新记录（用户 2026-09-24 人工确认无问题，见[文档入口](../../README.md) 的退役记录）**已确认**：启动页到渲染器的**交接瞬间**与窗口出现时刻（文档像素已由上一段覆盖；早先一次尝试性抓取拿到的是窗口尚未绘制文档时的表面色 `#121212`，因此脚本保留了"底色必须等于声明值"的防线，不把抓到的任意一帧当证据）、失焦与最小化（最小化后窗口不参与截图，失焦只改变原生按钮激活态）、125% / 150% / 200% 显示缩放与明暗桌面背景，以及**打包版**（已由本目录的打包版账本覆盖，见 CS-01/CS-02 与 CS-10 各节）。脚本不因此冒充人眼结论：像素证据只证明"长什么样"，不证明约 90 ms 的交接观感；DPR 代理只改设备像素比，不改窗口 CSS 尺寸、原生按钮与字体度量。
...
尚未核验：需要活跃任务才可达的隐藏/恢复周期（不在 2026-09-24 的人工确认范围内）。
...
自动化部分到此为止：所有能由脚本在真实 Electron 里取证的条目都已落地并留下账本（开发版视觉、打包版视觉、交互、恢复归属、失败态与重试，加上右侧工作区三指标与大历史数据根的开发版/打包版）；原任务书 59 条验收项已全部勾选并于 2026-09-24 退役。以下四项**只能**由真人确认，已按最新记录于 **2026-09-24 由用户人工确认无问题**，不再计作未完成：

- **系统重启后的完全冷启动**：本机所有样本都是热缓存重复，重启后的首次启动（含首次读二进制）无法自动化。
- **125% / 150% / 200% 显示缩放与明暗桌面背景**：CS-09 的脚本用 `Emulation.setDeviceMetricsOverride` 做了 DPR 1.25 / 1.5 / 2 的**代理**测量（这三档 DPR 下渲染器不溢出、阶段文字不与发送按钮重叠、不出现整窗条带），但代理只改设备像素比，不改窗口的 CSS 像素尺寸、原生标题栏/caption 按钮与字体度量，也不改 acrylic 合成——人工确认的是真实缩放下的观感，代理结论仍只覆盖 DPR 这一维。
- **失焦、最小化状态的观感与启动页→渲染器的交接瞬间**：最小化后窗口不参与截图（脚本只断言 DOM 状态），交接瞬间约 90 ms，需要人眼或高速拍摄。
- **安装包（NSIS）实机安装与干净机器首次运行**，以及**真实续接执行**（现有证据到"发现 + 归属"为止；点"继续"会在检查点记录的真实工作区执行工具）。

仍然只能由脚本证据说话、不在人工确认范围内的两项：
...
**改选目录**由用户于 2026-09-24 人工确认无问题（系统目录选择对话框无法在无头环境驱动，脚本侧仍只覆盖其余路径）。目录、普通文件预览、浏览器标签在未就绪期间的可用性、以及切换会话/关闭恢复窗口都已实测（见下两节）。

### CS-08 补充：未就绪期间的浏览器标签（实测）

`scripts/verify-desktop-cold-start-interaction.mjs` 的 `browser-tab-while-not-ready` 一步把浏览器标签也验到"真的渲染出页面"这一层，而不是只断言标签存在（注意：这一步属于交互脚本，不是上面 CS-08 工作区可用性用的 `measure-workspace-availability.mjs`）：脚本自己在 127.0.0.1 上起一个只返回 `<h1>MARKER-BROWSER-GUEST-PAGE</h1>` 的页面，在未就绪窗口里通过 `.workspace-add-trigger` 打开浏览器标签、提交地址，再用 webview 的 `getURL()`/`executeJavaScript()` 把客体的 URL 与正文读回来。

| 观测 | 结果 |
| --- | --- |
| 打开浏览器标签时的就绪状态 | `starting`（要点：执行能力还没发布） |
| 提交地址后客体状态 | `url: http://127.0.0.1:<本轮随机端口>/`，正文 = `MARKER-BROWSER-GUEST-PAGE`（端口每次运行都不同，以账本记录为准，不要把某个具体端口写进文档） |
| 读回正文时的就绪状态 | 仍是 `starting`——浏览器标签不等待 Runner |
| 就绪交接后 | URL 与正文一字不变（交接不重载页面） |
...
边界与未覆盖：正常启动的未就绪窗口只有约 300 ms，调试器附加往往已经落在 `ready` 之后，所以"正常启动不长出全局条"由**同一状态的拉长版本**测量，而不是靠抢帧；显示缩放只做了 DPR 代理，而代理**只在设备像素比这一维**上近似真机——窗口的 CSS 像素尺寸、原生标题栏/caption 按钮与字体度量都不会跟着变，所以它证明的是"在这三档 DPR 下渲染无溢出、无重叠、无条带"，不证明真实 125%/150%/200% 缩放下的观感——那一项由用户 2026-09-24 人工确认无误（见"未覆盖与待验收"），DPR 代理的覆盖边界不变。另外两处与本项无关、但在 800 px 窗口下顺带记录的现象：控制行本身已很拥挤（就绪后同样存在），右侧预览列在面板与文件树夹挤下会变成一行一个字——都属既有布局，未在本次改动范围内处理。
...
- 机制（实现细节见 `packages/app/src/main/local-app-api/README.md` 的"恢复期间的路由隔离"与"旧事件异步补扫"两条）：请求入口不再等待 RunRouter 恢复，元数据/Runtime/会话/工作区路由在恢复期立即应答，Run/Checkpoint/审批在路由未就绪时明确 503；现代租约与收件箱中可恢复的任务仍在执行就绪前处理，历史事件分区转后台兼容扫描；`DurableEventStore` 启动不再全量列目录并逐个读取历史分区，校验下移到读写与扫描路径。
...
观察值（**来自真实数据根，仓库内没有对应账本**；原记录写在已退役的冷启动任务书里，原文见 `git log --follow -- docs/taskbooks/desktop-cold-start-taskbook-2026-09-23.md`，因此不是分位数）：真实数据根约 399 个事件分区 / 13,769 个事件文件；修复前两次执行就绪约 **55.7 s / 84.5 s**；修复后一次重复启动：Renderer 首帧约 1.30 s、工作区目录条目约 1.17 s、执行就绪约 **5.67 s**；两段已有会话首屏历史 29 / 34 ms；一次新会话真实模型发送 HTTP 200 `ok`、约 4.68 s。
...
| --- | --- | --- |
| API 开始监听 | 615 ms | 644 ms |
| **执行就绪** | **1191 ms** | **1233 ms** |
| 恢复期最慢一次轻量请求 | `/sessions` 17 ms，200 | `/workspace/list` 15 ms，200 |
| 就绪后所选会话首屏历史（3 次 × 3 会话） | 14–18 ms，全部 200 | 13–17 ms，全部 200 |
| 缓存：首次进入某会话 | **1 次请求**，61 ms | **1 次请求**，55 ms |
| 缓存：切走再切回同一会话 | **0 次请求**，11 ms | **0 次请求**，11 ms |
| 缓存：快速切换（选中 → 切走 → 切回） | **1 次请求**（不是每次切换一个） | **1 次请求** |
| 被取消的历史请求 | 0 | 0 |
| 恢复失败 / 发现失败 | 0 / 0 | 0 / 0 |
| **打包版**（`release/win-unpacked`，同一脚本 `--app=packaged`，400 分区） | API 监听 911 ms、**执行就绪 1497 ms**、恢复期最慢 `/workspace/list` 19 ms（200）、首屏历史 1–17 ms（全部 200）、缓存首次 1 次请求（65 ms）/ 重访 **0 次**（12 ms）/ 快速切换 1 次、0 取消、0 恢复失败 |

（同一提交的两次运行；执行就绪与首帧/目录条目在同量级内波动，例如首帧与目录条目的更早一次运行是 1191.7 / 1152.7 ms 与 1172.8 / 1134.6 ms。缓存行来自后加入的 `session-history-cache` 一步：脚本打开 CDP 的 `Network` 域，数**渲染器自己发出的** `/sessions/<id>/messages` 请求，而不是从耗时反推。）

**结论**：历史分区数三倍（400 → 1,200）而执行就绪基本不变（1191 → 1233 ms，前一次 1277 → 1269 ms），即这条路径上的就绪**不再随历史规模增长**；恢复期间元数据、Runtime 与会话/工作区路由都在毫秒级应答。会话缓存的作用也能直接数出来：首次进入一个会话恰好一次请求，切走再切回**零请求**（耗时 61 → 11 ms），选中后立刻切走再切回仍然只有一次请求，且没有任何历史请求被取消——这正是"读取属于会话本身、切走不取消、切回复用"的契约。
