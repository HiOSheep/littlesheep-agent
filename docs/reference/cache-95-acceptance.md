...
- 因此总体命中率的结构是：`暖请求(96.15%)` 与 `首个前缀建立(≈5%)` 的加权；1 次冷启动摊到 40 个请求即得 94.0%。要跨过 95%，只能继续缩短那份**共享前缀**（system prompt + 工具 schema），或让前缀在多会话间更稳定。
...
**当前状态标记：能力裁剪已完成（P0–P4）；95% 未达成（最新实测 94.045%，暖请求 96.15%，冷启动按 Provider 缓存窗口计价）。**
