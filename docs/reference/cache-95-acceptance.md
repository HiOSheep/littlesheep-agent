...
- 暖请求未缓存量在时间上呈 **134–341 的锯齿**：动态后缀大（41 项、8,211 字节）时未缓存反而少（249），后缀小（3–7 项、约 2,800–3,400 字节）时未缓存更多（291–341）。即每个任务边界处的首个请求会重新发送 Provider 尚未连续缓存的内容；锯齿来源是**任务边界重建**，而不是前缀。
- 下一步方向据此确定：削减每个任务边界处重新发送的内容（历史窗口/任务切换时的上下文装配），而不是继续压缩共享前缀。
...
**当前状态标记：能力裁剪已完成（P0–P4）；95% 未达成（最新实测 93.538%，暖请求 95.579%，冷启动 5.9%；达标需暖请求达到 97.159%）。**
