...
1. **真实长任务红线（已完成，转入回归）**：目标、初始状态与整任务账本已按[缓存 95% 验收规程](../reference/cache-95-acceptance.md)与[真实长任务缓存基线](../reference/cache-baseline/real-long-task-baseline-2026-09-22.md)冻结（原任务书已于 2026-09-24 退役），跨 run 续接、新增输入、压缩触发与恢复成本都已优化并实测（长任务 99.13% / 99.21%，重启 98.99%，空闲 45 分钟 99.05%）；后续只需保持 `pnpm run check:cache-acceptance` 与冻结清单回归，并在 3 回合形状上不再期待红线。每项真实长任务在预先冻结的业务节点按会话累计值验收，初始冷启动低值不误报，全部用途成本仍完整核验，不用填充、预热或排除调用换比例；其他 Provider 在实际支持范围内另行校准。
