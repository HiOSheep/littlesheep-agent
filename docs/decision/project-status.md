...
**当前阶段：Memory v3 阶段 0-4 已完成隔离实现、双后端适配、安全迁移和完整工程验证。下一步进入阶段 5，统一层级、FTS、本地向量、证据封套与 KnownState 检索路径；当前 v2 仍是正式运行路径，不迁移正式用户数据。**
...
| Memory Service 与 Memory v3 | v2 基础闭环；v3 阶段 0-4 隔离完成 | v2 仍是正式默认且旧向量路径仍通过 Provider。v3 已完成双后端 facade、认识状态分类、稳定实体引用、Repository transaction、Memory Service/Runner、EVOLVE/CAPTURE、索引导航、Catalog 重建及安全迁移器。迁移器保留 v2 权威源，以 snapshot、版本 locator、同卷 staging、全量校验、故障恢复和受约束回滚完成隔离验收；未接入自动启动，也未迁移正式数据。统一检索/KnownState 和 atom 管理 UI 属于阶段 5-6 | `packages/memory-tree/src/memory-repository/`、`packages/memory-tree/src/v3/`、`packages/runner/src/memory-v3.integration.test.ts`、`docs/taskbooks/memory-atom-vector-catalog-taskbook-2026-07-15.md` |
...
| 开发快速门 | 通过：本轮影响 23 个变更文件并传播到 11 个相关 workspace 包；30 个相关测试文件，235 passed、1 skipped；总耗时约 22 秒 | `pnpm.cmd run verify:changed` |
| 核心 Agent 门 | 通过：仓库检查、全工作区类型检查和 7 个测试文件中的 69 项核心契约；总耗时约 7 秒 | `pnpm.cmd run verify:core` |
| 全量测试 | 通过：143 个测试文件；1119 passed、1 skipped | `pnpm.cmd test` |
...
1. 阶段 5：统一 v3 层级/FTS/本地向量检索、访问反馈、`MemoryEvidenceEnvelope`、D0-D3 和版本化 `KnownState`，并退役旧 Provider Embedding 旁路。
2. 阶段 6：让管理 UI 直接操作同一份 v3 atom/catalog，再在用户明确批准后迁移正式数据。
...
1. 进入 Memory v3 阶段 5，统一 v3 层级/FTS/本地向量检索、证据封套、D0-D3、访问反馈和 `KnownState`；阶段 6 再做 atom 管理 UI 与正式迁移审批。
2. 完成 OpenAI、DeepSeek、GLM 真实对话冒烟，用 Provider 结果校准 Context、reasoning、usage 与保守安全估算；远程 Embedding 不纳入默认路径。
3. 在阶段 5 数据链稳定后，把正式迁移审批、进度和回滚入口接入阶段 6 管理 UI；不自动迁移现有用户数据。
