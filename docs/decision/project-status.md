...
**修复方案的收益上限被算出：只修辅助路径达不到 95%（2026-09-21 16:45:00）**

上一轮我建议"授权修辅助路径前缀根因"即可改善三个不达标场景。本轮先把**收益上限**算出来，结论**推翻了这个建议**。

- **按形态分组的实测**（工具工作场景 18 请求）：主循环（12 工具）12 次 **94.42%**；冷启动 2 次 51.35%；零工具（强制收尾）2 次 75.76%；零工具（`ask_user`）2 次 41.09%；**稳态合计 91.996%**。
- **关键结论**：4 个零工具请求合计 5,470 输入 / 3,712 缓存（67.86%）。**即使它们完全达到主循环的 94.42%，稳态也只能到 94.418%——仍低于 95%。**
- 也就是说 **主循环自身的 94.42% 才是约束**：在工具工作场景里它本来就没到红线，辅助路径只是把 91.996% 再往下拖。修辅助路径仍然正确（消除前缀不一致），但**不足以达标**。
- **新发现（在本实现控制之内的前缀抖动源）**：主循环 14 个请求的工具集**同为 12 个工具**，但 `toolSchema` 指纹有**两个取值**。`normalizeTools` 会先按名排序再哈希，故顺序差异无法解释——差异只能来自某工具的**描述/参数内容**。定位到 `packages/skills/src/use_skill.ts` 把描述写成 **getter**，每次从活的 skill 索引重算 `Available skills: …`：索引一变，描述就变，**工具 schema 随之改变，可缓存前缀失效**。
- **候改方向（未实施，需授权）**：让工具描述保持常量，把可用技能列表交给本就带版本号、且已在前缀中的 system prompt `skills-index` 段表达——既消除抖动也不损失能力。
- **当前判断**：要让工具工作等场景跨过 95%，需同时处理 ① 辅助路径前缀不一致、② 主循环内工具 schema 抖动，可能还需 ③ 短会话 prompt 偏小（该场景主循环平均 prompt 仅约 4,541，而 `hit ≈ 1 − 192/prompt`）。三者都不属纯测量。
- 验证：仓库卫生 **33 passed / 0 failed**；提交 `ed66fed` 已推送。
