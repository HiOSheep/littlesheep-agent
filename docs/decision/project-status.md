...
**当前阶段**：Memory v3 阶段 0-26、统一 Tool Execution Service、工具调用级与 TaskBook 步骤级有界并行、shadow Git 检查点、退出冻结、前台实时 Provider API 表达来源/去重闸门、回答级记忆连续性判断、会话摘要精确字段保真、运行时事件安全边界与 Renderer 生产入口、TaskBookPatch、Runner 检查点续跑、应用启动恢复控制面、活动任务快照与控制、托盘、三档关闭策略、逻辑容器权限闸门和 LS 开发环境版本管理均已有工程基线。语义活动 `respond / execute / clarify`、直接回应紧凑 Prompt、有界历史、选择性上一轮摘要、显式工具提议、结构验证、无进展熔断和工具循环后续请求紧凑化已通过完整质量门及真实 Electron Runner 验收。用户明确点名且可证明为新鲜、自包含的单个 builtin `glob / grep / read` 请求继续使用 `decide_explicit_tool`；用户没有点名工具、但目标可证明为新鲜、自包含、无附件、无续接、无实际记忆介入且只需当前工作区只读检查时，DECIDE 由 LLM 在同一只读工具集中选择一个最小工具并提交有界参数提议，Runtime 重验名称、schema、路径、权限和副作用后直接执行。两条路径均为 `DECIDE -> Runtime tool -> execute_final_reply` 两次模型请求，并保留工具证据、SSE、结构 VERIFY 与调用审计；多工具、写入、执行、附件、续接、实际记忆介入、恢复态和边界不明任务仍进入完整路径。Flash 工具协议 `15/15` 校准已完成；下一主线是推进 Pro 工具协议、非字段事实、真实外部副作用和长期真实用户负载验收，并仅在安全契约不退化时继续缩短 Prompt。**
...
- 阶段 20 已退役 Memory v2 的 archive 摘要写入、旧 vector 装饰写入和 CLI archive adapter；既有 archive/vector 数据保持原样，只读兼容路径不能创建摘要、向量或 Atom。阶段 20 当时的验收基线为 180 个文件、1317 passed、1 skipped。2026-08-04 当前工作树为 263 个测试文件、1857 passed、1 skipped；仓库卫生 33/33、全量测试、27 包 typecheck、Electron build、恢复源检查、Flash 工具协议 `15/15`、真实 DeepSeek 显式/自主单只读工具、确定性七场景、后台九场景、跨重启回答连续性、多轮五字段摘要续答、三级压缩深度、主动断线恢复、120 秒诊断门和正式 2 小时持续任务验收均已通过。
...
- 2026-08-04 05:16:39 的最新自主选读工具验收不要求用户点名工具：DECIDE 在 builtin `glob / grep / read` 中选择一个工具，同时给出满足有界 JSON Schema 的参数、步骤摘要和验收标准；Runtime 再校验工具来源、schema、路径、权限与只读副作用并直接执行。真实路径为 `decide -> Runtime glob -> execute_final_reply`，随后通过 structural VERIFY；固定 2 次真实 DeepSeek API、1 次 `glob`，DECIDE/final prompt `744/376`、合计 `1,120`、Provider total `1,185`，相对最初完整路径 `11,050` 下降约 `89.9%`，相对上一版 `2,325` 再下降约 `51.8%`。两次本地 tokenizer 都与 Provider usage `exact_match`，Provider 工具协议请求数为 0；回归上限为 `900/450`，总上限 `1,400`。只有 `excluded` 的记忆候选不会误阻断紧凑路径；`adopted/conflicted`、活动 working set、摘要、附件、历史指代、运行时事件、恢复态或写入/执行语义都会失败关闭并回退完整 Context。该路径仍保留权限、工具 schema、真实工具结果、SSE、结构 VERIFY 和权威调用记录；直接提议无法安全采用时失败关闭或回退既有工具循环，不猜参数。
...
| Context Engine | 主要数据链、调用契约与直接回应/工具续接瘦身已实现；DeepSeek V4 Flash 普通请求和工具协议精确计数已闭环，Pro 工具协议与其他 Provider 待补齐 | 支持确定性候选、来源 segment、契约过滤、阶段软预算、真实模型窗口硬上限、版本化 Summary Memory、附件清单优先、按需附件工具、Provider usage 绑定和双账本 UI。DeepSeek V4 Flash 使用官方固定 revision tokenizer 与 Provider 校准后的最终 framing；普通请求、工具 schema、单工具续轮、仅历史工具消息和多工具乱序结果在 disabled/high/max 三档共 `15/15` 次 `exact_match`。用户点名 builtin 单只读工具时，显式路径为 2 次 API、Prompt `796`；用户只表达只读目标时，自主路径由 LLM 选择一个 `glob/grep/read` 并给出参数，Runtime 直执行后再调用最终回复，固定 2 次 API、1 次工具，最新 Prompt `1,120`、回归总上限 `1,400`。两条路径都保留权限、schema、工具证据和结构 VERIFY。Pro 普通请求保持 exact，Pro 工具协议与 OpenAI/GLM 未校准形态失败关闭，不显示伪精确数字。正式 2 小时门已量化持续任务资源回落；下一步优先补齐 Pro/实际启用 Provider，并按真实用户负载观察 Context 收益 | `packages/context/src/engine.ts`、`context-engine/`、`packages/context/src/tokenizers/`、`packages/types/src/token-ledger.ts`、`packages/harness/src/context-candidates.ts`、`packages/harness/src/compact-autonomous-read-task.ts`、`packages/harness/src/stages/execute/direct-tool-proposal.ts`、`model-observability.ts` |
...
| 全量测试 | 通过：263 个测试文件全部通过；测试项为 1857 passed、1 skipped，共 1858 项。新增覆盖自主只读 DECIDE 的有界 schema 合同、Runtime 直执行、无效紧凑决策的合同错误分类、最终紧凑回答证据闸门、回答连续性 `not_applicable` 边界，以及粗体标签配合代码格式值的真实 DeepSeek 回答；既有漏答、错答、否定旧值、明确失忆、Flash Provider 工具协议 framing、仅 `excluded` 记忆候选不阻断轻量路径、统一 SSE 心跳、慢观察者缓冲上限、观察连接断开后 Main-owned run/Checkpoint 继续执行、连续性任意动态字段、调用审计关联、清洗/截断拒绝和审批边界，以及工具调用统一超时与清理、`exec` 有界输出和进程树关闭、完全访问下显式内置 `exec`、工具完成后的安全暂停边界、轻量 TaskBook 的执行安全契约、输入栏与发送键等半径继续通过 | `pnpm.cmd test`、`packages/context/src/tokenizers/deepseek-v4-{counter,encoding}.test.ts`、`packages/harness/src/compact-{autonomous-read-task,read-only-result}.test.ts`、`packages/harness/src/stages/{decide,execute}.test.ts`、`packages/harness/src/response-continuity{,-dynamic-fields}.test.ts`、`packages/app/src/main/local-app-api/http.test.ts`、`packages/app/src/renderer/ui-radius-consistency.test.ts`、其余 workspace 测试 |
...
| 真实 DeepSeek 显式与自主单只读工具 | 通过：用户点名工具时走 `decide_explicit_tool`，只表达目标时由 DECIDE 在 builtin `glob / grep / read` 中自主选择；有界多工具/写入仍走完整 `decide` | 显式路径只让模型返回 `input` 或澄清对象，Runtime 锁定工具名并展开任务书；最新 `glob` 为 2 次 API、1 次工具、耗时 `2.558s`，DECIDE/final prompt `417/379`、全程 `796`、Provider total `826`。自主路径让 DECIDE 同时返回工具、参数、简短步骤与验收标准，Runtime 重验后直执行；固定 2 次 API、1 次 `glob`，耗时 `2.948s`，DECIDE/final prompt `744/376`、合计 `1,120`、Provider total `1,196`，两次均为 `exact_match`，回归总上限 `1,400`。两条路径都要求 builtin 来源、无审批、无写入/外部副作用、未清洗/截断，并由 Runtime 重验 schema、路径、权限、工具证据、完整 SSE、结构 VERIFY 和调用审计；附件、历史指代、实际记忆介入、恢复、插件同名工具、写入/执行或边界不明任务回退完整流程。耗时和 completion 随真实 Provider 波动，只把 Prompt 与调用数作为效率回归基线 | `pnpm.cmd run verify:electron-deepseek-single-tool`、`pnpm.cmd run verify:electron-deepseek-autonomous-read`、`packages/harness/src/compact-{explicit-tool-decision,autonomous-read-task}.ts`、`compact-read-only-result.ts`、`stages/execute/direct-tool-proposal.ts` |
...
| 真实 DeepSeek 后台两步任务 | 通过：9 个真实场景全部完成 | 隔离数据根和真实 Electron/DeepSeek 覆盖活动任务 SSE、关闭到托盘继续运行、运行时 profile 热重载、暂停检查点、强制终止、重启续跑、两步 `write -> read` 副作用、回答级记忆追问、中断检查点和彻底退出。TaskBook/执行均为 2 步，`write` 与 `read` 各成功 1 次且无需批准；恢复任务严格只有 `decide -> execute_final_reply` 两次模型调用、最新耗时 `7.943s`。本轮有权威 usage 的请求合计 Provider prompt `6,054`、completion `1,044`、total `7,098`，逐请求本地 prompt 全部 `exact_match`；中断分支的三次未完成请求不伪造 Provider usage。首次复跑暴露 `**字段：** \`值\`` 的外层 Markdown 清洗会留下前导标记，造成“回答正确但判失忆”；修正后以同一真实形态复跑，文件名与验收代号均命中近期历史，连续性为 `supported`、`matchedSources=recent_history`，漏答、错答、否定或明确失忆仍失败关闭；隔离数据已清理 | `pnpm.cmd run verify:electron-deepseek-background-task`、`scripts/verify-electron-deepseek-background-task.mjs`、`packages/harness/src/response-continuity-{text,targets,evidence}.ts` |
...
1. 保持当前 DeepSeek chat、continuity、tool、abort、跨重启最终回答连续性、Flash 普通请求与工具协议 `15/15 exact_match`、显式单工具 Prompt `796` 和自主单工具 Prompt `1,120` 为回归门；下一步为 Pro 工具协议建立独立校准，并只在权限、schema、工具证据、结构 VERIFY 和实时最终回复均保留时继续缩短自主路径。只有 OpenAI/GLM 实际配置并进入用户选择范围后，才为其执行同等真实校准和模型专用 tokenizer 验证。远程 Embedding 不纳入默认路径。
