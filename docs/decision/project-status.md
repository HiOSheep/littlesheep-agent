...
- 阶段 20 已退役 Memory v2 的 archive 摘要写入、旧 vector 装饰写入和 CLI archive adapter；既有 archive/vector 数据保持原样，只读兼容路径不能创建摘要、向量或 Atom。阶段 20 当时的验收基线为 180 个文件、1317 passed、1 skipped。2026-08-03 当前工作树为 258 个测试文件、1799 passed、1 skipped；仓库卫生 33/33、全量测试、27 包 typecheck、Electron build、恢复源检查、真实 DeepSeek 紧凑单只读工具、有界两步工具提议、后台任务、短时并行压力、跨重启回答连续性、一次有界摘要压缩回答和 120 秒分钟级持续任务验收均已通过。
...
- 2026-07-31 的真实前端 Runner 验收中，显式单步 `glob` 仍使用 3 次模型请求、总耗时 `13.666s`、Provider usage 累计 `16,316` tokens；2026-08-03 03:45:44 的第一版两调用基线为 `5.642s`。当前实现进一步把可证明自包含的内置单 `glob / grep / read` 决策拆为 `decide_explicit_tool`：只投影当前输入、工作目录、profile/reasoning 声明、紧凑语气约束、唯一工具 schema 和 Runtime awareness，不加载历史、附件、记忆、完整 Workflow 或 bootstrap。最新真实 `glob` 验收为 `decide_explicit_tool → Runtime glob → structural VERIFY → execute_final_reply`，2 次 DeepSeek API、1 次工具，总耗时 `2.901s`；紧凑决策 prompt `629` tokens，全程 prompt `1,479`、Provider total `1,702`，两次本地 tokenizer 均为 `exact_match`。Runtime 仍复核工具来源、名称、schema、路径边界、权限和副作用；同名插件/run-scoped 工具、多工具、写入、执行、附件、历史指代、记忆介入、恢复态或参数不完整时自动回退完整 `decide`。
...
| 全量测试 | 通过：258 个测试文件全部通过；测试项为 1799 passed、1 skipped，共 1800 项。新增覆盖工具调用统一超时与清理、`exec` 有界输出和进程树关闭、完全访问下显式内置 `exec`、工具完成后的安全暂停边界、轻量 TaskBook 的执行安全契约，以及按 LS 最终回答识别 `executionCount / ticks / completed`、短数字、布尔值、Markdown 表格和否定语义；既有回答连续性、摘要保真、恢复、权限、记忆和桌面回归继续通过 | `pnpm.cmd test`、`packages/tools/src/builtin/exec.test.ts`、`packages/tools/src/tool-execution-service.test.ts`、`packages/harness/src/stages/{decide,execute}.test.ts`、`packages/harness/src/response-continuity.test.ts`、其余 workspace 测试 |
...
| 真实 DeepSeek 显式工具提议 | 通过：自包含单只读工具走 `decide_explicit_tool`，有界多工具/写入仍走完整 `decide`；两类路径都只在证明可直接执行时进入 `execute_final_reply` | 单只读路径仅接受来源为 `builtin` 的 `glob / grep / read`，最新 `glob` 为 2 次 API、1 次工具、`2.901s`，紧凑决策 prompt `629`、全程 prompt `1,479`、Provider total `1,702`，两次本地 tokenizer 均为 `exact_match`。多工具仍最多暴露 4 种显式工具 schema、TaskBook 最多采用 8 个提议。所有路径均由 Runtime 重验名称、schema、资源、权限和副作用；附件、历史指代、记忆介入、恢复、插件同名工具、写入/执行或边界不明任务回退完整流程 | `pnpm.cmd run verify:electron-deepseek-single-tool`、`pnpm.cmd run verify:electron-deepseek-background-task`、`packages/harness/src/compact-explicit-tool-decision.ts`、`explicit-tool-instruction.ts`、`stages/execute/direct-tool-proposal.ts` |
...
| 真实 Electron + DeepSeek 分钟级持续任务 | 通过：默认 120 秒、每秒一个进度 tick 的单次内置 `exec` 只执行 1 次，生成 `120` 个 ticks 和完成证明；执行前只有一次 `decide`，用户在工具运行中发出的暂停请求在工具结束、步骤状态持久化后才生效，没有提前请求最终回答。恢复前先切回 Checkpoint 原模型，强制终止并重启后续跑没有重放副作用，产物哈希、大小与 mtime 保持不变；随后切换模型进行记忆追问，最终回答准确给出验收代号和 `executionCount=1`，连续性为 `supported`。采样期间无资源违规，RSS 约从 316 MiB 回落到 251 MiB，活动句柄从 12 回落到 4、最终空闲为 3，Runner、监听器和活动源回到基线，隔离数据已删除。该门证明分钟级持续工作与有界回收，不替代数小时、断网或真实外部系统副作用验收 | `pnpm.cmd run verify:electron-deepseek-sustained-load`、`scripts/verify-electron-deepseek-sustained-load.mjs` |
