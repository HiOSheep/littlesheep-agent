...
它还不是可直接宣称“生产就绪”的发行版。当前 DeepSeek 活动模型的基础 chat、continuity、tool 和 abort 已真实通过；DeepSeek V4 官方 tokenizer 与普通直接回答请求的最终 framing、本地精确账本和同请求 Provider usage 已完成零差值对账。包含历史 `tool_calls` 或 `tool` 结果的工具续轮仍未完成 Provider 校准，本地精确计数会失败关闭，等待响应后只采用真实 Provider usage，不冒充精确。OpenAI/GLM 尚无经过同等验证的本地精确计数器，未配置时更不能冒充已校准。隔离数据根验收已通过真实 Electron 启动、关闭、托盘恢复、强制终止、重启和 Checkpoint 续跑；2026-08-03 又用真实 DeepSeek 验证了完整退出重启后的最终回答连续性、两步 `write -> read` 任务在后台控制下的完整交付、4 个活动 run 与双 Checkpoint 的短时压力，以及一次原始旧消息被压缩后仅依赖版本化 `session_summary` 的双字段最终回答连续性。它们仍不等于多轮多次压缩、更多事实形态、数小时持续长任务、真实网络抖动、MCP、安装包发布或完整用户场景已经闭环。因此本项目不使用一个没有权重定义的百分比来伪装精确进度，而用能力状态和验收证据表示总进度。产品方向已明确为：解放用户生产力，让用户专注于想法，LS 负责将想法可靠落地；执行和输出统一采用渐进式披露。

**当前阶段：Memory v3 阶段 0-26、统一 Tool Execution Service、工具调用级与 TaskBook 步骤级有界并行、运行时事件安全边界与 Renderer 生产入口、TaskBookPatch、Runner 检查点续跑、应用启动恢复控制面、活动任务快照与控制、托盘、三档关闭策略、设置页“应用与后台”、shadow Git 检查点、退出冻结、前台实时 Provider API 表达来源/去重闸门、回答级记忆连续性判断、会话摘要精确字段保真、逻辑容器权限闸门和 LS 开发环境版本管理已有工程基线。语义活动 `respond / execute / clarify`、直接回应紧凑 Prompt、有界历史、选择性上一轮摘要、显式单工具与有界多工具提议路径、结构验证、无进展熔断和工具循环后续请求紧凑化已通过完整质量门及真实 Electron Runner 验收。当前 DeepSeek 模型四项真实校准、普通直接回答与显式工具提议路径的 V4 本地精确 tokenizer 对账、真实 DeepSeek 跨重启最终回答连续性、一次有界双字段摘要压缩续答，以及隔离数据根中的真实两步副作用任务、Electron Checkpoint、后台控制和短时并行压力验收均已通过；下一主线是含历史 Provider 工具消息的普通续轮 tokenizer 校准、多轮多次压缩与更多事实形态、数小时持续长任务、真实网络抖动与真实用户负载。**
...
- 阶段 20 已退役 Memory v2 的 archive 摘要写入、旧 vector 装饰写入和 CLI archive adapter；既有 archive/vector 数据保持原样，只读兼容路径不能创建摘要、向量或 Atom。阶段 20 当时的验收基线为 180 个文件、1317 passed、1 skipped。2026-08-03 当前工作树为 258 个测试文件、1776 passed、1 skipped；仓库卫生 33/33、全量测试、27 包 typecheck、Electron build、恢复源检查、真实 DeepSeek 显式单工具、有界两步工具提议、后台任务、短时并行压力、跨重启回答连续性和一次有界摘要压缩回答验收均已通过。
...
- FINALIZE 已增加回答级记忆连续性评估：本地评估实际发布且可由 `ReplyProvenance` 回查的 LLM 回答，与真实进入本轮因果模型调用链的 active/adopted Atom、版本化会话摘要和近期跨轮对话之间的独立锚点，并把 `supported / discontinuous / uncertain / not_applicable / unavailable`、Context 可观测性、来源、命中 Atom 和诊断写入 `AgentResult` 与执行日志。当前输入自身的词只证明回答贴合本轮；TaskBook 中由记忆介入后补出的细节仍可形成承接证据。任务续接和“你还记得上次的代号/颜色吗”这类直接记忆追问都会进入显式连续性门；明确追问多个历史字段时，最终回答必须逐项答出对应旧值，只有复述附带限制、漏答任一核心字段、答错、用否定句提到旧值、明确说不记得或要求用户重新提供，均不能算连续。即使所有旧值都在回答中出现，只要回答同时明确声称无法回忆，状态也必须是 `discontinuous`，且不得登记历史、摘要或 Atom 为已承接来源。字段旧值按“近期可观察消息 → 已进入调用链的版本化会话摘要 → 已采用且仍 active 的记忆”逐项解析；会话摘要只有 Runtime 精确保真封套中的字段可承担逐值验收，模型概率摘要正文不能自行升级为精确事实。中文 Markdown 的加粗标签和反引号只视为展示符号，不再让正确回答产生假阴性。纯记忆追问不会因此注入无关的上一轮工具耗时。只有 `supported` 可通过续答连续性门，`discontinuous`、`uncertain` 和 `unavailable` 都不能被 UI、Runtime 或验收脚本描述为“记忆连续”。Context Engine 已裁掉的来源、缺少请求快照的来源、释放/排除/冲突 Atom 均不计命中；执行中由 `memory_tree` 展开的 Atom 只有在对应工具结果确实进入后续模型请求时才可计入。回答实际采用的 Atom 或摘要只形成有界 routing/activation 反馈，不提高事实 confidence 或 verified usefulness，也不增加 Provider 调用。最新定向回归覆盖最终回答、Runtime 摘要隔离和记忆反馈；完整质量门数字见下方验证表。确定性验收 Provider 的七场景 Electron 门仍通过；真实 DeepSeek 隔离验收中，单会话最终回答准确复述 `background-proof.txt` 与 `deepseek-background-anchor-7319`。最新并行压力门又证明两条隔离会话在重启恢复后分别准确复述自己的文件名和验收码，均没有调用工具且判定 `supported`、`matchedSources=[recent_history]`。同日摘要压缩验收进一步证明：原始旧消息不进入重启后的回答请求，最终回答仍逐项给出代号和颜色，状态为 `supported` 且唯一来源为 `session_summary`；仅内部保存、Checkpoint、摘要正文或关键词碰巧命中不能通过。
- 2026-07-31 的真实前端 Runner 验收中，显式单步 `glob` 仍使用 3 次模型请求、总耗时 `13.666s`、Provider usage 累计 `16,316` tokens。2026-08-03 的隔离 Electron 验收已将同类任务收敛为 `DECIDE → Runtime glob → execute_final_reply`：模型在 DECIDE 中按唯一工具 JSON Schema 产生参数提议，Runtime 再校验工具名、schema、只读边界和审批后执行，不使用 Provider 工具协议，也不由 Runtime 猜参数。全程只发生 2 次 DeepSeek 请求、1 次成功 `glob`、结构验证通过，总耗时 `5.642s`；Provider 总量 `4,275` tokens，本地/Provider prompt 分别为 `2934/2934` 与 `870/870`，两次均为 `exact_match`。续接和记忆追问明确排除该快路径，继续优先保证回答连续性。
...
- 同日运行中 Main 对 `deepseek/deepseek-v4-flash` 完成四项脱敏真实校准：chat `872ms`，精确回复 `OK`；continuity `1,140ms`，正确采用最新值且未重复追问；tool `1,680ms`，只调用一次探针工具并正常收尾；abort `389ms`，在收到流式 chunk 后中断并返回 `AbortError`。当前 DeepSeek 凭证因此已被实测为有效；后续两步后台任务和短时并行压力又验证了真实副作用、暂停/中断、强制终止、并行重启续跑、热重载、回答连续性与资源回落。OpenAI/GLM、数小时持续长任务和达到调参门槛的真实用户负载仍未完成。
- 2026-08-01 已接入 DeepSeek V4 官方固定 revision tokenizer 资源和普通消息、thinking、工具定义的最终请求 framing。资源按固定大小与 SHA-256 下载后原子校验，计数使用真实 token id 数量；普通 `/run`、显式单工具、有界多工具提议路径与 2026-08-03 最新真实跨重启回答验收均保持本地 prompt 与 Provider prompt 零差值。单工具两次请求为 `2934/2934`、`870/870`；最新两步任务请求为 `3250/3250`、`1024/1024`，随后记忆追问为 `506/506`、`1244/1244`；跨重启三次请求为 `1003/1003`、`387/387`、`1285/1285`。DeepSeek V4 的 `auto` 现在显式发送 `thinking=disabled`，避免 Provider 缺省 framing 让本地计数来源不确定；含历史 `tool_calls`/`tool` 结果的普通工具续轮仍因尚未完成 Provider 校准而失败关闭 exact，响应完成后使用 Provider usage。OpenAI/GLM 继续明确为 unavailable，旧会话没有历史最终载荷时显示“本会话尚无本地计数”而不伪造数字。
...
- 活动任务后台控制面已接通：Runner 持有独立于 Renderer SSE 的中断信号，并提供有界活动快照、进度订阅及暂停、继续、中断；暂停在安全边界生成 `paused` 检查点，历史与重启恢复明确显示“已暂停”。Main 聚合当前与最多 4 个仍持有活动任务的退役 Runner，空闲后释放；Local App API 提供活动任务列表、`active_runs` SSE 与控制路由。设置页“应用与后台”可配置始终后台、仅活动任务时后台和始终退出三档策略，并以渐进式披露展示真实活动任务、步骤、进度、来源和运行标识；页面卸载会中止流、请求和重连计时器，Main 监听器有硬上限并在断连时释放。托盘最多展示 6 个任务，可显示应用、暂停/继续、中断和彻底退出；只有托盘成功创建时才允许隐藏窗口。2026-07-30 的真实窗口冒烟已验证设置入口、三档策略切换与恢复、同步状态、空任务态、手动刷新和全局前进/返回。2026-08-03 的隔离真实 DeepSeek 验收进一步覆盖活动任务 SSE、关闭到托盘继续运行、暂停检查点、强制终止后恢复、运行时 profile 热重载、两步 `write -> read` 副作用、回答级记忆连续、中断检查点和彻底退出；同日短时并行压力已覆盖 SSE 反复断开重连、4 个活动 run、双 Checkpoint 恢复和监听器/资源回落。真实网络断线、数小时持续负载和更长期资源回落仍待验收。
- 最新短时并行压力门同时启动 3 个旧 Runner 任务，并在模型/Profile 热重载后增加 1 个新 Runner 任务；峰值为 4 个活动 run、1 个退役 Runner 和 2 个 activity source。窗口隐藏期间任务继续，SSE 反复断开重连后 listener 回到基线。两个已完成 `write/read` 副作用的任务分别暂停和中断，强制终止应用后并行恢复；恢复工具调用均为 0，原文件的 SHA-256、大小和 mtime 保持不变，原始用户输入各只持久化 1 次。两条独立会话随后各用一次 LLM 请求完成回答级记忆追问并判定 `supported`。14 次代理请求中 13 次转发、1 次在延迟期取消，所有转发请求均有 Provider usage，本地 tokenizer 全部 `exact_match`；Provider 合计 prompt `23,358`、completion `2,986`、total `26,344`。重启后的最终稳定态相对起点 RSS 增长约 `51.9 MiB`、heap 增长约 `20.7 MiB`，低于 `256/128 MiB` 预算；活动 run、退役 Runner 均回到 0，source/listener 回到 1。该结果证明短时并行、恢复和资源释放，不替代数小时持续负载或真实断网。
...
| Context Engine | 主要数据链、调用契约与直接回应/工具续接瘦身已实现；DeepSeek V4 普通直接回答和显式工具提议路径精确计数已闭环，普通工具续轮校准与其他 Provider 待补齐 | 支持确定性候选、来源 segment、契约过滤、阶段软预算、真实模型窗口硬上限、版本化 Summary Memory、附件清单优先、按需附件工具、Provider usage 绑定和双账本 UI。DeepSeek V4 普通回答与不使用 Provider 工具协议的显式单/多工具提议路径使用官方固定 revision tokenizer 和最终请求 framing，本地账本优先显示、Provider usage 作同请求校准；含历史 `tool_calls`/`tool` 结果的普通续轮在校准完成前不生成 exact 本地账本。OpenAI/GLM 未验证时同样不显示伪精确数字。`respond` 路径只保留身份、能力名、受限记忆索引、`USER.md`、相关摘要/Atom、最近 8 条且最多 6000 字符的历史与紧凑输出约束；`execute` 工具循环首轮保留完整必要上下文，首轮工具结果后保留最近 2 条历史和必需附件 manifest。短时并行压力下 13 次转发请求全部取得 Provider usage 并与本地 tokenizer 精确一致；下一步用数小时持续任务量化长期资源回落，并校准含历史工具消息的普通续轮成本 | `packages/context/src/engine.ts`、`context-engine/`、`packages/context/src/tokenizers/`、`packages/types/src/token-ledger.ts`、`packages/harness/src/context-candidates.ts`、`packages/harness/src/stages/execute/tool-loop.ts`、`model-observability.ts` |
...
| 步骤级执行与恢复 | TaskBook 步骤级并行工程基线、真实两步恢复和双 Checkpoint 并行恢复已通过；长时间持续任务待验收 | 保留已完成步骤证据，失败或恢复时不重复执行已完成部分；步骤显式声明依赖、资源读写集合与副作用，Runtime 只并行无依赖、无冲突且权限明确的分支。默认并发 2、硬上限 4，串行步骤为屏障，分支取消独立，工具实际资源越过步骤封套时拒绝；消息、工具结果和执行证据按 TaskBook 顺序稳定归并。真实 DeepSeek 已验证两个独立 `write -> read` 任务分别暂停/中断后可在进程重启后并行恢复，已完成工具不重做，文件哈希、大小与 mtime 不变；真实网络中断、外部系统并行副作用和数小时性能收益仍待验收 | `packages/harness/src/stages/execute/task-{book,step}-runner.ts`、`task-step-scheduler.ts`、`execute/tool-loop.ts`、`packages/harness/src/checkpoint-resume.ts`、`packages/tools/src/tool-execution-service.ts`、`packages/runner/src/run-checkpoint.ts`、`scripts/verify-electron-deepseek-{background-task,parallel-load}.mjs` |
...
| 全量测试 | 通过：258 个测试文件全部通过；测试项为 1776 passed、1 skipped。覆盖回答级记忆连续性、Runtime 摘要精确字段封套、普通概率摘要拒绝承担精确事实、直接记忆追问、多个历史字段完整/部分回答、版本化摘要与 active/adopted Atom 显式字段恢复、否定旧值、显式失忆表述、复述全部旧值但同时否认记忆、记忆追问确定性路由、Checkpoint 完成证据归一与恢复回复、跨 Repository 同路径写入串行化、延迟代理取消与 usage 解析、显式单/多工具提议、Runtime 二次校验、DeepSeek `auto` 显式关闭 thinking、`respond` ContextSnapshot、统一工具执行、TaskBook 步骤级并行、简单任务防过度 TaskBook、无进展熔断、结构验证快路径、活动任务控制、隐藏桌面验收 API、完全访问一次确认与免逐次审批、会话重命名、对话布局稳定、控件行单行约束、普通表面圆角 token、折叠图标 `3px` 像素对齐 SVG 清晰度例外和字体/图标对齐 | `pnpm.cmd test`、`packages/classifier/src/rules.test.ts`、`packages/harness/src/response-continuity.test.ts`、`packages/harness/src/session-summary-fidelity-text.test.ts`、`packages/runner/src/session-summary-fidelity.test.ts`、`packages/harness/src/continuation-intent.test.ts`、`packages/harness/src/default-harness.test.ts`、`packages/harness/src/stages/{decide,execute,verify,recover,finalize}.test.ts`、`packages/memory-tree/src/memory-repository.test.ts`、`scripts/lib/delayed-http-proxy.test.mjs`、`packages/context/src/tokenizers/deepseek-v4-{counter,encoding}.test.ts`、`packages/runner/src/run-checkpoint-store.test.ts`、`runtime-event-queue.test.ts`、`packages/app/src/main/desktop-acceptance-api.test.ts`、`packages/app/src/main/run-activity-monitor.test.ts`、`packages/app/src/main/session-rename-api.test.ts`、`packages/app/src/renderer/chat-layout-stability.test.ts`、`packages/app/src/renderer/ui-radius-consistency.test.ts`、`packages/app/src/renderer/ui/icons.test.ts`、`packages/app/src/renderer/font-rendering.test.ts` |
...
| Memory v3 Provider 与回答连续性 | 本地结构门、确定性 Electron 七场景门、真实 DeepSeek 跨重启回答门、两步后台任务追问、双会话并行恢复追问和一次有界摘要压缩回答门均通过。FINALIZE 从真实发布回答反查其 Provider 请求与 ContextSnapshot，只把回答实际承接的 active/adopted Atom、版本化摘要、近期跨轮对话和已进入后续请求的记忆工具结果计为连续；当前输入重复、已裁剪来源或 released/excluded/conflicted Atom 不计入。任务续接及直接记忆追问只有 `supported` 可称为连续；多个被追问旧值必须全部出现在最终回答，漏答、答错、否定旧值或明确失忆均判为 `discontinuous`。摘要中的精确字段只读取 Runtime 保真封套，普通模型摘要不能冒充精确记忆。最新真实摘要门使用 4 次 DeepSeek 请求，Provider prompt/completion/total 为 `3301/38/3339`；重启后回答请求不包含原始旧消息，只包含版本化摘要，最终回答逐项给出 `summary-deepseek-anchor-8427 / 雾松青`，判定 `supported`、`matchedSources=[session_summary]`。漏答、答错、失忆声明和摘要未进入因果 Context 四类负例均未误判；稳定态相对重启基线 RSS/heap 增长约 `15.0/9.5 MiB`，低于 `256/128 MiB` 预算。仍未完成的是多轮多次压缩、更多字段与非字段事实、长时间持续负载、工具续轮 token 校准和 EVOLVE/CAPTURE 提案质量验收 | `packages/harness/src/continuation-intent.ts`、`response-continuity-{text,targets,evidence,types}.ts`、`session-summary-fidelity-text.ts`、`response-continuity.ts`、`packages/harness/src/stages/finalize.ts`、`packages/runner/src/session-summary-fidelity.ts`、`session-continuity.ts`、`scripts/verify-electron-{runtime-continuity,deepseek-reply-continuity,deepseek-compaction-continuity,deepseek-background-task,deepseek-parallel-load}.mjs` |
| 真实 DeepSeek 显式工具提议 | 通过：单工具与有界两步工具均只需 `decide -> execute_final_reply` | DECIDE 只收到用户明确点名的工具 schema；最多 4 种工具，单 schema 12,000 字符、总计 24,000 字符，TaskBook 最多采用 8 个提议。Runtime 再做名称、schema、依赖、资源、副作用、权限和审批校验，通过统一 Tool Execution Service 执行；当前直接执行只接受可证明且无需审批的 `read/write`。单工具 `glob` 与两步 `write -> read` 都不使用 Provider 工具协议，最终回答来自真实 DeepSeek，VERIFY 使用结构门，本地/Provider prompt 均为 `exact_match`。续接、记忆追问、否定工具、需审批或边界不明时不走该捷径 | `pnpm.cmd run verify:electron-deepseek-single-tool`、`pnpm.cmd run verify:electron-deepseek-background-task`、`scripts/verify-electron-deepseek-{single-tool,background-task}.mjs`、`packages/harness/src/explicit-tool-instruction.ts`、`packages/harness/src/stages/execute/direct-tool-proposal.ts` |
...
| 真实 DeepSeek 后台两步任务 | 通过：9 个真实场景全部完成 | 隔离数据根和真实 Electron/DeepSeek 覆盖活动任务 SSE、关闭到托盘继续运行、运行时 profile 热重载、暂停检查点、强制终止、重启续跑、两步 `write -> read` 副作用、回答级记忆追问、中断检查点和彻底退出。TaskBook/执行均为 2 步，`write` 与 `read` 各成功 1 次且无需批准；恢复任务严格只有 `decide -> execute_final_reply` 两次模型调用、耗时 `6.651s`。任务与追问合计 Provider prompt `6,024`、completion `900`、total `6,924`，四次请求本地 prompt 全部 `exact_match`。最终回答准确复述文件名和验收代号，连续性为 `supported`、`matchedSources=recent_history`；隔离数据已清理 | `pnpm.cmd run verify:electron-deepseek-background-task`、`scripts/verify-electron-deepseek-background-task.mjs` |
| 真实 Electron + DeepSeek 并行压力 | 通过：10 个真实场景全部完成 | 隔离数据根中覆盖 3 个并行旧 Runner、热重载后的第 4 个 run、活动任务 SSE、关闭到托盘、暂停/中断、强制终止、双 Checkpoint 并行恢复、恢复不重复工具、双会话回答级连续性和彻底退出。两个恢复任务工具调用均为 0，文件哈希/大小/mtime 不变；两条记忆回答分别准确包含 `parallel-a.txt / parallel-deepseek-anchor-a-6417` 与 `parallel-b.txt / parallel-deepseek-anchor-b-5824`，均为 `supported`、来源为 `recent_history`。13 次转发请求全部取得 Provider usage，本地 tokenizer 全部 `exact_match`；Provider prompt `23,358`、completion `2,986`、total `26,344`。显式 `glob` 仍为 2 次模型调用、1 次工具，prompt 为 `2934 + 802`。最终 active run/retired Runner 为 `0/0`，source/listener 为 `1/1`；重启后稳定态 RSS/heap 增长约 `51.9/20.7 MiB`，低于 `256/128 MiB` 预算；隔离数据已清理 | `pnpm.cmd run verify:electron-deepseek-parallel-load`、`scripts/verify-electron-deepseek-parallel-load.mjs`、`scripts/lib/delayed-http-proxy.mjs` |
...
- 文档已明确区分目标架构、当前事实、演进建议和专项任务书；Context Engine 按“阶段 1 主要数据链与 DeepSeek V4 普通请求及显式工具提议路径精确本地计数已实现，普通工具续轮和其他 Provider 仍待收敛”记录，Tool Execution Service 按“本地工程基线完成、生态扩展待验收”记录，Mode Registry 仍不按已完成能力记录。
...
2. 隔离数据根中的真实 Electron + DeepSeek 已验证活动 run 跨重启、暂停/中断 Checkpoint 强制终止后续跑、profile 热重载、两步 TaskBook、真实 `write/read` 副作用账本、4 个活动 run、双 Checkpoint 并行恢复、回答级记忆连续性、Token 精确对账和短时资源回落；下一步验证数小时持续任务、真实网络中断、外部系统副作用和更长期资源回落。
...
6. 后台任务已配套托盘、状态提示、暂停/继续/中断、彻底退出、三档关闭策略和设置页活动列表；列表使用 `active_runs` SSE 实时同步，并保留手动快照刷新作为校准入口。隔离真实 Electron + DeepSeek 已验证关闭到托盘继续运行、活动任务 SSE、暂停/中断、强制终止、并行重启续跑、profile 热重载、SSE 反复断开重连后的 listener 释放和彻底退出；下一步验证托盘不可用时失败关闭、真实网络断线、数小时持续任务和更长期资源回落。
...
版本化 Summary Memory、附件清单优先与按需正文工具、T0 根索引、项目记忆投影、稳定项目 ID、路径重绑定、统一 `MemoryService`、数据根迁移和压缩阈值设置继续沿既有工程基线工作。统一 Tool Execution Service、工具调用级与 TaskBook 步骤级有界并行、显式单/多工具提议路径、运行中事件安全消费、确定性 TaskBookPatch、Renderer 事件生产/反馈入口、Runner 显式续跑、应用启动恢复、活动任务控制、托盘、三档关闭策略和设置页后台控制入口已经实现。2026-08-03 的仓库卫生 33/33、258 个测试文件/1776 passed/1 skipped、27 包 workspace typecheck、Electron build、恢复源检查、真实 DeepSeek 单工具、两步后台副作用、跨重启回答、一次有界摘要压缩回答和短时并行压力验收均已通过；恢复检查仍明确报告部分旧 run 缺执行日志与可选 workspace artifact 索引缺失。这些证据仍不等于多轮多次压缩、数小时持续负载、其他 Provider 矩阵或生产发布已就绪。
...
1. 保持当前 DeepSeek chat、continuity、tool、abort、跨重启最终回答连续性、普通回答与显式单/多工具提议路径的本地精确 token 对账为回归门；下一步校准含历史 `tool_calls`/`tool` 结果的普通续轮 framing。只有 OpenAI/GLM 实际配置并进入用户选择范围后，才为其执行同等真实校准和模型专用 tokenizer 验证。远程 Embedding 不纳入默认路径。
...
4. 保持已完成的 RuntimeEventQueue、安全边界、TaskBookPatch、Renderer 事件生产、TaskBook 步骤级并行、Runner 续跑、应用启动恢复、活动任务 SSE、设置页“应用与后台”、托盘、三档关闭策略、shadow Git 检查点、隔离 Electron 跨重启、真实 DeepSeek 两步任务和短时并行压力质量门；下一阶段验收数小时持续长任务、真实网络中断、外部系统副作用和更长期资源回落，之后再推进开发环境正式分发、Mode Registry、插件 API v2 与 MCP。
