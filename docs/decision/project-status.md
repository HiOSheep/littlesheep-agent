...
- **现行红线对齐 DeepSeek Harness 前端会话累计值，在真实长任务验收节点判定**，95% 为不可低于的红线、95%～99.5% 为目标工作范围；允许能力收缩。初始冷启动可低于红线，首次输入仍计入累计；独立辅助调用进入完整成本账本另列，不与主会话指标混称。完整口径见[验收规程现行条款](../reference/cache-95-acceptance.md#真实长任务现行红线2026-09-22)。上表属于既有冻结负载，其总体与稳态均未达到 95%；不能用它代替尚未完成的真实长任务新基线。上一批能力裁剪/结构修复与真实长任务红线达成继续分别标记。
...
1. **真实长任务红线优先**：按[新任务书](../taskbooks/real-long-task-cache-taskbook-2026-09-22.md)先冻结真实目标和整任务账本，再优化跨 run 消息续接、新增输入、压缩及恢复成本；优先一个已校准模型，允许能力收缩。每项真实长任务在预先冻结的业务节点按会话累计值验收，初始冷启动低值不误报，全部用途成本仍完整核验，不用填充、预热或排除调用换比例；其他 Provider 在实际支持范围内另行校准。
