...
- 阶段 20 已退役 Memory v2 的 archive 摘要写入、旧 vector 装饰写入和 CLI archive adapter；既有 archive/vector 数据保持原样，只读兼容路径不能创建摘要、向量或 Atom。阶段 20 当时的验收基线为 180 个文件、1317 passed、1 skipped。2026-08-03 当前工作树为 258 个测试文件、1786 passed、1 skipped；仓库卫生 33/33、全量测试、27 包 typecheck、Electron build、恢复源检查、真实 DeepSeek 紧凑单只读工具、有界两步工具提议、后台任务、短时并行压力、跨重启回答连续性和一次有界摘要压缩回答验收均已通过。
...
| 全量测试 | 通过：258 个测试文件全部通过；测试项为 1786 passed、1 skipped。新增覆盖 `decide_explicit_tool` 调用契约、紧凑输出向既有 TaskBook 的展开、上下文依赖/附件/写入/记忆/run-scoped 工具回退，以及紧凑澄清回答的因果 Context 可观测性；既有回答级记忆连续性、摘要精确字段封套、工具执行、恢复、权限和桌面布局回归继续通过 | `pnpm.cmd test`、`packages/harness/src/stages/decide.test.ts`、`packages/harness/src/response-continuity.test.ts`、`packages/harness/src/default-harness.test.ts`、其余 workspace 测试 |
...
版本化 Summary Memory、附件清单优先与按需正文工具、T0 根索引、项目记忆投影、稳定项目 ID、路径重绑定、统一 `MemoryService`、数据根迁移和压缩阈值设置继续沿既有工程基线工作。统一 Tool Execution Service、工具调用级与 TaskBook 步骤级有界并行、自包含单只读工具的 `decide_explicit_tool`、完整显式多工具提议路径、运行中事件安全消费、确定性 TaskBookPatch、Renderer 事件生产/反馈入口、Runner 显式续跑、应用启动恢复、活动任务控制、托盘、三档关闭策略和设置页后台控制入口已经实现。2026-08-03 的仓库卫生 33/33、258 个测试文件/1786 passed/1 skipped、27 包 workspace typecheck、Electron build、恢复源检查、真实 DeepSeek 单工具、两步后台副作用、跨重启回答、一次有界摘要压缩回答和短时并行压力验收均已通过；恢复检查仍明确报告部分旧 run 缺执行日志与可选 workspace artifact 索引缺失。这些证据仍不等于多轮多次压缩、数小时持续负载、其他 Provider 矩阵或生产发布已就绪。
