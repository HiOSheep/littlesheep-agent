...
**当前阶段：Memory v3 阶段 0-5 已完成；阶段 6 已接通同源 D0-D3 读取控制面，并完成“确认登记 -> 重启 -> 运行时写入者初始化前迁移/回滚”的正式生命周期、失败恢复和受约束回滚。下一步补齐 v3 atom 写管理与隔离长时间运行；当前正式用户数据仍为 v2，实际切换继续等待用户单独确认。**
...
| Memory Service 与 Memory v3 | v2 基础闭环；v3 阶段 0-5 完成，阶段 6 读取与迁移生命周期完成 | v2 仍是正式默认。Runner 已退役默认 Provider Embedding 旁路；v3 通过本地 Embedding、层级/FTS/向量统一检索、D0-D3 证据封套和版本化 KnownState 工作。管理 UI 从同一 Repository/atom/Catalog 按需展开 D0-D3，并可确认登记迁移或回滚；结构性操作只在下一次启动、所有运行时写入者创建前执行，失败保持原 backend，配置跟随 locator。正式用户数据未迁移 | `packages/memory-tree/src/memory-repository/`、`packages/memory-tree/src/v3/`、`packages/app/src/main/memory-v3-bootstrap.ts`、`packages/app/src/main/memory-tree-control.ts`、`packages/app/src/renderer/MemoryTreeView.tsx`、`docs/taskbooks/memory-atom-vector-catalog-taskbook-2026-07-15.md` |
...
| 记忆管理控制面 | v2 写管理基础闭环；v3 同源读取与迁移控制已接通 | UI 操作真实运行时索引与注册表。v2 可查看、归档、恢复、删除记忆并管理资源/项目投影；v3 树概况只加载 D0/D1，D2 正文与 D3 证据/历史/关系按需展开，详情来自同一 Repository/atom/Catalog。迁移页支持无副作用预检、确认登记、取消、重启执行、失败恢复和安全回滚；v3 atom 移动、合并、失效、恢复和导出尚未开放 | `packages/app/src/renderer/MemoryTreeView.tsx`、`packages/app/src/renderer/memory-tree/`、`packages/app/src/main/memory-tree-control.ts`、`packages/app/src/main/memory-v3-migration-control.ts` |
...
| 核心 Agent 门 | 本轮未单独重复执行；核心契约包含在全量 1136 项测试中 | `pnpm.cmd run verify:core` |
| 全量测试 | 通过：145 个测试文件；1136 passed、1 skipped | `pnpm.cmd test` |
...
16. 阶段 6 读取与迁移生命周期已完成：树接口只返回 D0/D1，节点 D2/D3 通过可取消请求按需展开；详情缓存限制为 24 条。Repository management facade 可检查 Catalog、atom、Embedding、认识状态、证据和关系邻域；迁移页可预检、确认登记、取消、重启执行、恢复和受约束回滚。运行中的 API 不执行迁移，启动协调器在 Runner、Local App API、渠道插件、SQLite 与 Embedding 创建前处理 durable locator，并把配置同步到实际 backend。

仍未完成：

1. 完成 v3 atom 移动、合并、失效、恢复和导出的写管理能力；所有操作继续使用同一 Repository transaction，不建立 UI 数据副本。
2. 使用隔离的长时间运行继续验证 v3 写入、检索、向量维护、关闭释放、迁移恢复和重启恢复，不把工程测试冒充正式用户数据验收。
...
1. 继续 Memory v3 阶段 6：在已完成的启动前迁移生命周期上补齐 atom 移动、合并、失效、恢复和导出，并完成隔离长时间运行；正式切换继续等待用户明确批准。
