...
**当前阶段**：Memory v3 阶段 0-26、统一 Tool Execution Service、工具调用级与 TaskBook 步骤级有界并行、shadow Git 检查点、退出冻结、前台实时 Provider API 表达来源/去重闸门、回答级记忆连续性判断、会话摘要精确字段保真、运行时事件安全边界与 Renderer 生产入口、TaskBookPatch、Runner 检查点续跑、应用启动恢复控制面、活动任务快照与控制、托盘、三档关闭策略、逻辑容器权限闸门和 LS 开发环境版本管理均已有工程基线。语义活动 `respond / execute / clarify`、直接回应紧凑 Prompt、有界历史、选择性上一轮摘要、显式工具提议、结构验证、无进展熔断和工具循环后续请求紧凑化已通过完整质量门及真实 Electron Runner 验收。自包含且来源可证明为内置的单个 `glob / grep / read` 请求使用 `decide_explicit_tool` 紧凑契约；模型只返回 `input` 或澄清对象，工具名和执行任务书由 Runtime 锁定、展开并复核；多工具、写入、执行、附件、续接、记忆介入、恢复态和边界不明任务仍进入完整 `decide`。Local App API 的长生命周期 SSE 统一使用 15 秒注释心跳和 512 KiB 待写缓冲上限；普通 Agent run 或 Checkpoint 续跑的观察连接断开只结束观察，不取消 Main 持有的任务，终端主动命令仍保持断连取消。正式小时门已经通过，下一主线是校准含历史 Provider 工具消息的普通续轮 tokenizer，并继续非字段事实、真实外部副作用和长期真实用户负载验收。**
...
- 阶段 20 已退役 Memory v2 的 archive 摘要写入、旧 vector 装饰写入和 CLI archive adapter；既有 archive/vector 数据保持原样，只读兼容路径不能创建摘要、向量或 Atom。阶段 20 当时的验收基线为 180 个文件、1317 passed、1 skipped。2026-08-04 当前工作树为 262 个测试文件、1838 passed、1 skipped；仓库卫生 33/33、全量测试、27 包 typecheck、Electron build、恢复源检查、真实 DeepSeek 紧凑单只读工具、有界两步工具提议、后台任务、短时并行压力、跨重启回答连续性、多轮五字段摘要续答、三级压缩深度、主动断线恢复、120 秒诊断门和正式 2 小时持续任务验收均已通过。
...
| 全量测试 | 通过：262 个测试文件全部通过；测试项为 1838 passed、1 skipped，共 1839 项。新增覆盖统一 SSE 心跳、慢观察者缓冲上限、观察连接断开后 Main-owned run/Checkpoint 继续执行、连续性任意动态字段、紧凑单只读结果的调用审计关联、清洗/截断拒绝和审批边界，以及工具调用统一超时与清理、`exec` 有界输出和进程树关闭、完全访问下显式内置 `exec`、工具完成后的安全暂停边界、轻量 TaskBook 的执行安全契约、按 LS 最终回答识别多形态字段和值、输入栏与发送键等半径；既有摘要保真、恢复、权限、记忆和桌面回归继续通过 | `pnpm.cmd test`、`packages/app/src/main/local-app-api/http.test.ts`、`packages/app/src/main/{run-stream-api,run-checkpoint-api}.test.ts`、`packages/harness/src/response-continuity-dynamic-fields.test.ts`、`packages/app/src/renderer/ui-radius-consistency.test.ts`、其余 workspace 测试 |
