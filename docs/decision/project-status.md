...
**当前阶段：Memory v3 阶段 0-26、统一 Tool Execution Service、工具调用级与 TaskBook 步骤级有界并行、运行时事件安全边界与 Renderer 生产入口、TaskBookPatch、Runner 检查点续跑、应用启动恢复控制面、活动任务快照与控制、托盘、三档关闭策略、设置页“应用与后台”、shadow Git 检查点、退出冻结、前台实时 Provider API 表达来源/去重闸门、回答级记忆连续性判断、逻辑容器权限闸门和 LS 开发环境版本管理已有工程基线。语义活动 `respond / execute / clarify`、直接回应紧凑 Prompt、有界历史、选择性上一轮摘要、显式单只读工具两调用快路径、结构验证、无进展熔断和工具循环后续请求紧凑化已通过完整质量门及真实 Electron Runner 验收。当前 DeepSeek 模型四项真实校准、普通直接回答与显式单只读工具的 V4 本地精确 tokenizer 对账、真实 DeepSeek 跨重启最终回答连续性，以及隔离数据根中的真实两步副作用任务、Electron Checkpoint 和后台控制验收均已通过；下一主线是工具续轮 tokenizer 校准、持续/并行长任务、网络抖动与真实用户负载。**
...
- 阶段 20 已退役 Memory v2 的 archive 摘要写入、旧 vector 装饰写入和 CLI archive adapter；既有 archive/vector 数据保持原样，只读兼容路径不能创建摘要、向量或 Atom。阶段 20 当时的验收基线为 180 个文件、1317 passed、1 skipped。2026-08-03 当前工作树为 255 个测试文件、1746 passed、1 skipped；仓库卫生 33/33、全量测试、27 包 typecheck、Electron build、恢复源检查、真实 DeepSeek 显式单工具快路径、两步后台任务和跨重启回答连续性验收均已通过。
...
- 2026-08-02 23:10:30 工具循环完成第一轮请求紧凑化：首轮仍保留最多 8 条有界历史和必需附件 manifest；首次工具结果之后，后续模型请求只保留最近 2 条历史、附件 manifest、当前用户消息、系统/步骤契约以及已产生的工具证据。没有删除工具 schema、权限判断或工具结果。2026-08-03 的真实 DeepSeek 两步 `write -> read` 基线共 7 次模型请求，恢复任务耗时 `14.391s`；完成任务与记忆追问合计 Provider prompt `25,136`、completion `1,345`、total `26,481`。当前数字证明链路可测且可恢复，也表明后续仍需继续减少工具续轮请求和重复 Prompt；尚无严格前后对照，不宣称节省比例。
...
- 2026-08-01 已接入 DeepSeek V4 官方固定 revision tokenizer 资源和普通消息、thinking、工具定义的最终请求 framing。资源按固定大小与 SHA-256 下载后原子校验，计数使用真实 token id 数量；普通 `/run`、显式单只读工具两调用路径与 2026-08-03 最新真实跨重启回答验收均保持本地 prompt 与 Provider prompt 零差值。单工具两次请求为 `2934/2934`、`870/870`；跨重启三次请求为 `1003/1003`、`387/387`、`1285/1285`。DeepSeek V4 的 `auto` 现在显式发送 `thinking=disabled`，避免 Provider 缺省 framing 让本地计数来源不确定；含历史 `tool_calls`/`tool` 结果的真实工具续轮仍因尚未完成 Provider 校准而失败关闭 exact，响应完成后使用 Provider usage。OpenAI/GLM 继续明确为 unavailable，旧会话没有历史最终载荷时显示“本会话尚无本地计数”而不伪造数字。
...
| 全量测试 | 通过：255 个测试文件全部通过；测试项为 1746 passed、1 skipped。覆盖回答级记忆连续性、中文 Markdown 字段恢复、直接记忆追问、多个历史字段完整/部分回答、版本化摘要与 active/adopted Atom 显式字段恢复、否定旧值、显式失忆表述、复述全部旧值但同时否认记忆、显式单工具提议与 Runtime 二次校验、DeepSeek `auto` 显式关闭 thinking、`respond` ContextSnapshot、统一工具执行、TaskBook 步骤级并行、简单任务防过度 TaskBook、无进展熔断、结构验证快路径、恢复策略、活动任务控制、Checkpoint JSON 语义与旧 `null` 事件兼容、隐藏桌面验收 API、完全访问一次确认与免逐次审批、会话重命名、对话布局稳定、控件行单行约束、普通表面圆角 token、折叠图标 `3px` 像素对齐 SVG 清晰度例外和字体/图标对齐 | `pnpm.cmd test`、`packages/harness/src/response-continuity.test.ts`、`packages/harness/src/continuation-intent.test.ts`、`packages/harness/src/default-harness.test.ts`、`packages/harness/src/stages/{decide,execute,verify,recover,finalize}.test.ts`、`packages/context/src/tokenizers/deepseek-v4-{counter,encoding}.test.ts`、`packages/runner/src/run-checkpoint-store.test.ts`、`runtime-event-queue.test.ts`、`packages/app/src/main/desktop-acceptance-api.test.ts`、`packages/app/src/main/session-rename-api.test.ts`、`packages/app/src/renderer/chat-layout-stability.test.ts`、`packages/app/src/renderer/ui-radius-consistency.test.ts`、`packages/app/src/renderer/ui/icons.test.ts`、`packages/app/src/renderer/font-rendering.test.ts` |
...
| 真实 DeepSeek 显式单只读工具 | 通过：严格两次 API 调用、一次工具调用 | 请求顺序为 `decide → execute_final_reply`，两次请求都不使用 Provider 工具协议；DECIDE 仅收到用户明确点名的唯一工具 schema，并产生 `toolProposal`，Runtime 再做名称、schema、只读资源、权限和审批校验后通过统一 Tool Execution Service 执行。唯一 `glob` 成功，工作区摘要前后一致，VERIFY 使用结构门；最终回答来自真实 DeepSeek。两次本地/Provider prompt 均为 `exact_match`，隔离数据根已清理 | `pnpm.cmd run verify:electron-deepseek-single-tool`、`scripts/verify-electron-deepseek-single-tool.mjs`、`packages/harness/src/explicit-tool-instruction.ts`、`packages/harness/src/stages/execute/direct-tool-proposal.ts` |
...
| 真实 DeepSeek 后台两步任务 | 通过：9 个真实场景全部完成 | 隔离数据根和真实 Electron/DeepSeek 覆盖活动任务 SSE、关闭到托盘继续运行、运行时 profile 热重载、暂停检查点、强制终止、重启续跑、两步 `write -> read` 副作用、回答级记忆追问、中断检查点和彻底退出。TaskBook/执行均为 2 步，`write` 与 `read` 各成功 1 次且无需批准；恢复任务 7 次模型调用、`14.391s`，任务加追问 Provider total `26,481`。最终回答准确复述文件名和验收代号，连续性为 `supported`、`matchedSources=recent_history`；隔离数据已清理 | `pnpm.cmd run verify:electron-deepseek-background-task`、`scripts/verify-electron-deepseek-background-task.mjs` |
...
版本化 Summary Memory、附件清单优先与按需正文工具、T0 根索引、项目记忆投影、稳定项目 ID、路径重绑定、统一 `MemoryService`、数据根迁移和压缩阈值设置继续沿既有工程基线工作。统一 Tool Execution Service、工具调用级与 TaskBook 步骤级有界并行、显式单只读工具两调用路径、运行中事件安全消费、确定性 TaskBookPatch、Renderer 事件生产/反馈入口、Runner 显式续跑、应用启动恢复、活动任务控制、托盘、三档关闭策略和设置页后台控制入口已经实现。2026-08-03 的仓库卫生 33/33、255 个测试文件/1746 passed/1 skipped、27 包 workspace typecheck、Electron build、恢复源检查、真实 DeepSeek 单工具、两步后台副作用与跨重启回答验收均已通过；恢复检查仍明确报告部分旧 run 缺执行日志与可选 workspace artifact 索引缺失。这些证据仍不等于持续并行负载、其他 Provider 矩阵或生产发布已就绪。
...
1. 保持当前 DeepSeek chat、continuity、tool、abort、跨重启最终回答连续性、普通回答与显式单只读工具两调用路径的本地精确 token 对账为回归门；下一步校准含历史 `tool_calls`/`tool` 结果的续轮 framing。只有 OpenAI/GLM 实际配置并进入用户选择范围后，才为其执行同等真实校准和模型专用 tokenizer 验证。远程 Embedding 不纳入默认路径。
