...
1. 保持当前 DeepSeek chat、continuity、tool、abort 与本地精确 token 同请求对账为回归门，并用真实 Provider usage/耗时继续量化明确单工具任务的 Context 紧凑化收益；只有 OpenAI/GLM 实际配置并进入用户选择范围后，才为其执行同等真实校准和模型专用 tokenizer 验证。远程 Embedding 不纳入默认路径。
