...
- 阶段 20 已退役 Memory v2 的 archive 摘要写入、旧 vector 装饰写入和 CLI archive adapter；既有 archive/vector 数据保持原样，只读兼容路径不能创建摘要、向量或 Atom。阶段 20 当时的验收基线为 180 个文件、1317 passed、1 skipped。2026-08-04 当前工作树为 263 个测试文件、1856 passed、1 skipped；仓库卫生 33/33、全量测试、27 包 typecheck、Electron build、恢复源检查、Flash 工具协议 `15/15`、真实 DeepSeek 显式/自主单只读工具、确定性七场景、后台九场景、跨重启回答连续性、多轮五字段摘要续答、三级压缩深度、主动断线恢复、120 秒诊断门和正式 2 小时持续任务验收均已通过。
...
| 全量测试 | 通过：263 个测试文件全部通过；测试项为 1856 passed、1 skipped，共 1857 项。新增覆盖自主只读 DECIDE 的有界 schema 合同、Runtime 直执行、无效紧凑决策的合同错误分类、最终紧凑回答证据闸门和回答连续性 `not_applicable` 边界；既有 DeepSeek V4 Flash Provider 工具协议 framing、仅 `excluded` 记忆候选不阻断轻量路径、统一 SSE 心跳、慢观察者缓冲上限、观察连接断开后 Main-owned run/Checkpoint 继续执行、连续性任意动态字段、调用审计关联、清洗/截断拒绝和审批边界，以及工具调用统一超时与清理、`exec` 有界输出和进程树关闭、完全访问下显式内置 `exec`、工具完成后的安全暂停边界、轻量 TaskBook 的执行安全契约、按 LS 最终回答识别多形态字段和值、输入栏与发送键等半径继续通过 | `pnpm.cmd test`、`packages/context/src/tokenizers/deepseek-v4-{counter,encoding}.test.ts`、`packages/harness/src/compact-{autonomous-read-task,read-only-result}.test.ts`、`packages/harness/src/stages/{decide,execute}.test.ts`、`packages/app/src/main/local-app-api/http.test.ts`、`packages/harness/src/response-continuity-dynamic-fields.test.ts`、`packages/app/src/renderer/ui-radius-consistency.test.ts`、其余 workspace 测试 |
...
| 真实 DeepSeek 显式与自主单只读工具 | 通过：用户点名工具时走 `decide_explicit_tool`，只表达目标时由 DECIDE 在 builtin `glob / grep / read` 中自主选择；有界多工具/写入仍走完整 `decide` | 显式路径只让模型返回 `input` 或澄清对象，Runtime 锁定工具名并展开任务书；最新 `glob` 为 2 次 API、1 次工具、耗时 `2.201s`，DECIDE/final prompt `417/379`、全程 `796`、Provider total `837`。自主路径让 DECIDE 同时返回工具、参数、简短步骤与验收标准，Runtime 重验后直执行；固定 2 次 API、1 次 `glob`，耗时 `3.879s`，DECIDE/final prompt `744/376`、合计 `1,120`、Provider total `1,185`，两次均为 `exact_match`，回归总上限 `1,400`。两条路径都要求 builtin 来源、无审批、无写入/外部副作用、未清洗/截断，并由 Runtime 重验 schema、路径、权限、工具证据、SSE、结构 VERIFY 和调用审计；附件、历史指代、实际记忆介入、恢复、插件同名工具、写入/执行或边界不明任务回退完整流程 | `pnpm.cmd run verify:electron-deepseek-single-tool`、`pnpm.cmd run verify:electron-deepseek-autonomous-read`、`packages/harness/src/compact-{explicit-tool-decision,autonomous-read-task}.ts`、`compact-read-only-result.ts`、`stages/execute/direct-tool-proposal.ts` |
