...
执行状态：Memory v3 阶段 0-5 已完成；阶段 6 的同源 D0-D3 读取、迁移生命周期、atom 高级管理、首次自动选择、运行中 release 与 immutable fact 孤儿恢复已完成；正式路径仍为 v2，隔离长时间运行和正式用户数据切换尚未完成
...
因此，不建议继续在集中式 Memory v2 文档上叠加新能力。Memory v3 阶段 6 已完成直接读取同一 fact/atom/catalog 的 D0-D3 管理视图、“显式确认登记 -> 重启 -> 所有运行时写入者创建前迁移/回滚”的正式生命周期，以及 atom 高级管理、首次 working set、运行中 release 和 fact 孤儿恢复；失败保持原 backend，恢复和回滚状态持久可见。当前重点转为隔离长时间运行、真实窗口和正式用户批准后的迁移验收。真实 Provider 对话验收继续作为并行质量门，之后再推进统一 Tool Execution Service 与 RuntimeEventQueue。
...
状态：进行中。v2 的 Memory Service、T0-T3、资源目录、管理 UI 和项目投影已落地；v3 阶段 0-5 已在隔离目录实现契约、immutable facts、atom projections、journal、catalog、FTS、本地 Embedding、实体关系、双后端 facade、Repository transaction、认识状态分类、Memory Service/Runner 适配、统一检索、D0-D3、版本化 KnownState、重启恢复和安全迁移。阶段 6 已接通同源 D0-D3 读取、启动前迁移/回滚、atom 高级管理、首次 D1→D2 working set、运行中 release 和 fact 孤儿恢复；隔离长时间运行和正式用户数据切换尚未完成。
...
- 按 [原子记忆与内置向量目录任务书 2026-07-15](../taskbooks/memory-atom-vector-catalog-taskbook-2026-07-15.md) 以 immutable facts 保存事件真相、以语义 atom 形成可治理投影，并用可重建 SQLite catalog 管理 parent、实体、有向关系、FTS、向量和恢复投影；
...
仓库基元化阶段 0-7 与 Memory v3 阶段 0-5 已完成，阶段 6 的读取、迁移生命周期、atom 高级管理、working set 和事实孤儿恢复也已完成。建议现在先做隔离长时间运行与真实窗口验收；正式用户数据切换继续等待用户明确批准。同时进行 Context Engine 的真实供应商对话校准，之后收敛 Tool Execution Service，而不是提前扩张新插件类型或无关 UI 范围。推进时持续遵守：
