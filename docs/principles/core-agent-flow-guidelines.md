...
- 运行结束时的记忆沉淀已随极简方案删除：EVOLVE 编排、自动 Skill 创建和 legacy-per-run 的 CAPTURE 总结都不再存在。持久记忆只由明确写入（经统一校验与写入闸门）和压缩路径产生，任何路径都不能直接写入旧式扁平文件。
...
- Runner 只持有一个索引优先记忆运行时实例，供提示词装配、单一 `memory_tree` 导航入口与明确写入、缓存失效、执行日志和设置概览共同使用。
...
- 明确写入只提交结构化记忆内容；模型不得自报 verified 或 authority。运行时会验证 asserted source，证据不足时降级为 LS 自身的未验证陈述并丢弃不可信主体 id/label；`invalidate` 与 `conflict` 只延期审计而不直接修改记忆。`PHILOSOPHY.md` 已作为显式理念资源注册，只沿索引按任务相关性和预算展开，不进入常驻 Prompt bootstrap。
