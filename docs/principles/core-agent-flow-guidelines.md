...
- 记忆持久层分为 immutable facts 与 atom projections：事件原始事实只追加且不修改；atom 用于层级、相关性、检索和治理，可移动、合并、失效或恢复。Recovery journal 是有界恢复队列，不是事实真相源。
...
- 影响记忆的用户纠正、工具/VERIFY 证据、项目/资源变化、能力变化、冲突处理和时间到期统一生成版本化 `MemoryUpdateEvent`。事件先写入不可变 fact，再登记到可恢复 journal，随后幂等更新 atom 与 catalog；fact 落盘后即使在 journal 登记前崩溃，也必须在重启时自动补投影。处理失败进入恢复状态，禁止只更新内存或静默覆盖原事实。
