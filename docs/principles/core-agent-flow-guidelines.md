...
- 回答级记忆连续性的本地结构门已经完成；后续必须用真实 DeepSeek 长会话覆盖“先记住、后转题、再续答”、摘要压缩、执行中按索引补取、记忆释放后不再承接和跨重启恢复。真实验收需要同时检查最终回答、ContextSnapshot、命中来源与执行日志，不能只凭回答听起来连贯就判定通过。
